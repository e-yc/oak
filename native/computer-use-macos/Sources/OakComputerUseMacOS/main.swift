...
import Foundation
import ImageIO
import OakComputerUseMacOSCore
import ScreenCaptureKit

private let providerName = "oak-computer-use-macos"
    ...
    private func buildSnapshot(
        ...
        guard accessibilityTrusted() else {
            ...
            throw ProviderError.coded(
                "permission_denied",
                "Accessibility permission is required for Oak Computer Use. Run `oak computer permissions` or open Settings > Computer Use, grant Accessibility to Oak Computer Use, then retry."
            )
        }
        ...
        let screenshotStatus: ScreenshotStatus = if screenshot != nil {
            ...
            .failed("Screen Recording permission is required for Oak Computer Use; grant permission or pass --no-screenshot to inspect accessibility state only.")
...
private func focusedWindow(appElement: AXUIElement, app: AppDescriptor, visibleWindowCount: Int, allowRecovery: Bool) throws -> AXUIElement {
    ...
    let permissionHint = visibleWindowCount > 0
        ? " The app has visible windows, so macOS Accessibility may need Oak Computer Use toggled off and on again in System Settings."
        : ""
...
private struct WindowCapture {
    ...
    private static func captureImage(windowId: CGWindowID, bounds: CGRect) -> CapturedImage? {
        if ProcessInfo.processInfo.environment["OAK_COMPUTER_USE_SCK_SCREENSHOTS"] == "1",
...
private final class PermissionWindowController: NSWindowController {
    ...
    convenience init(initialPermission: PermissionKind? = nil, terminateWhenDragAssistantCloses: Bool = false) {
        ...
        window.title = "Enable Oak Computer Use"
...
private enum PermissionKind: CaseIterable {
    ...
    var dragInstruction: String {
        switch self {
        case .accessibility:
            "Drag Oak Computer Use into the list above to allow Accessibility."
        case .screenshots:
            "Drag Oak Computer Use into the list above to allow Screenshots."
        }
    }
...
private final class PermissionView: NSView {
    ...
    private func build() {
        ...
        let title = label(ready ? "Computer Use is Ready" : "Enable Oak Computer Use", size: 22, weight: .bold)
        let subtitle = label(
            ready ? "Oak can use local apps when you ask." : "Grant permissions so Oak can use apps when you ask.",
            size: 12,
            weight: .regular
        )
    ...
    private func permissionRow(permission: PermissionKind, action: @escaping () -> Void) -> NSView {
        ...
        objc_setAssociatedObject(button, "oak-action", target, .OBJC_ASSOCIATION_RETAIN_NONATOMIC)
...
private final class PermissionDragAssistantController: NSWindowController {
    ...
    convenience init(
        ...
        window.title = "Drag Oak Computer Use"
...
private final class PermissionDragAssistantView: NSView {
    ...
    private func build() {
        ...
        objc_setAssociatedObject(closeButton, "oak-action", target, .OBJC_ASSOCIATION_RETAIN_NONATOMIC)
...
private final class DraggableAppTile: NSView, NSDraggingSource {
    ...
    private func build() {
        ...
        let title = NSTextField(labelWithString: "Oak Computer Use")
...
private func isAuthorizedAgentPeer(_ pid: pid_t) -> Bool {
    ...
    if isTrustedOakApplication(pid) {
        return true
    }
    guard let parentPid = parentProcessId(pid) else { return false }
    return isTrustedOakApplication(parentPid)
}

private func isTrustedOakApplication(_ pid: pid_t) -> Bool {
    guard let app = NSRunningApplication(processIdentifier: pid),
    ...
    // Oak-owned bundle ids; the sidecar peer check must still authorize them.
    return bundleId == "com.stablyai.oak" ||
        bundleId.hasPrefix("com.stablyai.oak.dev.") ||
        bundleId == "com.github.Electron"
}
...
private func runAgent(socketPath: String, token: String?) {
    ...
    setenv("OAK_COMPUTER_USE_SCK_SCREENSHOTS", "1", 1)
    app.run()
}
...
private func runStdio() {
    fputs("Oak Computer Use provider must be launched by Oak in app-agent mode.\n", stderr)
    exit(13)
}
...
if arguments.first == "--agent" {
    guard arguments.count >= 2 else {
        fputs("usage: oak-computer-use-macos --agent <socket-path> --token-file <token-path>\n", stderr)
        exit(2)
    }
    ...
    guard let token, !token.isEmpty else {
        fputs("oak-computer-use-macos --agent requires a non-empty --token-file\n", stderr)
        exit(2)
    }
    ...
    guard arguments.count >= 2 else {
        fputs("usage: oak-computer-use-macos --permission-status-file <path>\n", stderr)
        exit(2)
    }
