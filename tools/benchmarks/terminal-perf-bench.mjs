...
function createLocalRepoFixture() {
  const baseDir = mkdtempSync(path.join(os.tmpdir(), 'oak-termperf-'))
  const repoPath = path.join(baseDir, 'repo')
  mkdirSync(repoPath, { recursive: true })
  git(repoPath, 'init', '--initial-branch=main')
  git(repoPath, 'config', 'user.email', 'bench@oak.local')
  git(repoPath, 'config', 'user.name', 'Oak Bench')
...
async function main() {
  ...
  try {
    ...
    if (launched) {
      ...
      // Main-process phase attribution (requires OAK_PTY_SPAWN_TIMING=1 in the
