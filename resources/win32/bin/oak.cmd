...
REM launcher resolves APP_DIR back to resources/ and `oak open` cannot find
REM Oak.exe on packaged Windows installs.
for %%I in ("%RESOURCES_DIR%\..") do set "APP_DIR=%%~fI"
set "ELECTRON=%APP_DIR%\Oak.exe"

if not exist "%ELECTRON%" (
  echo Unable to locate Oak.exe next to "%RESOURCES_DIR%" 1>&2
  exit /b 1
)

REM Why: Oak packages the CLI entrypoint outside app.asar so the public shell
