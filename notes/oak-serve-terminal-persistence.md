# Oak Serve Terminal Persistence

## Problem

`oak serve` exposes terminal tabs to paired web clients through the runtime
...
- Persist `oak serve` terminal tab, leaf, and PTY/session bindings in the
...
- Do not make browser panes supported in headless `oak serve`.
