...
React Native companion app for Oak. Monitor worktrees, view terminal output, and send commands from your phone.

Local development uses two processes:

- Oak desktop/Electron from the repo root. This hosts the mobile WebSocket RPC server on port `6768`.
...
oak fill --element @e1 --value "ls" --json
oak screenshot --json
```
...
```bash
cd mobile
OAK_MOBILE_WS_URL=ws://127.0.0.1:6768 pnpm exec tsx scripts/test-subscribe.ts <deviceToken> <serverPublicKeyB64>
```
...
```bash
cd mobile
OAK_MOBILE_WS_URL=ws://127.0.0.1:6768 pnpm exec tsx scripts/repro-terminal-colors.ts \
...
Develop the mobile app without a running Oak desktop instance:
...
## Connecting to Real Oak

1. Start Oak desktop with WebSocket transport enabled
2. In Oak, go to Settings > Mobile and scan the QR code with this app
