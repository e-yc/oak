...
const REPO_NAMES = ['oak', 'dashboard', 'mobile', 'runtime', 'docs', 'api', 'desktop', 'site']
const WORKTREE_NAMES = ['manta', 'narwhal', 'otter', 'squid', 'turtle', 'beluga', 'marlin', 'oak']
...
export function createMockRepos(count: number): MockRepo[] {
  return Array.from({ length: count }, (_, index) => {
    ...
    return {
      id: `repo-${index + 1}`,
      displayName,
      path: `/tmp/oak-mobile-repro/${displayName}`,
