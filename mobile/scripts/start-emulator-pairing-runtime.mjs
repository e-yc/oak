...
export async function startHeadlessPairingRuntime({
  ...
  const runDir = mkdtempSync(path.join(os.tmpdir(), 'oak-mobile-run.'))
  ...
  const child = spawn(
    ...
    ['serve', '--mobile-pairing', '--pairing-address', pairingAddress, '--json'],
    {
      ...
      env: {
        ...process.env,
        OAK_E2E_USER_DATA_DIR: userData
      },
...
export async function registerWorktreeForPairingRuntime(runtime, worktree, tools) {
  ...
  await tools.oak(['repo', 'add', '--path', worktree, '--json'], {
    cwd: worktree,
    env: runtime.env,
    timeout: 60000
  })
