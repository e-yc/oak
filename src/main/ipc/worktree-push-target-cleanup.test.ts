...
const FORK_URL = 'git@github.com:contributor/oak.git'
const FORK_REMOTE = 'pr-contributor-oak'
...
describe('cleanupUnusedWorktreePushTargetRemoteWithExec', () => {
  it('removes an Oak-created fork remote that nothing else uses', async () => {
    const exec = makeExec()
  ...
  it('keeps a remote Oak did not create (remoteCreated falsy)', async () => {
    const exec = makeExec()
    await cleanupUnusedWorktreePushTargetRemoteWithExec(
      REPO_PATH,
      'repo-1::/wt/a',
  ...
  it('keeps the remote when its URL no longer matches the fork (repurposed by the user)', async () => {
    const exec = makeExec({ getUrl: 'git@github.com:someone-else/oak.git' })
    await cleanupUnusedWorktreePushTargetRemoteWithExec(
      REPO_PATH,
      'repo-1::/wt/a',
      forkTarget(),
...
describe('sameGitHubRemoteUrl', () => {
  it('matches SSH and HTTPS forms of the same GitHub fork', () => {
    expect(
      sameGitHubRemoteUrl(
        'git@github.com:contributor/oak.git',
        'https://github.com/contributor/oak.git'
      )
    ).toBe(true)
  })
  ...
  it('does not match different forks', () => {
    expect(
      sameGitHubRemoteUrl(
        'git@github.com:contributor/oak.git',
        'git@github.com:someone-else/oak.git'
      )
    ).toBe(false)
  })
  ...
  it('falls back to exact equality for non-GitHub hosts', () => {
    expect(
      sameGitHubRemoteUrl(
        'git@gitlab.com:contributor/oak.git',
        'git@gitlab.com:contributor/oak.git'
      )
    ).toBe(true)
    expect(
      sameGitHubRemoteUrl(
        'git@gitlab.com:contributor/oak.git',
        'https://gitlab.com/contributor/oak.git'
      )
    ).toBe(false)
  })
})
