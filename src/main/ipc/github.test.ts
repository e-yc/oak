...
describe('registerGitHubHandlers', () => {
  ...
  it('emits app_starred_oak once after a successful star with cohort context', async () => {
    starOakMock.mockResolvedValue(true)
    getCohortAtEmitMock.mockReturnValue({ nth_repo_added: 3 })

    registerGitHubHandlers(store as never, stats as never)

    await expect(handlers['gh:starOak'](null, 'settings')).resolves.toBe(true)

    expect(starOakMock).toHaveBeenCalledTimes(1)
  ...
  it('accepts every app star source for success telemetry', async () => {
    ...
    for (const source of [
      ...
      await expect(handlers['gh:starOak'](null, source)).resolves.toBe(true)
    }
  ...
  it('does not emit app_starred_oak when the star action returns false', async () => {
    starOakMock.mockResolvedValue(false)

    registerGitHubHandlers(store as never, stats as never)

    await expect(handlers['gh:starOak'](null, 'landing')).resolves.toBe(false)
  ...
  it('does not emit app_starred_oak when the star action throws', async () => {
    starOakMock.mockRejectedValue(new Error('gh failed'))

    registerGitHubHandlers(store as never, stats as never)

    await expect(handlers['gh:starOak'](null, 'star_nag')).rejects.toThrow('gh failed')
  ...
  it('preserves star result but skips telemetry for an invalid IPC source', async () => {
    ...
    await expect(handlers['gh:starOak'](null, 'github_website')).resolves.toBe(true)
