...
describe('diagnostics IPC handlers', () => {
  beforeEach(() => {
    ...
    delete (globalThis as { OAK_BUILD_IDENTITY?: unknown }).OAK_BUILD_IDENTITY
    delete (globalThis as { OAK_DIAGNOSTICS_TOKEN_URL?: unknown }).OAK_DIAGNOSTICS_TOKEN_URL
    process.env.OAK_DIAGNOSTICS_TOKEN_URL = 'https://diagnostics.example.com/diagnostics/token'
  ...
  it('pins official builds to the compile-time diagnostics endpoint', async () => {
    ...
    const globalOverrides = globalThis as {
      OAK_BUILD_IDENTITY?: 'stable'
      OAK_DIAGNOSTICS_TOKEN_URL?: string
    }
    globalOverrides.OAK_BUILD_IDENTITY = 'stable'
    globalOverrides.OAK_DIAGNOSTICS_TOKEN_URL = 'https://official.example.com/diagnostics/token'
    process.env.OAK_DIAGNOSTICS_TOKEN_URL = 'https://attacker.example.com/diagnostics/token'
    collectDiagnosticBundleMock.mockReturnValue(bundle)
    readFileSyncMock.mockReturnValue(bundle.payload)
    const collect = handlers.get('diagnostics:collectBundle')!
    const openPreview = handlers.get('diagnostics:openBundlePreview')!
