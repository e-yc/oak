...
export async function importExternalPathsSsh(
  ...
  try {
    if (options?.ensureDir) {
      // Why: terminal-drop staging needs `${worktree}/.oak/drops` to exist
      ...
      // .oak/.gitignore marker only when absent prevents clobbering user-
      // authored patterns. .oak/ is reserved as Oak-owned remote state;
...
async function ensureDropStagingDir(sftp: SFTPWrapper, destDir: string): Promise<void> {
  // destDir is a posix remote path, expected to be `${worktreePath}/.oak/drops`.
  ...
  if (!(await sftpPathExists(sftp, gitignorePath))) {
    // Why: negate the marker so .oak/.gitignore itself is trackable if we
