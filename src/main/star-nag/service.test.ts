...
vi.mock('../github/client', () => ({
  checkOakStarred: checkOakStarredMock,
  starOak: starOakMock
}))
...
describe('StarNagService', () => {
  ...
  beforeEach(() => {
    ...
    checkOakStarredMock.mockReset()
    checkOakStarredMock.mockResolvedValue(false)
    starOakMock.mockReset()
    starOakMock.mockResolvedValue(true)
    trackMock.mockReset()
  ...
  it('shows the browser fallback when checkOakStarred cannot determine star state', async () => {
    const window = createWindow()
    browserWindowMock.getAllWindows.mockReturnValue([window])
    checkOakStarredMock.mockResolvedValue(null)
    const { service, emitAgentStarted } = createHarness()
  ...
  it('does not log a threshold exposure when checkOakStarred returns true', async () => {
    const window = createWindow()
    browserWindowMock.getAllWindows.mockReturnValue([window])
    checkOakStarredMock.mockResolvedValue(true)
    const { service, emitAgentStarted } = createHarness()
  ...
  it('hides a superseded visible card when onboarding completion detects an existing star', async () => {
    const window = createWindow()
    browserWindowMock.getAllWindows.mockReturnValue([window])
    checkOakStarredMock.mockResolvedValueOnce(true)
    const { service, ui } = createHarness()
  ...
  it('queues onboarding completed while a threshold star check is in flight', async () => {
    ...
    checkOakStarredMock.mockReturnValueOnce(deferredStarCheck.promise).mockResolvedValueOnce(null)
    const { service, emitAgentStarted, ui } = createHarness()
  ...
  it('keeps threshold source when force_show is requested during a successful threshold evaluation', async () => {
    ...
    browserWindowMock.getAllWindows.mockReturnValue([window])
    const deferredStarCheck = createDeferred<boolean | null>()
    checkOakStarredMock.mockReturnValue(deferredStarCheck.promise)
    const { service, emitAgentStarted } = createHarness()
  ...
  it('does not replay a stale queued force_show after threshold delivery wins', async () => {
    ...
    checkOakStarredMock.mockReturnValueOnce(firstStarCheck.promise).mockResolvedValue(null)
    const { service, emitAgentStarted } = createHarness()
  ...
  it('emits direct-star attempted and succeeded outcomes plus app_starred_oak', async () => {
    const window = createWindow()
    browserWindowMock.getAllWindows.mockReturnValue([window])
    const { service, ui } = createHarness()
    ...
    service.registerIpcHandlers()
    getIpcHandler('star-nag:forceShow')()
    const ok = await getIpcHandler('star-nag:starOak')()
    ...
    expect(trackMock).toHaveBeenCalledWith('app_starred_oak', {
      source: 'star_nag',
      nth_repo_added: 3
    })
  })
  ...
  it('uses the source moment for confirmed direct-star success telemetry', async () => {
    ...
    service.registerIpcHandlers()
    await getIpcHandler('star-nag:onboardingCompleted')()
    await getIpcHandler('star-nag:starOak')()
  ...
  it('uses fresh cohort context for canonical app_starred_oak success telemetry', async () => {
    const window = createWindow()
    browserWindowMock.getAllWindows.mockReturnValue([window])
    getCohortAtEmitMock
  ...
  it('records success and completion when direct star resolves after dismissal cleared the visible session', async () => {
    ...
    starOakMock.mockReturnValue(deferredStar.promise)
    const { service, ui } = createHarness()

    service.registerIpcHandlers()
    getIpcHandler('star-nag:forceShow')()
    const starPromise = getIpcHandler('star-nag:starOak')()
    getIpcHandler('star-nag:dismiss')()
  ...
  it('clears the in-flight direct-star guard after thrown attempts so the user can retry', async () => {
    const window = createWindow()
    browserWindowMock.getAllWindows.mockReturnValue([window])
    starOakMock.mockRejectedValueOnce(new Error('gh failed')).mockResolvedValueOnce(true)
    const { service, ui } = createHarness()

    service.registerIpcHandlers()
    getIpcHandler('star-nag:forceShow')()
    const starFromNag = getIpcHandler('star-nag:starOak')

    await expect(starFromNag()).rejects.toThrow('gh failed')
    await expect(starFromNag()).resolves.toBe(true)

    expect(starOakMock).toHaveBeenCalledTimes(2)
    expect(ui.starNagCompleted).toBe(true)
  })
