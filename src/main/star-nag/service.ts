...
import { checkOakStarred } from '../github/client'
...
export class StarNagService {
  ...
  registerIpcHandlers(): void {
    ...
    ipcMain.handle('star-nag:starOak', () => this.starOakFromNag())
  ...
  private async maybeShow(
    ...
    try {
      // Why: checkOakStarred lets us skip users who already starred outside
      ...
      // button.
      const starred = await checkOakStarred()
  ...
  private async starOakFromNag(): Promise<boolean> {
    const session = this.promptSession
    if (!session) {
      return false
    }
    ...
    const attempt = this.runStarOakAttempt(session)
  ...
  private async runStarOakAttempt(session: StarNagPromptSession): Promise<boolean> {
