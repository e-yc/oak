...
const HERMES_PLUGIN_NAME = 'oak-status'
const HERMES_PLUGIN_MARKER = 'Managed by Oak. Do not edit; changes may be overwritten.'
...
function enablePlugin(config: HermesConfig): HermesConfig {
  ...
  if (disabled === null) {
    ...
    // so Oak's install status matches what the real Hermes loader will do.
    plugins.disabled = []
...
function getPluginFilesState(pluginDir = getPluginDir()): {
  ...
  try {
    ...
    return {
      present: true,
      managed,
      detail: managed ? null : 'Hermes oak-status plugin exists but is not Oak-managed'
    }
  } catch (error) {
    return {
      present: true,
...
function buildStatus(configPath: string, config: HermesConfig): AgentHookInstallStatus {
  ...
  const details = [
    pluginFiles.detail,
    enablement.detail,
    !enablement.enabled ? 'oak-status is not enabled in Hermes config.yaml' : null,
    enablement.disabled ? 'oak-status is disabled in Hermes config.yaml' : null
...
function getPluginManifest(): string {
  return [
    ...
    'version: 1.0.0',
    'description: "Reports Hermes Agent lifecycle events to Oak."',
    'author: "Oak"',
    'kind: standalone',
    'provides_hooks:',
...
def _endpoint_env() -> dict[str, str]:
    env = dict(os.environ)
    endpoint = env.get("OAK_AGENT_HOOK_ENDPOINT", "")
...
def _post_to_oak(payload: dict[str, Any]) -> None:
    env = _endpoint_env()
    port = env.get("OAK_AGENT_HOOK_PORT", "")
    token = env.get("OAK_AGENT_HOOK_TOKEN", "")
    pane_key = env.get("OAK_PANE_KEY", "")
    if not port or not token or not pane_key:
        return
    body = {
        "paneKey": pane_key,
        "launchToken": env.get("OAK_AGENT_LAUNCH_TOKEN", ""),
        "tabId": env.get("OAK_TAB_ID", ""),
        "worktreeId": env.get("OAK_WORKTREE_ID", ""),
        "env": env.get("OAK_AGENT_HOOK_ENV", ""),
        "version": env.get("OAK_AGENT_HOOK_VERSION", ""),
        "payload": payload,
    }
...
def _make_hook(event_name: str) -> Callable[..., None]:
    def _hook(**kwargs: Any) -> None:
        _post_to_oak(_payload_for_event(event_name, kwargs))
