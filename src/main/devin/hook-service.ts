...
import {
  ...
  readConfigFromOakOverlapDetail,
...
function getManagedScript(target: 'local' | 'posix' = 'local'): string {
  ...
  return [
    ...
    'curl -sS -X POST "http://127.0.0.1:${OAK_AGENT_HOOK_PORT}/hook/devin" \\',
...
export class DevinHookService {
  getStatus(): AgentHookInstallStatus {
    ...
    return {
      ...
      configPath,
      managedHooksPresent,
      detail: mergeHookInstallDetail(detail, readConfigFromOakOverlapDetail(config))
    }
  }
  ...
  // Why: install Oak's Devin hook settings on the remote box rather than the
