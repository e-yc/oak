...
describe('OpenCodeUsageStore', () => {
  ...
  it('reports no data for Oak scope when only non-Oak OpenCode usage exists', async () => {
  ...
  it('uses recorded OpenCode costs and token totals without model pricing inference', async () => {
    ...
    const summary = await store.getSummary('oak', '30d')
    const daily = await store.getDaily('oak', '30d')
  ...
  it('returns recent sessions with OpenCode event and token fields', async () => {
    ...
    const sessions = await store.getRecentSessions('oak', '30d', 5)
