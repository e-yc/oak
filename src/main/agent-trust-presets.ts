 ...
 * first launch.
 *
 * Why: Oak's "drop URL into agent input as a draft" flow injects the URL
...
export function markCodexProjectTrusted(workspacePath: string): void {
  ...
  // Why: Oak-launched Codex runs with an Oak-owned CODEX_HOME, so the trust
  // preset must also update the runtime config Codex will actually read.
  upsertProjectTrustLevel(join(getOakManagedCodexHomePath(), 'config.toml'), absPath, 'trusted')
}
...
function canonicalize(p: string): string {
  ...
  // (oak caches realpath()'d worktree paths) matches the agent's lookup.
  try {
    if (existsSync(p)) {
