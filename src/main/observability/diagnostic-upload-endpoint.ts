...
export function resolveDiagnosticBuildTokenEndpoint(): string | null {
  const endpoint =
    typeof OAK_DIAGNOSTICS_TOKEN_URL !== 'undefined'
      ? OAK_DIAGNOSTICS_TOKEN_URL
      : ((globalThis as { OAK_DIAGNOSTICS_TOKEN_URL?: string | null }).OAK_DIAGNOSTICS_TOKEN_URL ??
        null)
...
export function resolveDiagnosticBuildIdentity(): 'stable' | 'rc' | null {
  const ident =
    typeof OAK_BUILD_IDENTITY !== 'undefined'
      ? OAK_BUILD_IDENTITY
      : ((globalThis as { OAK_BUILD_IDENTITY?: 'stable' | 'rc' | null }).OAK_BUILD_IDENTITY ?? null)
...
export function resolveDiagnosticTokenEndpoint(): string | null {
  ...
  // cannot redirect uploads that the UI labels as going to Oak support.
  if (resolveDiagnosticBuildIdentity()) {
    return buildEndpoint
  }
  const fromEnv = process.env.OAK_DIAGNOSTICS_TOKEN_URL
...
export function resolveDiagnosticOakChannel(): 'stable' | 'rc' | 'dev' {
