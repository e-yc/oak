...
contract for Codex inside Oak. Keeping path resolution, system-default
...
export class CodexRuntimeHomeService {
  ...
  // Why: tracks the auth.json content Oak last wrote to the runtime CODEX_HOME.
  // Between syncs, if the file differs, Codex CLI refreshed the token — so
  // Oak writes back the refreshed token to managed storage before overwriting.
  // On managed→system-default transition, if the file differs, an external
  // login (e.g. `codex auth login`) overwrote it — so Oak adopts the file as
  ...
  private initializeLastSyncedState(): void {
    ...
    // transition and can restore/delete host auth that Oak never touched.
  ...
  syncForCurrentSelection(target?: CodexAccountSelectionTarget): void {
    ...
    if (!activeAccount) {
      ...
      if (this.lastSyncedAccountId !== null) {
        ...
        if (logoutMarkerStatus.kind === 'applies') {
          ...
          // Why: Oak-launched Codex sessions now use an Oak-owned CODEX_HOME
          // even when no managed account is selected. Seed that runtime home
          // from the user's current system-default auth once so dev/prod Oak
    ...
    // If we detect the runtime file differs from what Oak last wrote, the CLI
  ...
  private readBackRefreshedTokensFromPath(
    ...
    try {
      ...
      // Why: after app restart, Oak has no last-written baseline. Identity
  ...
  private getWslRuntimeHomePath(distro: string): string | null {
    const home = getWslHome(distro)
    return home
      ? this.joinWslPath(home, '.local', 'share', 'oak', 'codex-runtime-home', 'home')
      : null
  }
  ...
  private runtimeAuthMatchesAccount(
    ...
    // still match the account Oak believes is selected.
  ...
  private runtimeAuthMatchesSystemDefaultIdentity(
    ...
    // system-default identity Oak mirrored earlier.
    if (
  ...
  private getRuntimeHomePath(): string {
    return getOakManagedCodexHomePath()
  }
  ...
  private getLegacyManagedHomes(): string[] {
    ...
    for (const entry of accountEntries) {
      ...
      if (existsSync(join(managedHomePath, '.oak-managed-home'))) {
  ...
  private getPreservedLegacySessionPath(runtimeFilePath: string, accountId: string): string {
    ...
    return `${basename}.oak-legacy-${accountId}${extension}`
  }
    ...
    try {
      ...
      if (runtimeAuth !== systemDefaultAuth) {
        ...
        if (
          ...
          // Why: system-default Codex now refreshes tokens inside Oak's
        ...
        // external logins/logouts into Oak's runtime home so ordinary Oak
  ...
  private restoreSystemDefaultSnapshot(options: { detectExternalLogin: boolean }): void {
    ...
    if (options.detectExternalLogin && !existsSync(runtimeAuthPath)) {
      // Why: once Oak owns the runtime CODEX_HOME, deleting auth.json there is
      // a local logout signal for Oak-launched Codex sessions, not a reason to
  ...
  private clearRuntimeAuthAfterSystemDefaultLogout(runtimeAuthPath: string): void {
    // Why: when the real ~/.codex auth disappears, Oak should treat that as an
    // external logout for unmanaged sessions, even if runtime auth had already
    // refreshed inside Oak's CODEX_HOME.
    rmSync(runtimeAuthPath, { force: true })
    this.captureSystemDefaultSnapshot({ force: true })
    this.persistRuntimeLogoutMarker()
    this.lastWrittenAuthJson = null
