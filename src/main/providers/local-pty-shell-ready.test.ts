...
describe('scanForShellReady', () => {
  it('flushes marker-like output when the full marker is not BEL-terminated', () => {
    ...
    expect(scanForShellReady(state, 'before \x1b]777;oak-shell-readyx')).toEqual({
      output: 'before \x1b]777;oak-shell-readyx',
      matched: false,
  ...
  it('reports post-marker bytes only when bytes follow the BEL terminator in the matching call', () => {
    ...
    expect(scanForShellReady(state, 'before \x1b]777;oak-shell-ready\x07')).toEqual({
      output: 'before ',
      matched: true,
      postMarkerBytesObserved: false
    })
    ...
    state = createShellReadyScanState()
    expect(scanForShellReady(state, 'before \x1b]777;oak-shell-ready')).toEqual({
      output: 'before ',
      matched: false,
      postMarkerBytesObserved: false
    })
    ...
    state = createShellReadyScanState()
    expect(scanForShellReady(state, '\x1b]777;oak-shell-ready')).toEqual({
      output: '',
      matched: false,
      postMarkerBytesObserved: false
    })
...
describePosix('local PTY shell-ready launch config', () => {
  ...
  it('falls back to HOME for OAK_ORIG_ZDOTDIR when inherited ZDOTDIR points at a wrapper dir', async () => {
    ...
    // PTYs spawned by the renderer/local provider when Oak is launched from
    // inside an Oak terminal (e.g. `pn dev`).
...
describePosix('live zsh subprocess tests', () => {
  ...
  describeIfZsh('ZDOTDIR discovery with real zsh', () => {
    ...
    beforeEach(async () => {
      testHome = mkdtempSync(join(tmpdir(), 'oak-zsh-test-home-'))
      userDataPath = mkdtempSync(join(tmpdir(), 'oak-zsh-test-userdata-'))
      getUserDataPathMock.mockReturnValue(userDataPath)
    })
...
`
      ...
      // Build clean env: use wrapper ZDOTDIR but let wrapper discover OAK_ORIG_ZDOTDIR at runtime
      ...
      delete cleanEnv.OAK_ORIG_ZDOTDIR
      ...
      delete cleanEnv.OAK_ATTRIBUTION_SHIM_DIR
      cleanEnv.ZDOTDIR = config.env.ZDOTDIR // Point to Oak wrapper dir
      ...
      const result = spawnSync(
        ...
        [
          '-i',
          '-c',
          'echo "OAK_ORIG_ZDOTDIR=${OAK_ORIG_ZDOTDIR}" && echo "PATH_HAS_CUSTOM=${PATH%%:*}"'
        ],
        {
      ...
      expect(output).toContain(`OAK_ORIG_ZDOTDIR=${xdgZshDir}`)
    ...
    it('preserves top-level .zshenv path and function side effects', async () => {
      ...
      // Dropping those side effects regresses non-Oak zsh startup semantics.
      const xdgZshDir = join(testHome, '.config', 'zsh')
      mkdirSync(xdgZshDir, { recursive: true })
      writeFileSync(
        join(testHome, '.zshenv'),
...
oak_zshenv_func() { echo "from-zshenv-function"; }
export ZDOTDIR="$HOME/.config/zsh"
`
      )
      ...
      const result = spawnSync(
        'zsh',
        ['-c', 'echo "PATH_HEAD=${PATH%%:*}" && echo "MY_VAR=${MY_VAR:-unset}" && oak_zshenv_func'],
        {
          env: cleanEnv as NodeJS.ProcessEnv,
          encoding: 'utf8'
        }
...
`
      ...
      const result = spawnSync(
        'zsh',
        ['-c', 'echo "survived" && echo "OAK_ORIG_ZDOTDIR=${OAK_ORIG_ZDOTDIR}"'],
        {
          env: cleanEnv as NodeJS.ProcessEnv,
          encoding: 'utf8'
        }
      ...
      expect(result.stdout).toContain(`OAK_ORIG_ZDOTDIR=${testHome}`)
    })
...
`
      ...
      const result = spawnSync('zsh', ['-c', 'echo "OAK_ORIG_ZDOTDIR=${OAK_ORIG_ZDOTDIR}"'], {
        env: cleanEnv as NodeJS.ProcessEnv,
        encoding: 'utf8'
      })
  ...
  describeIfZsh('high-priority edge cases', () => {
    ...
    beforeEach(async () => {
      testHome = mkdtempSync(join(tmpdir(), 'oak-zsh-edge-'))
      userDataPath = mkdtempSync(join(tmpdir(), 'oak-zsh-userdata-'))
      getUserDataPathMock.mockReturnValue(userDataPath)
    })
    ...
    it('discovers ZDOTDIR when .zshenv sources another file that sets it', async () => {
      ...
      expect(result.status).toBe(0)
      expect(result.stdout).toContain(`OAK_ORIG_ZDOTDIR=${xdgZshDir}`)
    })
    ...
    it('preserves ZDOTDIR with spaces in path', async () => {
      ...
      expect(result.status).toBe(0)
      expect(result.stdout).toContain(`OAK_ORIG_ZDOTDIR=${spacePath}`)
    })
    ...
    it('captures last ZDOTDIR value when set multiple times', async () => {
      ...
      expect(result.status).toBe(0)
      expect(result.stdout).toContain(`OAK_ORIG_ZDOTDIR=${lastPath}`)
    })
    ...
    it('handles conditional ZDOTDIR based on environment', async () => {
      ...
      let result = spawnSync('zsh', ['-c', 'echo "OAK_ORIG_ZDOTDIR=${OAK_ORIG_ZDOTDIR}"'], {
        env: cleanEnv as NodeJS.ProcessEnv,
        encoding: 'utf8'
      })

      expect(result.status).toBe(0)
      expect(result.stdout).toContain(`OAK_ORIG_ZDOTDIR=${localPath}`)
      ...
      result = spawnSync('zsh', ['-c', 'echo "OAK_ORIG_ZDOTDIR=${OAK_ORIG_ZDOTDIR}"'], {
        env: cleanEnv as NodeJS.ProcessEnv,
        encoding: 'utf8'
      })

      expect(result.status).toBe(0)
      expect(result.stdout).toContain(`OAK_ORIG_ZDOTDIR=${remotePath}`)
    })
    ...
    it('does not source /.zshenv when HOME is empty', async () => {
      ...
      expect(zshenv).toContain('if [[ -n "${_oak_zshenv_source_dir:-}"')
    })
    ...
    it('handles ZDOTDIR with single quote in path', async () => {
      ...
      expect(result.status).toBe(0)
      expect(result.stdout).toContain(`OAK_ORIG_ZDOTDIR=${quotePath}`)
    })
    ...
    it('does not evaluate command substitution in ZDOTDIR', async () => {
      ...
      expect(result.stdout).toContain(`OAK_ORIG_ZDOTDIR=${safePath}`)
    })
    ...
    it('handles ZDOTDIR with multiple trailing slashes', async () => {
      ...
      expect(result.stdout).toContain(`OAK_ORIG_ZDOTDIR=${cleanPath}`)
    })
  })
  ...
  describeIfZsh('terminal emulator edge cases', () => {
    ...
    beforeEach(async () => {
      testHome = mkdtempSync(join(tmpdir(), 'oak-term-'))
      userDataPath = mkdtempSync(join(tmpdir(), 'oak-term-userdata-'))
      getUserDataPathMock.mockReturnValue(userDataPath)
    })
    ...
    it('handles sudo -E where HOME and ZDOTDIR mismatch', async () => {
      ...
      try {
        ...
        expect(config.env.OAK_ORIG_ZDOTDIR).toBe(userZdotdir)
      } finally {
        if (previousZdotdir === undefined) {
          delete process.env.ZDOTDIR
        } else {
    ...
    it('re-discovers ZDOTDIR despite stale OAK_ORIG_ZDOTDIR from previous session', async () => {
      ...
      const previousOakZdotdir = process.env.OAK_ORIG_ZDOTDIR
      process.env.OAK_ORIG_ZDOTDIR = '/opt/oak-old/shell-ready/zsh' // stale wrapper path
      ...
      try {
        ...
        const cleanEnv: Record<string, string | undefined> = {
          ...process.env,
          HOME: testHome,
          OAK_ORIG_ZDOTDIR: '/opt/oak-old/shell-ready/zsh'
        }
        delete cleanEnv.ZDOTDIR
        cleanEnv.ZDOTDIR = config.env.ZDOTDIR
        ...
        expect(result.stdout).toContain(`OAK_ORIG_ZDOTDIR=${currentZdotdir}`)
      } finally {
        if (previousOakZdotdir === undefined) {
          delete process.env.OAK_ORIG_ZDOTDIR
        } else {
          process.env.OAK_ORIG_ZDOTDIR = previousOakZdotdir
        }
      }
    })

    it('prioritizes fresh discovery over inherited OAK_ORIG_ZDOTDIR', async () => {
      ...
      process.env.OAK_ORIG_ZDOTDIR = oldZdotdir
      ...
      try {
        ...
        expect(result.stdout).toContain(`OAK_ORIG_ZDOTDIR=${freshZdotdir}`)
      } finally {
        if (previousOakZdotdir === undefined) {
          delete process.env.OAK_ORIG_ZDOTDIR
        } else {
    ...
    it('sources launch-time ZDOTDIR .zshenv when it is explicitly inherited', async () => {
      ...
      try {
        const { getShellReadyLaunchConfig } = await importFreshLocalPtyShellReady()
        const config = getShellReadyLaunchConfig('/bin/zsh')
        expect(config.env.OAK_ZSHENV_SOURCE_DIR).toBe(inheritedZdotdir)
        ...
        const result = spawnSync(
          ...
          [
            '-c',
            'echo "OAK_ORIG_ZDOTDIR=${OAK_ORIG_ZDOTDIR}" && echo "SOURCE_MARKER=${SOURCE_MARKER:-unset}"'
          ],
          {
            env: cleanEnv as NodeJS.ProcessEnv,
            encoding: 'utf8'
        ...
        expect(result.status).toBe(0)
        expect(result.stdout).toContain(`OAK_ORIG_ZDOTDIR=${inheritedZdotdir}`)
  ...
  describeIfZsh('automation and edge cases', () => {
    ...
    beforeEach(async () => {
      testHome = mkdtempSync(join(tmpdir(), 'oak-auto-'))
      userDataPath = mkdtempSync(join(tmpdir(), 'oak-auto-userdata-'))
      getUserDataPathMock.mockReturnValue(userDataPath)
    })
    ...
    it('handles empty HOME gracefully', async () => {
      ...
      expect(result.status).toBe(0)
      // Empty HOME falls back to empty OAK_ORIG_ZDOTDIR
      expect(result.stdout).toContain('OAK_ORIG_ZDOTDIR=\n')
    })
    ...
    it('handles unset HOME gracefully', async () => {
      ...
      expect(result.stdout).toMatch(/OAK_ORIG_ZDOTDIR=.+/)
    })
