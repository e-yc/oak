...
describe('PowerShell OSC 133 bootstrap', () => {
  it('wraps prompt/readline without bypassing profiles or execution policy', () => {
    ...
    expect(script).toContain('OAK_OPENCODE_CONFIG_DIR')
    expect(script).toContain('OAK_MIMOCODE_HOME')
    expect(script).not.toContain('OAK_PI_CODING_AGENT_DIR')
    expect(script).not.toContain('OAK_OMP_CODING_AGENT_DIR')
    expect(script).toContain('OAK_OMP_STATUS_EXTENSION')
    expect(script).toContain('function Global:omp')
    expect(script).toContain('--extension $env:OAK_OMP_STATUS_EXTENSION')
    expect(script).toContain('OAK_CODEX_HOME')
