...
const JSON_INSTALLERS = [
  ...
  },
  {
    agent: 'grok',
    timeout: MANAGED_HOOK_TIMEOUT_SECONDS,
    configPath: `${REMOTE_HOME}/.grok/hooks/oak-status.json`,
    install: (sftp: SFTPWrapper) => new GrokHookService().installRemote(sftp, REMOTE_HOME)
  },
  {
    agent: 'copilot',
    timeout: 5,
    configPath: `${REMOTE_HOME}/.copilot/hooks/oak.json`,
...
const MANAGED_HOOKS_DIR_NEEDLE = '/.oak/agent-hooks/'

// Walk the parsed config and assert every Oak-managed command carrier (a node
...
describe('managed agent hook timeouts', () => {
  ...
  it('writes a config-level timeout on local-only Droid hooks', () => {
    const homeDir = mkdtempSync(join(tmpdir(), 'oak-droid-hook-timeout-'))
  ...
  it('bounds every generated POSIX curl wrapper with --connect-timeout and --max-time', async () => {
    ...
    const kimiWrapper = kimi.fs.files.get(`${REMOTE_HOME}/.oak/agent-hooks/kimi-hook.sh`)!
  ...
  describe('dead-endpoint transport budget', () => {
      ...
      return new Promise((resolve, reject) => {
        const child = spawn('sh', [scriptPath], {
          env: {
            ...
            OAK_AGENT_HOOK_TOKEN: 'test-token',
            OAK_PANE_KEY: 'pane-1',
            OAK_TAB_ID: 'tab-1',
    ...
    it.skipIf(!hasPosixCurl)(
      ...
      async () => {
        ...
        const wrapperBody = fs.files.get(`${REMOTE_HOME}/.oak/agent-hooks/codex-hook.sh`)!

        tempDir = mkdtempSync(join(tmpdir(), 'oak-hook-timeout-'))
