...
export type FirstWorkBranchRenameDeps = {
  ...
  /** True only for Oak-created worktrees whose branch Oak is allowed to rename. */
  canRenameOakCreatedBranch: (worktreeId: string) => boolean
...
async function runAutoRename(
  ...
  if (!deps.canRenameOakCreatedBranch(worktreeId)) {
    return stop(`worktree is not eligible for auto-rename`, true)
  }
  // Only rename a branch Oak auto-named — never one the user chose.
