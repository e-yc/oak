...
beforeEach(() => {
  tmpDir = mkdtempSync(join(tmpdir(), 'oak-copilot-hooks-'))
...
function readConfig(): Record<string, unknown> {
  return JSON.parse(readFileSync(join(copilotHome, 'hooks', 'oak.json'), 'utf-8'))
}
...
describe('CopilotHookService', () => {
  it('installs a user-level Copilot hook file under COPILOT_HOME', () => {
    ...
    expect(status.configPath).toBe(join(copilotHome, 'hooks', 'oak.json'))
    ...
    if (process.platform === 'win32') {
      ...
      expect(firstPromptHook.powershell).toContain('OAK_COPILOT_HOOK_EVENT')
      expect(firstPromptHook.powershell).toContain('UserPromptSubmit')
    } else {
      expect(firstPromptHook.bash).toContain('if [ -x ')
      expect(firstPromptHook.bash).toContain('.oak/agent-hooks/copilot-hook.sh')
      expect(firstPromptHook.bash).toContain("OAK_COPILOT_HOOK_EVENT='UserPromptSubmit'")
    }
    expect(existsSync(join(tmpDir, '.oak', 'agent-hooks', 'copilot-hook.sh'))).toBe(
  ...
  it('preserves user-authored hooks and sweeps stale managed entries', () => {
    const configPath = join(copilotHome, 'hooks', 'oak.json')
  ...
  it('remove deletes only Oak-managed Copilot hooks', () => {
    const service = new CopilotHookService()
    service.install()
    const configPath = join(copilotHome, 'hooks', 'oak.json')
    const config = readConfig()
  ...
  it('remove does not create an oak.json file when nothing is installed', () => {
    const status = new CopilotHookService().remove()

    expect(status.state).toBe('not_installed')
    expect(existsSync(join(copilotHome, 'hooks', 'oak.json'))).toBe(false)
  })
  ...
  it('returns an error status for malformed JSON', () => {
    mkdirSync(join(copilotHome, 'hooks'), { recursive: true })
    writeFileSync(join(copilotHome, 'hooks', 'oak.json'), '{not json')
    ...
    expect(status).toEqual({
      agent: 'copilot',
      state: 'error',
      configPath: join(copilotHome, 'hooks', 'oak.json'),
