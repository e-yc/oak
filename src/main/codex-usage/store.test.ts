...
const { getPathMock } = vi.hoisted(() => ({
  getPathMock: vi.fn(() => '/tmp/oak-test-userdata')
}))
...
describe('CodexUsageStore', () => {
  ...
  beforeEach(() => {
    tempUserData = mkdtempSync(join(tmpdir(), 'oak-codex-usage-store-'))
  ...
  it('persists a successful refresh with one compact disk write', async () => {
    ...
    const persistedJson = readFileSync(join(tempUserData, 'oak-codex-usage.json'), 'utf-8')
  ...
  it('reports no data for Oak scope when only non-Oak Codex usage exists', async () => {
    const store = createStoreWithState({
      sessions: [
        {
          sessionId: 'session-1',
    ...
    const summary = await store.getSummary('oak', '30d')
  ...
  it('prices current Codex models with current model rates', async () => {
    ...
    const summary = await store.getSummary('oak', '30d')
    const breakdown = await store.getBreakdown('oak', '30d', 'model')
  ...
  it('uses only Oak-scoped models when projecting mixed-scope sessions', async () => {
    const store = createStoreWithState({
      sessions: [
        {
          sessionId: 'session-1',
    ...
    const breakdown = await store.getBreakdown('oak', '30d', 'model')
    const recentSessions = await store.getRecentSessions('oak', '30d', 10)
