...
import { OAK_GIT_COMMIT_TRAILER } from '../../shared/oak-attribution'

const ATTRIBUTION_ROOT_DIR = 'oak-terminal-attribution'
const ATTRIBUTION_SHIM_VERSION = '6'
const OAK_PRODUCT_URL = 'https://github.com/e-yc/oak'
const OAK_GH_FOOTER = `Made with [Oak](${OAK_PRODUCT_URL}) 🐋`
const SHELL_DOLLAR = '$'
const POWERSHELL_TICK = '`'
const ATTRIBUTION_ENV_KEYS = [
  'OAK_ENABLE_GIT_ATTRIBUTION',
  'OAK_GIT_COMMIT_TRAILER',
  'OAK_GH_PR_FOOTER',
  'OAK_GH_ISSUE_FOOTER',
...
export function applyTerminalAttributionEnv(
  ...
  // cannot recursively point OAK_REAL_* at themselves.
  ...
  // Why: these wrappers should affect only Oak-managed PTYs. Prepending the
  // shim directory here keeps the attribution behavior scoped to Oak's live
  ...
  baseEnv.OAK_ENABLE_GIT_ATTRIBUTION = '1'
  baseEnv.OAK_GIT_COMMIT_TRAILER = OAK_GIT_COMMIT_TRAILER
  baseEnv.OAK_GH_PR_FOOTER = OAK_GH_FOOTER
  baseEnv.OAK_GH_ISSUE_FOOTER = OAK_GH_FOOTER
  if (shellFamily === 'posix') {
    baseEnv.OAK_ATTRIBUTION_SHIM_DIR = posixDir
  } else {
  ...
  if (platform === 'win32') {
    if (resolvedGit) {
      baseEnv.OAK_REAL_GIT = resolvedGit
    }
    if (resolvedGh) {
      baseEnv.OAK_REAL_GH = resolvedGh
    }
  }
}
...
function stripAttributionPathEntries(pathValue: string, pathDelimiter: string): string {
  return pathValue
    ...
    .filter((entry) => {
      ...
      return !normalized.includes('/oak-terminal-attribution/')
    })
    .join(pathDelimiter)
}
...
clean_path() {
  ...
  for entry in "${SHELL_DOLLAR}{entries[@]}"; do
    case "$entry" in
      "$script_dir"|*/oak-terminal-attribution/posix|*/oak-terminal-attribution/win32|*\\oak-terminal-attribution\\posix|*\\oak-terminal-attribution\\win32)
        ;;
      *)
        cleaned+=("$entry")
        ;;
...
if [[ -z "$real_git" ]]; then
  echo "Oak attribution wrapper could not locate git on PATH." >&2
  exit 127
fi
...
if [[ "\${OAK_ENABLE_GIT_ATTRIBUTION:-0}" != "1" || "\${OAK_ATTRIBUTION_BYPASS:-0}" == "1" ]] || ! is_commit_command "$@"; then
...
trailer="\${OAK_GIT_COMMIT_TRAILER:-Co-authored-by: Oak <help@stably.ai>}"
...
OAK_ATTRIBUTION_BYPASS=1 PATH="$real_path" exec "$real_git" "${SHELL_DOLLAR}{attributed_args[@]}"
`
...
if [[ -z "$real_gh" ]]; then
  echo "Oak attribution wrapper could not locate gh on PATH." >&2
  exit 127
fi
...
if [[ "\${OAK_ENABLE_GIT_ATTRIBUTION:-0}" != "1" || "\${OAK_ATTRIBUTION_BYPASS:-0}" == "1" ]]; then
  PATH="$real_path" exec "$real_gh" "$@"
fi

if [[ "\${1:-}" == "pr" && "\${2:-}" == "create" ]]; then
  footer="\${OAK_GH_PR_FOOTER:-Made with [Oak](https://github.com/e-yc/oak) 🐋}"
...
if [[ "\${1:-}" == "issue" && "\${2:-}" == "create" ]]; then
  footer="\${OAK_GH_ISSUE_FOOTER:-Made with [Oak](https://github.com/e-yc/oak) 🐋}"
  if has_passthrough_create_args "$@"; then
    PATH="$real_path" exec "$real_gh" "$@"
  fi
  if ! has_noninteractive_create_args "$@"; then
...
setlocal
if not "%OAK_ENABLE_GIT_ATTRIBUTION%"=="1" goto run
if "%OAK_ATTRIBUTION_BYPASS%"=="1" goto run
call :oak_is_git_commit %*
if errorlevel 1 goto run
powershell.exe -NoProfile -ExecutionPolicy Bypass -File "%~dp0git-wrapper.ps1" %*
exit /b %ERRORLEVEL%
:run
if defined OAK_REAL_GIT (
  "%OAK_REAL_GIT%" %*
) else (
  echo Oak attribution wrapper could not locate git on PATH. 1>&2
  exit /b 127
)
exit /b %ERRORLEVEL%
...
set "oak_git_arg=%~1"
if /I "%oak_git_arg%"=="-c" goto skip_two
if /I "%oak_git_arg%"=="--config" goto skip_two
if /I "%oak_git_arg%"=="-C" goto skip_two
if /I "%oak_git_arg%"=="--git-dir" goto skip_two
if /I "%oak_git_arg%"=="--work-tree" goto skip_two
if /I "%oak_git_arg%"=="--namespace" goto skip_two
if /I "%oak_git_arg:~0,9%"=="--config=" goto skip_one
if /I "%oak_git_arg:~0,10%"=="--git-dir=" goto skip_one
if /I "%oak_git_arg:~0,12%"=="--work-tree=" goto skip_one
if /I "%oak_git_arg:~0,12%"=="--namespace=" goto skip_one
if "%oak_git_arg:~0,1%"=="-" goto skip_one
exit /b 1
:skip_two
shift
shift
goto oak_is_git_commit
:skip_one
shift
goto oak_is_git_commit
`
...
exit /b %ERRORLEVEL%
:run
if defined OAK_REAL_GH (
  "%OAK_REAL_GH%" %*
) else (
  echo Oak attribution wrapper could not locate gh on PATH. 1>&2
  exit /b 127
)
exit /b %ERRORLEVEL%
`
...
$realGit = if ($env:OAK_REAL_GIT) { $env:OAK_REAL_GIT } else { 'git' }
$trailer = if ($env:OAK_GIT_COMMIT_TRAILER) { $env:OAK_GIT_COMMIT_TRAILER } else { 'Co-authored-by: Oak <help@stably.ai>' }
...
$realGh = if ($env:OAK_REAL_GH) { $env:OAK_REAL_GH } else { 'gh' }
...
if ($isPrCreate) {
  ...
  if ($createdUrl) {
    ...
    $footer = if ($env:OAK_GH_PR_FOOTER) { $env:OAK_GH_PR_FOOTER } else { 'Made with [Oak](https://github.com/e-yc/oak) 🐋' }
...
if ($isIssueCreate) {
  ...
  if ($createdUrl) {
    ...
    $footer = if ($env:OAK_GH_ISSUE_FOOTER) { $env:OAK_GH_ISSUE_FOOTER } else { 'Made with [Oak](https://github.com/e-yc/oak) 🐋' }
    if ($null -ne $body -and $body -notmatch [Regex]::Escape($footer)) {
      $tmpFile = [System.IO.Path]::GetTempFileName()
      try {
        $trimmed = $body.TrimEnd("${POWERSHELL_TICK}r", "${POWERSHELL_TICK}n")
