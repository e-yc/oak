...
function getAuthConfig(): AzureDevOpsCreateAuthConfig {
  return {
    apiBaseUrl: envValue('OAK_AZURE_DEVOPS_API_BASE_URL'),
    pat: envValue('OAK_AZURE_DEVOPS_TOKEN') ?? envValue('OAK_AZURE_DEVOPS_PAT'),
    accessToken: envValue('OAK_AZURE_DEVOPS_ACCESS_TOKEN'),
    username: envValue('OAK_AZURE_DEVOPS_USERNAME')
  }
}
...
function classifyCreateError(error: unknown): CreateHostedReviewResult {
  ...
  if (
    ...
    return {
      ...
      code: 'auth_required',
      error:
        'Create PR failed: Azure DevOps is not authenticated. Next step: set OAK_AZURE_DEVOPS_TOKEN in this environment.'
    }
  }
