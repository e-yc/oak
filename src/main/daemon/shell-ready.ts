...
const OAK_USER_DATA_PATH_ENV = 'OAK_USER_DATA_PATH'
const SHELL_READY_MARKER = '\\033]777;oak-shell-ready\\007'

let didEnsureShellReadyWrappers = false

function getShellReadyWrapperRoot(): string {
  const userDataPath = process.env[OAK_USER_DATA_PATH_ENV]
  // Why: older/test launchers may not seed OAK_USER_DATA_PATH. Keep a
  // fallback so daemon startup does not fail before the parent can be fixed.
  return join(userDataPath || tmpdir(), userDataPath ? 'shell-ready' : 'oak-shell-ready')
}

// Why: if our own process inherited ZDOTDIR from a parent shell that was
// itself an Oak PTY (e.g. the user launched Oak from a terminal inside a
// running Oak), that ZDOTDIR points at an Oak shell-ready wrapper dir.
...
// Oak, or a different dev build). Treat it as if ZDOTDIR were unset so the
...
export function getDaemonBashShellReadyRcfileContent(): string {
  return `# Oak daemon bash shell-ready wrapper
...
export function getDaemonZshShellReadyRcfileContent(): string {
  return `# Oak daemon zsh shell-ready wrapper
${getZshStartupFileSourceBlock({
  fileName: '.zshrc',
...
function ensureShellReadyWrappers(): void {
  ...
  const zshProfile = `# Oak daemon zsh shell-ready wrapper
${getZshStartupFileSourceBlock({ fileName: '.zprofile' })}
`
  const zshRc = getDaemonZshShellReadyRcfileContent()
  const zshLogin = `# Oak daemon zsh shell-ready wrapper
...
export function resolvePtyShellPath(env: Record<string, string>): string {
  if (process.platform === 'win32') {
    return env.OAK_TERMINAL_WINDOWS_SHELL || 'powershell.exe'
  }
