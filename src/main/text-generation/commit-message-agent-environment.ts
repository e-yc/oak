...
function prepareShellConfigDirEnv(agentId: string): { ok: true; env?: NodeJS.ProcessEnv } | null {
  ...
  // Why: each kind owns a distinct OAK_*_SOURCE_* shadow so a headless commit
  ...
  const sourceVar =
    agentId === 'opencode'
      ? 'OAK_OPENCODE_SOURCE_CONFIG_DIR'
      : agentId === 'pi'
        ? 'OAK_PI_SOURCE_AGENT_DIR'
        : agentId === 'omp'
          ? 'OAK_OMP_SOURCE_AGENT_DIR'
          : undefined
  ...
  // Why: GUI-launched Oak may not inherit shell startup exports, but these
  // vars point the headless CLI at the user's auth/config root. Nested Oak
  // launches inherit PTY overlays, so prefer OAK_*_SOURCE_* when present.
