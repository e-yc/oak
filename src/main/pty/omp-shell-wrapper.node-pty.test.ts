...
function makeTempDir(): string {
  const dir = mkdtempSync(join(tmpdir(), 'oak-omp-node-pty-'))
  tempDirs.push(dir)
  return dir
}
...
function writeFakeOmp(binDir: string): void {
  ...
  writeFileSync(
    ompPath,
    `#!/bin/sh
agent_dir="\${PI_CODING_AGENT_DIR:-\${OAK_FAKE_OMP_DEFAULT_DIR:-}}"
...
describePosix('OMP shell wrapper node-pty reproduction', () => {
  ...
  itWithBash('reproduces why restored shells miss OMP status without the wrapper', async () => {
    ...
    const statusExtension = join(extensionDir, 'oak-agent-status.ts')
    ...
    const makeEnv = (captureFile: string, afterPiFile: string): Record<string, string> => ({
      ...
      OAK_OMP_STATUS_EXTENSION: statusExtension,
      OAK_FAKE_OMP_DEFAULT_DIR: ompDir,
      OAK_CAPTURE_FILE: captureFile,
      OAK_AFTER_PI_FILE: afterPiFile,
    ...
    await runInteractiveBashPty({
      ...
      input: `omp ask
printf '%s' "$PI_CODING_AGENT_DIR" > "$OAK_AFTER_PI_FILE"
exit 0
`
    })
  ...
  itWithBash('runs OMP config subcommands without redirecting the home', async () => {
    ...
    await runInteractiveBashPty({
      ...
      env: {
        ...
        OAK_PI_CODING_AGENT_DIR: '',
        OAK_OMP_CODING_AGENT_DIR: '',
        OAK_OMP_SOURCE_AGENT_DIR: sourceDir,
        OAK_OMP_STATUS_EXTENSION: statusExtension,
        OAK_FAKE_OMP_DEFAULT_DIR: sourceDir,
        OAK_CAPTURE_FILE: captureFile,
        TERM: process.env.TERM || 'xterm-256color'
      },
      input: `omp config
    ...
    async () => {
      ...
      await runInteractiveBashPty({
        ...
        env: {
          ...
          OAK_OMP_CODING_AGENT_DIR: '',
          OAK_OMP_STATUS_EXTENSION: statusExtension,
          OAK_FAKE_OMP_DEFAULT_DIR: defaultOmpDir,
          OAK_CAPTURE_FILE: captureFile,
          TERM: process.env.TERM || 'xterm-256color'
        },
        input: `omp config
