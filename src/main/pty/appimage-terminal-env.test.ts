...
describe('removeAppImageRuntimeEnv', () => {
  ...
  it('removes AppImage runtime identity and mount paths from Linux terminal env', () => {
    ...
    const env = {
      APPIMAGE: '/data/apps/oak.appimage',
      APPDIR: '/tmp/.mount_oak123',
      ARGV0: '/data/apps/oak.appimage',
      OWD: '/home/user',
      APPIMAGE_LIBRARY_PATH: '/tmp/.mount_oak123/usr/lib',
      PATH: ['/tmp/.mount_oak123', '/tmp/.mount_oak123/usr/sbin', '/usr/bin'].join(delimiter),
      LD_LIBRARY_PATH: ['/tmp/.mount_oak123/usr/lib', '/opt/audio/lib'].join(delimiter),
      HOME: '/home/user'
    }
  ...
  it('leaves AppImage-looking env untouched outside Linux', () => {
    ...
    const env = {
      ...
      APPDIR: '/tmp/.mount_oak123',
      ARGV0: '/data/apps/oak.appimage',
      PATH: ['/tmp/.mount_oak123/usr/bin', '/usr/bin'].join(delimiter)
    }
