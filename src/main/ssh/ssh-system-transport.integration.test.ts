...
const sentinel = 'OAK-RELAY v0.1.0 READY\\n';
...
function createRelayTree(root: string, remoteHome: string): void {
  ...
  const remoteDir = join(remoteHome, '.oak-remote', `relay-${RELAY_VERSION}`)
...
describe('system SSH transport integration', () => {
  ...
  beforeEach(() => {
    ...
    tempDir = mkdtempSync(join('/tmp', 'oak-ssh-'))
    oldHome = process.env.HOME
    oldRelayPath = process.env.OAK_RELAY_PATH
    oldSystemSshPath = process.env.OAK_SYSTEM_SSH_PATH
    oldForceSystemTransport = process.env.OAK_SSH_FORCE_SYSTEM_TRANSPORT
    ...
    process.env.OAK_RELAY_PATH = relayRoot
    process.env.OAK_SYSTEM_SSH_PATH = writeFakeSsh(tempDir)
    process.env.OAK_SSH_FORCE_SYSTEM_TRANSPORT = '1'
  })
  ...
  afterEach(() => {
    ...
    if (oldRelayPath === undefined) {
      delete process.env.OAK_RELAY_PATH
    } else {
      process.env.OAK_RELAY_PATH = oldRelayPath
    }
    if (oldSystemSshPath === undefined) {
      delete process.env.OAK_SYSTEM_SSH_PATH
    } else {
      process.env.OAK_SYSTEM_SSH_PATH = oldSystemSshPath
    }
    if (oldForceSystemTransport === undefined) {
      delete process.env.OAK_SSH_FORCE_SYSTEM_TRANSPORT
    } else {
      process.env.OAK_SSH_FORCE_SYSTEM_TRANSPORT = oldForceSystemTransport
    }
