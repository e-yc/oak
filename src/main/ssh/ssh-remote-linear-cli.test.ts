...
describe('runRemoteOakCli Linear commands', () => {
  ...
  it('accepts leading boolean flags before SSH Linear commands', async () => {
    ...
    const result = await runRemoteOakCli(runtime, {
      ...
      cwd: '/home/alice/remote-repo',
      env: { OAK_TERMINAL_HANDLE: 'term_ssh' }
    })
  ...
  it('dispatches Linear discovery and list reads through the remote runtime', async () => {
    ...
    const teamList = await runRemoteOakCli(runtime, {
      argv: ['linear', 'team', 'list', '--workspace', 'all', '--json'],
      cwd: '/home/alice/remote-repo',
      env: { OAK_TERMINAL_HANDLE: 'term_ssh' }
    })
    const labels = await runRemoteOakCli(runtime, {
      argv: ['linear', 'team', 'labels', '--team', 'ENG', '--workspace', 'workspace-1', '--json'],
      cwd: '/home/alice/remote-repo',
      env: { OAK_TERMINAL_HANDLE: 'term_ssh' }
    })
    const list = await runRemoteOakCli(runtime, {
      argv: [
        'linear',
        'list',
        '--filter',
    ...
    const projects = await runRemoteOakCli(runtime, {
      argv: [
        'linear',
        'project',
        'list',
  ...
  it('prints SSH Linear non-json next steps from structured errors', async () => {
    ...
    linearIssueAddComment.mockRejectedValueOnce(
      Object.assign(new Error('Linear may have applied the write.'), {
        ...
        data: { nextSteps: ['Retry once with the pinned command: `oak linear comment add`.'] }
      })
    )
  ...
  it('shows SSH Linear command help without dispatching to the runtime', async () => {
    ...
    expect(result.exitCode).toBe(0)
    expect(result.stdout).toContain('oak linear issue')
    expect(result.stdout).toContain('Usage: oak linear issue')
  ...
  it('shows SSH Linear group help without dispatching to the runtime', async () => {
    ...
    expect(result.exitCode).toBe(0)
    expect(result.stdout).toContain('oak linear')
    expect(result.stdout).toContain('Usage: oak linear <command> [options]')
  ...
  it('shows SSH Linear help through the local help command form', async () => {
    ...
    const group = await runRemoteOakCli(runtime, {
      argv: ['help', 'linear'],
      cwd: '/home/alice/remote-repo',
      env: { OAK_TERMINAL_HANDLE: 'term_ssh' }
    })
    const issue = await runRemoteOakCli(runtime, {
    ...
    expect(group.stdout).toContain('Usage: oak linear <command> [options]')
    expect(issue.exitCode).toBe(0)
    expect(issue.stdout).toContain('Usage: oak linear issue')
    expect(linearIssueContext).not.toHaveBeenCalled()
  })
