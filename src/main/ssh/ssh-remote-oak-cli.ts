...
export type RemoteOakCliRequest = {
  argv: string[]
  cwd: string
...
export type RemoteOakCliResult = {
  stdout: string
  stderr: string
  exitCode: number
}
...
export async function runRemoteOakCli(
  runtime: OakRuntimeService,
  request: RemoteOakCliRequest
): Promise<RemoteOakCliResult> {
...
async function dispatchRemoteCli(
  ...
  switch (command) {
    ...
    default:
      throw new Error(`Unsupported SSH Oak CLI command: ${command}`)
  }
}
...
function resolveHandle(
  ...
  return optionalString(flags, flagName) ?? env.OAK_TERMINAL_HANDLE ?? 'unknown'
}
