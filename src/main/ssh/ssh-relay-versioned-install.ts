...
// (~/.oak-remote/relay-v0.1.0) which the deploy step would overwrite in place
...
export function readLocalFullVersion(localRelayDir: string): string {
  ...
  if (!existsSync(versionFile)) {
    throw new Error(
      `Oak's local relay build is missing its version marker at ${versionFile}. ` +
        `This usually indicates a packaging or build problem; reinstall Oak.`
    )
  }
  const v = readFileSync(versionFile, 'utf-8').trim()
  if (!v) {
    throw new Error(
      `Oak's local relay version marker at ${versionFile} is empty. ` +
        `This usually indicates a packaging or build problem; reinstall Oak.`
    )
  }
  return v
