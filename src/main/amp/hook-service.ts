...
const AMP_PLUGIN_FILE = 'oak-agent-status.ts'
const AMP_PLUGIN_MARKER = 'Managed by Oak. Do not edit; changes may be overwritten.'
  ...
  switch (state.kind) {
    ...
    case 'unmanaged':
      return {
        ...
        configPath: pluginPath,
        managedHooksPresent: false,
        detail: 'Amp Oak status plugin exists but is not Oak-managed'
      }
    case 'error':
      return {
        agent: 'amp',
...
function getAmpPluginSource(): string {
  return [
    ...
    '  const endpointPath = process.env.OAK_AGENT_HOOK_ENDPOINT',
    ...
    "      if (match[1] === 'OAK_AGENT_HOOK_PORT') out.port = value",
    "      if (match[1] === 'OAK_AGENT_HOOK_TOKEN') out.token = value",
    "      if (match[1] === 'OAK_AGENT_HOOK_ENV') out.env = value",
    "      if (match[1] === 'OAK_AGENT_HOOK_VERSION') out.version = value",
    '    }',
    ...
    "      console.warn('[oak-hook] failed to parse Amp endpoint file:', (error as Error).message)",
    '    }',
    '    return null',
    '  }',
    '}',
    ...
    '  // Why: Amp sessions can outlive an Oak restart; the endpoint file is',
    '  // rewritten on each start, so read it per event before falling back to env.',
    '  const fileEnv = readEndpointFile() ?? {}',
    '  return {',
    '    port: fileEnv.port || process.env.OAK_AGENT_HOOK_PORT,',
    '    token: fileEnv.token || process.env.OAK_AGENT_HOOK_TOKEN,',
    '    env: fileEnv.env || process.env.OAK_AGENT_HOOK_ENV || "",',
    '    version: fileEnv.version || process.env.OAK_AGENT_HOOK_VERSION || ""',
    '  }',
    '}',
    '',
    ...
    '    })',
    '  } catch {',
    '    // Why: Oak status reporting must never affect the Amp run.',
    '  } finally {',
    ...
    '  // payload closures when Oak is down and each POST waits for timeout.',
