...
describe('run-electron-vite-dev', () => {
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      const tempDir = mkdtempSync(join(tmpdir(), 'oak-dev-wrapper-'))
      ...
      const wrapper = spawn(process.execPath, [wrapperPath], {
        ...
        env: devWrapperTestEnv({
          OAK_ELECTRON_VITE_CLI: fakeCliPath,
          OAK_SKIP_DEV_CLI_PREPARE: '1',
          OAK_SKIP_DEV_ELECTRON_APP_PREPARE: '1',
          OAK_SKIP_DEV_WEB_PREPARE: '1',
          OAK_DEV_WRAPPER_TEST_PID_FILE: pidFile
        }),
        stdio: 'ignore'
      })
  ...
  it('forwards dev instance identity to electron-vite', async () => {
    ...
    const wrapper = spawn(process.execPath, [wrapperPath, '--remote-debugging-port=9444'], {
      ...
      env: devWrapperTestEnv({
        ...
        OAK_SKIP_DEV_WEB_PREPARE: '1',
        OAK_DEV_WRAPPER_TEST_PID_FILE: pidFile,
        OAK_DEV_WRAPPER_TEST_ENV_FILE: envFile,
    ...
    expect(envSnapshot.dockTitle).toBe('Oak: feature/billing-shell')
  ...
  it.skipIf(process.platform === 'win32')(
    'prepares userData oak and oak-dev wrappers for dev terminals',
    async () => {
      const tempDir = mkdtempSync(join(tmpdir(), 'oak-dev-wrapper-'))
      ...
      const wrapper = spawn(process.execPath, [wrapperPath], {
        cwd: resolve('.'),
        env: devWrapperTestEnv({
          OAK_DEV_USER_DATA_PATH: userDataPath,
          OAK_ELECTRON_VITE_CLI: fakeCliPath,
          OAK_SKIP_DEV_ELECTRON_APP_PREPARE: '1',
          OAK_SKIP_DEV_WEB_PREPARE: '1',
          OAK_DEV_WRAPPER_TEST_PID_FILE: pidFile,
      ...
      const trackedPids = trackPidFile(pidFile)
      const devWrapper = readFileSync(join(userDataPath, 'cli', 'bin', 'oak-dev'), 'utf8')
      const publicAliasWrapper = readFileSync(join(userDataPath, 'cli', 'bin', 'oak'), 'utf8')
      expect(publicAliasWrapper).toBe(devWrapper)
      expect(publicAliasWrapper).toContain('OAK_USER_DATA_PATH')
    ...
    const wrapper = spawn(
      ...
      [wrapperPath, '--stable-name', '--remote-debugging-port=9445'],
      {
        ...
        env: devWrapperTestEnv({
          ...
          OAK_DEV_WRAPPER_TEST_PID_FILE: pidFile,
          OAK_DEV_WRAPPER_TEST_ENV_FILE: envFile,
          OAK_DEV_BRANCH: 'feature/stable-name',
          OAK_DEV_WORKTREE_NAME: 'stable-ui'
        }),
        stdio: 'ignore'
      }
    )
  ...
  it.skipIf(process.platform !== 'darwin')(
    ...
    async () => {
      ...
      const baseEnv = devWrapperTestEnv({
        ...
        OAK_SKIP_DEV_CLI_PREPARE: '1',
        OAK_SKIP_DEV_WEB_PREPARE: '1',
        OAK_DEV_BRANCH: 'feature/rebuild-electron-app',
        OAK_DEV_WORKTREE_NAME: 'electron-app-rebuild'
      })
    ...
    async () => {
      ...
      const wrapper = spawn(process.execPath, [wrapperPath, '--remote-debugging-port=9448'], {
        ...
        env: devWrapperTestEnv({
          ...
          OAK_DEV_WRAPPER_TEST_PID_FILE: pidFile,
          OAK_DEV_WRAPPER_TEST_ENV_FILE: envFile,
          OAK_DEV_BRANCH: 'feature/framework-symlinks',
          OAK_DEV_WORKTREE_NAME: 'symlink-ui'
        }),
        stdio: 'ignore'
      })
