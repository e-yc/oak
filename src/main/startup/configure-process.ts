...
const HTTP1_COMPATIBILITY_ENV_VAR = 'OAK_DISABLE_HTTP2'
...
function readPersistedHttp1CompatibilityMode(userDataPath: string): boolean {
  const dataFile = join(userDataPath, 'oak-data.json')
...
function requestDevParentShutdown(): void {
  ...
  const forceExitTimer = setTimeout(() => {
    ...
    // handlers or window-close guards stall and would otherwise leave Oak
...
export function configureDevUserDataPath(isDev: boolean): void {
  ...
  // public `oak` CLI look broken even though the packaged app is still open.
  app.setPath('userData', join(app.getPath('appData'), 'oak-dev'))
}

export function configureOakUserDataPathEnv(): void {
  // Why: app relaunches can inherit an OAK_USER_DATA_PATH from an older CLI or
  // updater process. Main must canonicalize it before CLI-shared modules build
  // runtime-home paths, or migrations can bridge two Oak app-data directories.
  process.env.OAK_USER_DATA_PATH = app.getPath('userData')
}

export function shouldInstallManagedHooks(isDev: boolean): boolean {
  void isDev
  // Why: managed hook installation now targets Oak-owned, environment-scoped
...
export function installDevParentWatchdog(isDev: boolean): void {
  ...
  const timer = setInterval(() => {
    ...
    if (parentPidChanged || parentMissing) {
      ...
      // the dev runner while leaving Oak open. Watching the original parent PID
