...
describe('AppImage CLI redirect', () => {
  it('detects direct AppImage CLI commands', () => {
    expect(
      getAppImageCliArgs(
        ['oak-linux.AppImage', 'status', '--json'],
        { APPIMAGE: '/opt/oak' },
        {
          platform: 'linux',
          isPackaged: true,
  ...
  it('allows CLI global flags before the command', () => {
    expect(
      getAppImageCliArgs(
        ['oak-linux.AppImage', '--pairing-code', 'abc123', '--json', 'terminal', 'list'],
        {
          APPIMAGE: '/opt/oak'
        },
        {
  ...
  it('spawns the unpacked CLI entrypoint with Electron node mode', async () => {
    const root = await mkdtemp(join(tmpdir(), 'oak-appimage-cli-redirect-'))
    ...
    const result = maybeRedirectAppImageCliLaunch({
      argv: ['oak-linux.AppImage', 'status', '--json'],
      env: {
        APPIMAGE: '/opt/oak/oak-linux.AppImage',
    ...
    expect(spawn).toHaveBeenCalledWith('/opt/oak/oak-ide', [cliEntryPath, 'status', '--json'], {
      env: expect.objectContaining({
        APPIMAGE: '/opt/oak/oak-linux.AppImage',
        ELECTRON_RUN_AS_NODE: '1',
        OAK_NODE_OPTIONS: '--inspect',
        OAK_NODE_REPL_EXTERNAL_MODULE: '/tmp/repl.js'
      }),
      stdio: 'inherit'
    })
