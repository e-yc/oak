...
function stashWebBuild(): () => void {
  ...
  const tempDir = mkdtempSync(join(dirname(outWebPath), '.oak-dev-web-stash-'))
    ...
    try {
      const wrapper = spawn(process.execPath, [wrapperPath, '--remote-debugging-port=9446'], {
        ...
        env: {
          ...
          OAK_DEV_WRAPPER_TEST_VITE_FILE: viteFile
        },
