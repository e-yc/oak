...
describe('patchPackagedProcessPath', () => {
  ...
  it('prepends agent-CLI install dirs (~/.opencode/bin, ~/.vite-plus/bin) for packaged darwin runs', async () => {
    ...
    // Without them on PATH, GUI-launched Oak reports both as "Not installed"
...
describe('configureDevUserDataPath', () => {
  it('uses an explicit dev userData override when provided', async () => {
    ...
    const originalOverride = process.env.OAK_DEV_USER_DATA_PATH
    process.env.OAK_DEV_USER_DATA_PATH = '/tmp/oak-dev-repro'

    try {
      configureDevUserDataPath(true)
    } finally {
      if (originalOverride === undefined) {
        delete process.env.OAK_DEV_USER_DATA_PATH
      } else {
        process.env.OAK_DEV_USER_DATA_PATH = originalOverride
      }
    }

    expect(app.setPath).toHaveBeenCalledWith('userData', '/tmp/oak-dev-repro')
  })

  it('moves dev runs onto an oak-dev userData path', async () => {
    const { app } = await import('electron')
    const { configureDevUserDataPath } = await import('./configure-process')
    ...
    // Why: production code uses path.join(app.getPath('appData'), 'oak-dev')
    // which produces platform-specific separators.
    expect(app.setPath).toHaveBeenCalledWith('userData', join('/tmp/app-data', 'oak-dev'))
  })
...
describe('configureOakUserDataPathEnv', () => {
  it('overwrites stale inherited OAK_USER_DATA_PATH with Electron userData', async () => {
    const { app } = await import('electron')
    const { configureOakUserDataPathEnv } = await import('./configure-process')
    const originalUserDataPath = process.env.OAK_USER_DATA_PATH
    process.env.OAK_USER_DATA_PATH = '/tmp/stale-oak-user-data'
    app.setPath('userData', '/tmp/current-oak-user-data')
    let configuredUserDataPath: string | undefined

    try {
      configureOakUserDataPathEnv()
      configuredUserDataPath = process.env.OAK_USER_DATA_PATH
    } finally {
    ...
    expect(configuredUserDataPath).toBe('/tmp/current-oak-user-data')
  })
})
...
describe('configureElectronNetworkCompatibility', () => {
  ...
  const originalEnvValue = process.env.OAK_DISABLE_HTTP2

  function createUserDataDir(settings: Record<string, unknown>): string {
    const userDataPath = mkdtempSync(join(tmpdir(), 'oak-http1-compat-'))
    tempDirs.push(userDataPath)
    writeFileSync(join(userDataPath, 'oak-data.json'), JSON.stringify({ settings }), 'utf-8')
    return userDataPath
  }
  ...
  afterEach(() => {
    ...
    if (originalEnvValue === undefined) {
      delete process.env.OAK_DISABLE_HTTP2
    } else {
      process.env.OAK_DISABLE_HTTP2 = originalEnvValue
    }
  })
...
describe('enableMainProcessGpuFeatures', () => {
  const originalPlatform = Object.getOwnPropertyDescriptor(process, 'platform')
  const originalE2EUserDataDir = process.env.OAK_E2E_USER_DATA_DIR
  ...
  afterEach(() => {
    ...
    if (originalE2EUserDataDir === undefined) {
      delete process.env.OAK_E2E_USER_DATA_DIR
    } else {
      process.env.OAK_E2E_USER_DATA_DIR = originalE2EUserDataDir
    }
  })
  ...
  it('disables the GPU process for Linux E2E runs', async () => {
    ...
    setPlatform('linux')
    process.env.OAK_E2E_USER_DATA_DIR = '/tmp/oak-e2e'
    vi.mocked(app.disableHardwareAcceleration).mockClear()
    vi.mocked(app.commandLine.appendSwitch).mockClear()
