...
export const SINGLE_INSTANCE_LOCK_FAILURE_MESSAGE =
  '[single-instance] Another Oak instance is already running for this userData profile; exiting this launch after requesting the existing window. If no Oak process is running, this may be an Electron/macOS single-instance lock failure.'
export const SINGLE_INSTANCE_LOCK_BYPASS_ENV = 'OAK_BYPASS_SINGLE_INSTANCE_LOCK'
export const SINGLE_INSTANCE_LOCK_BYPASS_MESSAGE =
  '[single-instance] OAK_BYPASS_SINGLE_INSTANCE_LOCK=1 is set; bypassing the packaged macOS single-instance lock for diagnostics. Do not use this with another Oak instance running for the same profile.'

/**
 * Why: Oak writes two canonical discovery files into `<userData>/`:
 * `oak-runtime.json` (RPC endpoint + authToken for the bundled CLI) and
 ...
 * instance quits, metadata points at a dead pid and `oak status` reports
 ...
 * way dev (`oak-dev` userData) and packaged (`oak` userData) runs lock in
