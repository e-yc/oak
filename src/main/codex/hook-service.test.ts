...
beforeEach(() => {
  tmpHome = mkdtempSync(join(tmpdir(), 'oak-codex-home-'))
  userDataDir = mkdtempSync(join(tmpdir(), 'oak-codex-user-data-'))
...
describe('CodexHookService', () => {
  it('installs PermissionRequest with trust so Codex approval prompts reach Oak', () => {
  ...
  it.skipIf(process.platform !== 'win32')(
    ...
    () => {
      const spaceHome = join(tmpdir(), 'oak home with spaces')
  ...
  it.skipIf(process.platform !== 'win32')(
    ...
    () => {
      const metacharHome = join(tmpdir(), 'oak %OAK_TEST% ^ home')
  ...
  it.skipIf(process.platform !== 'win32')(
    ...
    () => {
      ...
      const cmdSafe = /^[A-Za-z0-9_.:\\~-]+$/.test(join(tmpHome, '.oak', 'agent-hooks'))
      if (cmdSafe) {
  ...
  it.skipIf(process.platform !== 'win32')(
    ...
    async () => {
      ...
      const scriptPath = join(homedir(), '.oak', 'agent-hooks', 'codex-hook.cmd')
      ...
      try {
        ...
        // Why: this suite may run inside an Oak-launched terminal whose env
        // already carries OAK_AGENT_HOOK_ENDPOINT/PORT/TOKEN. The managed
        // script sources that endpoint file, so leave it out or the hook posts
        // to the live Oak instead of this test's listener.
        ...
        const child = spawn('cmd.exe', ['/d', '/c', scriptPath], {
          env: {
            ...cleanEnv,
            OAK_AGENT_HOOK_PORT: String(port),
            OAK_AGENT_HOOK_TOKEN: 'tok123',
            OAK_PANE_KEY: '42:leaf-abc',
            OAK_TAB_ID: '42',
            OAK_WORKTREE_ID: 'C:\\work trees\\my repo & co',
        ...
        expect(received.headers['x-oak-agent-hook-token']).toBe('tok123')
  ...
  it('keeps hooks isolated by Oak userData instead of mutating system ~/.codex', () => {
    const systemCodexHome = join(tmpHome, '.codex')
    ...
    const devUserDataDir = mkdtempSync(join(tmpdir(), 'oak-dev-codex-user-data-'))
    const prodUserDataDir = mkdtempSync(join(tmpdir(), 'oak-prod-codex-user-data-'))
    try {
      getPathMock.mockImplementation((name: string) => {
        if (name === 'userData') {
      ...
      process.env.OAK_USER_DATA_PATH = devUserDataDir
      expect(new CodexHookService().install().state).toBe('installed')
      ...
      process.env.OAK_USER_DATA_PATH = prodUserDataDir
      expect(new CodexHookService().install().state).toBe('installed')
  ...
  it('refreshes runtime user hooks without installing Oak-managed hooks', () => {
    const systemCodexHome = join(tmpHome, '.codex')
    const systemHooksPath = join(systemCodexHome, 'hooks.json')
    mkdirSync(systemCodexHome, { recursive: true })
    writeFileSync(
  ...
  it('removes legacy Oak-managed hooks from system ~/.codex during install', () => {
    const systemCodexHome = join(tmpHome, '.codex')
    const systemHooksPath = join(systemCodexHome, 'hooks.json')
  ...
  it('removes very large legacy Oak-managed hook lists from system ~/.codex', () => {
    const systemCodexHome = join(tmpHome, '.codex')
    const systemHooksPath = join(systemCodexHome, 'hooks.json')
    const legacyScriptPath = join(
      tmpHome,
  ...
  it('removes the legacy Oak Codex profile file when it only contains managed hooks', () => {
    const systemCodexHome = join(tmpHome, '.codex')
    const profilePath = join(systemCodexHome, 'oak-agent-status.config.toml')
    mkdirSync(systemCodexHome, { recursive: true })
    writeFileSync(
      profilePath,
      [
        '# BEGIN OAK AGENT STATUS HOOKS',
        '[[hooks.PermissionRequest]]',
        '[[hooks.PermissionRequest.hooks]]',
        'type = "command"',
        'command = "codex-hook"',
        '# END OAK AGENT STATUS HOOKS',
        ''
      ].join('\n'),
      'utf-8'
    )
  ...
  it('removes only the legacy Oak block from a user-edited Codex profile file', () => {
    const systemCodexHome = join(tmpHome, '.codex')
    const profilePath = join(systemCodexHome, 'oak-agent-status.config.toml')
    mkdirSync(systemCodexHome, { recursive: true })
    writeFileSync(
    ...
    expect(profileConfig).not.toContain('OAK AGENT STATUS HOOKS')
  ...
  it('cleans duplicate Codex hook representations while keeping status hooks in runtime CODEX_HOME', () => {
    ...
    const legacyProfilePath = join(systemCodexHome, 'oak-agent-status.config.toml')
    const legacyScriptPath = join(
      tmpHome,
      '.oak',
      'agent-hooks',
  ...
  it('removes managed trust entries when userData resolves through a symlink', () => {
    ...
    process.env.OAK_USER_DATA_PATH = linkedUserDataDir
