...
export function syncSystemCodexSessionsIntoManagedHome(): void {
  ...
  const managedSessionsRoot = join(getOakManagedCodexHomePath(), 'sessions')
...
function replaceSymlinkSessionBridgeWithHardlink(
  ...
  try {
    ...
    replacementPath = `${targetPath}.oak-link-${process.pid}-${Date.now()}`
...
function getLegacySessionCopyMarkerPath(relativePath: string): string {
  return join(getOakManagedCodexHomePath(), '.oak-session-copies', `${relativePath}.json`)
}
