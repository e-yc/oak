...
export function getOakManagedCodexHomePath(): string {
  const managedHomePath = join(getOakUserDataPath(), 'codex-runtime-home', 'home')
  mkdirSync(managedHomePath, { recursive: true })
  return managedHomePath
}

function getOakUserDataPath(): string {
  ...
  if (process.platform === 'win32') {
    return join(process.env.APPDATA ?? join(homedir(), 'AppData', 'Roaming'), 'oak')
  }
...
export function syncSystemCodexResourcesIntoManagedHome(): void {
  ...
  const managedHomePath = getOakManagedCodexHomePath()
...
function getResourceCopyMarkerPath(managedHomePath: string, entryName: string): string {
  return join(managedHomePath, '.oak-resource-copies', `${entryName}.json`)
}
