...
describe('canCleanupUnregisteredOakLeftoverDirectory', () => {
  const repo = { path: '/repos/main' }
  const ownedMeta = { oakCreatedAt: 1, oakCreationSource: 'runtime' as const }
  const baseArgs = {
    meta: ownedMeta,
    worktreePath: '/workspaces/oak-owned',
    runtimeWorktreePath: '/workspaces/oak-owned',
    repo,
  ...
  it('rejects unregistered existing targets that are files or symlinks', async () => {
    ...
    await expect(
      canCleanupUnregisteredOakLeftoverDirectory({
        ...baseArgs,
        statPath: makeStatPath(['/workspaces/oak-owned']),
        isGitRepository
      })
    ).resolves.toBe(false)
    await expect(
  ...
  it('rejects unregistered leftover directories with a .git marker', async () => {
    ...
    await expect(
      canCleanupUnregisteredOakLeftoverDirectory({
        ...baseArgs,
        statPath: makeStatPath(['/workspaces/oak-owned/.git'], ['/workspaces/oak-owned']),
        isGitRepository
      })
    ).resolves.toBe(false)
  ...
  it('rejects no-marker cleanup when only the Oak path shape matches', async () => {
    const isGitRepository = vi.fn().mockResolvedValue(false)
  ...
  it('rejects unregistered leftover directories that still answer git status', async () => {
    ...
    expect(isGitRepository).toHaveBeenCalledWith('/workspaces/oak-owned')
  })
    ...
    await expect(
      canCleanupUnregisteredOakLeftoverDirectory({
        ...
        registeredWorktrees: [
          ...
          makeGitWorktree('/workspaces/oak-owned/child')
        ],
        statPath: makeStatPath([], ['/workspaces/oak-owned']),
        isGitRepository: vi.fn().mockResolvedValue(false)
      })
    ).rejects.toThrow(
      'Refusing to delete worktree because it contains another registered worktree: /workspaces/oak-owned/child'
    )
  })
  ...
  it('uses runtime paths for filesystem proof and original paths for nested worktree checks', async () => {
    const statPath = vi.fn(async (path: string) => {
      if (path === '/mnt/c/workspaces/oak-owned') {
        return { type: 'directory' }
      }
      throw missingPath(path)
    })
    ...
    await expect(
      canCleanupUnregisteredOakLeftoverDirectory({
        ...baseArgs,
        worktreePath: 'C:\\workspaces\\oak-owned',
        runtimeWorktreePath: '/mnt/c/workspaces/oak-owned',
        repo: { path: 'C:\\repos\\main' },
        runtimeRepoPath: '/mnt/c/repos/main',
        registeredWorktrees: [
          makeGitWorktree('C:\\repos\\main', true),
          makeGitWorktree('C:\\workspaces\\oak-owned-sibling')
        ],
        statPath,
        isGitRepository
      })
    ...
    expect(statPath).toHaveBeenCalledWith('/mnt/c/workspaces/oak-owned')
    expect(statPath).toHaveBeenCalledWith('/mnt/c/workspaces/oak-owned/.git')
    expect(statPath).not.toHaveBeenCalledWith('C:\\workspaces\\oak-owned')
    expect(isGitRepository).toHaveBeenCalledWith('/mnt/c/workspaces/oak-owned')
  })
  ...
  it('rejects translated-runtime cleanup when original path contains a registered child', async () => {
    await expect(
      canCleanupUnregisteredOakLeftoverDirectory({
        ...
        registeredWorktrees: [
          makeGitWorktree('C:\\repos\\main', true),
          makeGitWorktree('C:\\workspaces\\oak-owned\\child')
        ],
        statPath: makeStatPath([], ['/mnt/c/workspaces/oak-owned']),
        isGitRepository: vi.fn().mockResolvedValue(false)
      })
    ).rejects.toThrow(
      'Refusing to delete worktree because it contains another registered worktree: C:\\workspaces\\oak-owned\\child'
    )
  })
})
