...
function configuredApiBaseUrl(repo: GiteaRepoRef): string {
  const configured = envValue('OAK_GITEA_API_BASE_URL')
...
export function isGiteaReviewCreationAuthenticated(): boolean {
  return envValue('OAK_GITEA_TOKEN') !== null
}

function authHeaders(): Record<string, string> {
  const token = envValue('OAK_GITEA_TOKEN')
...
function classifyCreateError(error: unknown): CreateHostedReviewResult {
  ...
  if (
    ...
    return {
      ...
      code: 'auth_required',
      error:
        'Create PR failed: Gitea is not authenticated. Next step: set OAK_GITEA_TOKEN in this environment.'
    }
  }
