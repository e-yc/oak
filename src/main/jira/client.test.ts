...
function tokenPathForSite(siteId: string): string {
  return join(tempHome, '.oak', 'jira-tokens', `${Buffer.from(siteId).toString('base64url')}.enc`)
}

function writeJiraFiles(siteId: string, token: string | Buffer): void {
  const oakDir = join(tempHome, '.oak')
  mkdirSync(join(oakDir, 'jira-tokens'), { recursive: true })
  writeFileSync(
    join(oakDir, 'jira-sites.json'),
    JSON.stringify(
      {
        version: 1,
        activeSiteId: siteId,
...
beforeEach(() => {
  tempHome = mkdtempLike('oak-jira-client-')
...
describe('Jira client credential storage', () => {
  ...
  it('bridges proxy environment settings before Jira connect requests', async () => {
    ...
    expect(netFetchMock).toHaveBeenCalledTimes(1)
    const headers = netFetchMock.mock.calls[0]?.[1]?.headers as Headers
    expect(headers.get('User-Agent')).toBe('Oak')
    expect(fetchMock).not.toHaveBeenCalled()
  })
})
