...
describe('DroidHookService', () => {
  ...
  beforeEach(() => {
    homeDir = mkdtempSync(join(tmpdir(), 'oak-droid-home-'))
    userDataDir = mkdtempSync(join(tmpdir(), 'oak-droid-user-data-'))
  ...
  it.skipIf(process.platform !== 'win32')(
    ...
    () => {
      const spaceHome = join(tmpdir(), 'oak droid home with spaces')
