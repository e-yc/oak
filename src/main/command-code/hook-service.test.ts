...
describe('CommandCodeHookService', () => {
  ...
  beforeEach(() => {
    homeDir = mkdtempSync(join(tmpdir(), 'oak-command-code-home-'))
  ...
  it.skipIf(process.platform !== 'win32')(
    ...
    () => {
      const spaceHome = join(tmpdir(), 'oak command-code home with spaces')
  ...
  it('installs a hook script that can recover the endpoint when Command Code strips token env', () => {
    ...
    const script = readFileSync(join(homeDir, '.oak', 'agent-hooks', scriptFileName), 'utf8')

    if (process.platform === 'win32') {
      expect(script).toContain('sourceEndpointByPort')
      expect(script).toContain('oak-dev\\agent-hooks')
      expect(script).toContain('set OAK_AGENT_HOOK_PORT=')
    } else {
      ...
      expect(script).toContain('__oak_read_ancestor_var')
      expect(script).toContain('__oak_fill_from_endpoint_file')
      expect(script).toContain('[ "$__oak_endpoint_port" != "$OAK_AGENT_HOOK_PORT" ]')
      expect(script).toContain('OAK_PANE_KEY')
      expect(script).toContain('OAK_AGENT_LAUNCH_TOKEN')
      expect(script).toContain('oak-dev/agent-hooks')
  ...
  itPosix('does not let stale endpoint files clobber recovered connection fields', async () => {
    ...
    const server = createServer((req, res) => {
      ...
      req.on('end', () => {
        requests.push({ body, token: req.headers['x-oak-agent-hook-token'] })
        res.statusCode = 204
        res.end()
      })
    })
    ...
    try {
      ...
      const scriptPath = join(homeDir, '.oak', 'agent-hooks', 'command-code-hook.sh')
      const child = spawn('/bin/sh', [scriptPath], {
        env: {
          ...process.env,
          HOME: homeDir,
          OAK_AGENT_HOOK_ENDPOINT: staleEndpointPath,
          OAK_AGENT_HOOK_PORT: String(address.port),
          OAK_AGENT_HOOK_TOKEN: 'current-token',
          OAK_PANE_KEY: 'tab:leaf',
          OAK_TAB_ID: 'tab',
          OAK_WORKTREE_ID: 'worktree',
          OAK_AGENT_HOOK_ENV: 'development',
          OAK_AGENT_HOOK_VERSION: '1'
        },
