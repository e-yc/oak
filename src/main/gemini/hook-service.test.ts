...
describe('GeminiHookService', () => {
  ...
  beforeAll(() => {
    homeDir = mkdtempSync(join(tmpdir(), 'oak-gemini-home-'))
    userDataDir = mkdtempSync(join(tmpdir(), 'oak-gemini-userdata-'))
  ...
  it('removes stale PreToolUse hooks when reinstalling managed Gemini hooks', () => {
    ...
    const staleManagedHookPath =
      process.platform === 'win32'
        ? `C:\\Users\\ramzi\\.oak\\agent-hooks\\${managedHookFileName}`
        : `/Users/ramzi/.oak/agent-hooks/${managedHookFileName}`
    const staleManagedCommand =
      process.platform === 'win32'
        ? staleManagedHookPath
        : `if [ -x '${staleManagedHookPath}' ]; then /bin/sh '${staleManagedHookPath}'; fi`
    const managedHookPath = join(homeDir, '.oak', 'agent-hooks', managedHookFileName)
  ...
  it.skipIf(process.platform !== 'win32')(
    ...
    () => {
      const spaceHome = join(tmpdir(), 'oak gemini home with spaces')
