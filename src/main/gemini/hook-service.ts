...
// (approvals flow through inline UI), so Oak cannot surface a waiting state
// for Gemini — that is an upstream limitation, not an Oak bug.
//
...
function getManagedScript(target: 'local' | 'posix' = 'local'): string {
  ...
  return [
    ...
    'curl -sS -X POST "http://127.0.0.1:${OAK_AGENT_HOOK_PORT}/hook/gemini" \\',
...
export class GeminiHookService {
  ...
  install(): AgentHookInstallStatus {
    ...
    // Why: when Oak stops subscribing to an event, install() must sweep the
  ...
  // Why: install Oak's managed Gemini hooks on the remote box. Mirrors
  ...
  async installRemote(sftp: SFTPWrapper, remoteHome: string): Promise<AgentHookInstallStatus> {
    ...
    const remoteScriptPath = `${remoteHome.replace(/\/$/, '')}/.oak/agent-hooks/gemini-hook.sh`
    try {
