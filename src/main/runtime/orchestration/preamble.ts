...
// Why: the dispatch preamble teaches agents about Oak's CLI commands for
...
export function buildDispatchPreamble(params: PreambleParams): string {
  // Why: in dev mode, agents must use oak-dev to connect to the dev runtime's
  // socket. Without this, agents inside the dev Electron app would call the
  // production CLI and talk to the wrong Oak instance (Section 6.4).
  const cli = params.devMode ? 'oak-dev' : 'oak'

  const header = `You are working inside Oak, a multi-agent IDE. You are a dispatched worker.
