...
import { RuntimeBrowserCommands } from './oak-runtime-browser'
...
import { RuntimeEmulatorCommands, setEmulatorBridge } from './oak-runtime-emulator'
import { serveSimStateWatcher } from '../emulator/serve-sim-state-watcher'
import type { EmulatorBridge } from '../emulator/emulator-bridge'
import { RuntimeFileCommands } from './oak-runtime-files'
import { RuntimeGitCommands } from './oak-runtime-git'
...
function createTerminalRevealWarning(handle: string, error?: unknown): string {
  const reason =
    error instanceof Error && error.message.trim().length > 0
      ...
      : ''
  return [
    `Terminal ${handle} is running, but Oak could not make it discoverable.${reason}`,
    `Run \`oak terminal focus --terminal ${handle}\` to reveal and focus it.`
  ].join(' ')
}
...
export class OakRuntimeService {
  ...
  getStatus(): RuntimeStatus {
    ...
    return {
      ...
      // Why: headless oak serve cannot create/stream BrowserViews, so clients
  ...
  // RPC, but agents need their own handle at spawn time (via OAK_TERMINAL_HANDLE
  ...
  // Why: daemon-backed PTYs that the runtime adopted after an Oak relaunch
  ...
  async addRepo(path: string, kind: 'git' | 'folder' = 'git'): Promise<Repo> {
    ...
    if (!isAbsolute(path)) {
      ...
      // server-side repo paths to be explicit so `oak serve` cwd is irrelevant.
      throw new Error('Project path must be an absolute path')
    }
  ...
  async getRepoHooks(repoSelector: string) {
    ...
    if (repo.connectionId) {
      ...
      try {
        ...
        const hooks = result.isBinary ? null : parseOakYaml(result.content)
        return {
          hasHooksFile: Boolean(hooks),
          hooks,
          setupRunPolicy: getEffectiveSetupRunPolicy(repo),
          source: hooks ? 'oak.yaml' : null,
    ...
    return {
      ...
      hooks,
      setupRunPolicy,
      source: hasFile ? 'oak.yaml' : hooks ? 'legacy' : null,
      setupTrust: this.getSharedSetupHookTrustPayload(
        repo,
  ...
  async checkRepoHooks(repoSelector: string) {
    ...
    if (repo.connectionId) {
      ...
      try {
        ...
        return { hasHooks: true, hooks: parseOakYaml(result.content), mayNeedUpdate: false }
      } catch {
        return { hasHooks: false, hooks: null, mayNeedUpdate: false }
      }
    }
    ...
    return {
      hasHooks: has,
      hooks,
      mayNeedUpdate: has && !hooks && hasUnrecognizedOakYamlKeys(repo.path)
    }
  }
  ...
  private async readRemoteSharedIssueCommand(
    ...
    try {
      const result = await fsProvider.readFile(joinWorktreeRelativePath(repoPath, 'oak.yaml'))
      if (result.isBinary) {
        return null
      }
      return parseOakYaml(result.content)?.issueCommand?.trim() || null
    } catch {
      return null
    }
  }
  ...
  async writeRepoIssueCommand(repoSelector: string, content: string): Promise<{ ok: true }> {
    ...
    if (repo.connectionId) {
      ...
      await this.ensureRemoteOakDirIgnored(fsProvider, repo.path)
  ...
  private async ensureRemoteOakDirIgnored(
    fsProvider: IFilesystemProvider,
    repoPath: string,
    ...
    try {
      ...
      if (!isENOENT(error)) {
        ...
        console.warn('[runtime] Could not inspect remote .gitignore for .oak', error)
        return
      }
      try {
        ...
        console.warn('[runtime] Could not update remote .gitignore to exclude .oak', writeError)
      }
      return
    }
    if (result.isBinary) {
      if (options.required) {
        throw new Error('Remote .gitignore is binary; cannot verify .oak is ignored')
      }
      return
    }
    if (/^\.oak\/?$/m.test(result.content)) {
      return
    }
  ...
  async listDetectedManagedWorktrees(repoSelector: string): Promise<DetectedWorktreeListResult> {
    ...
    const detected = scan.worktrees.map((gitWorktree) => {
      ...
      return {
        ...detectedWorktree,
        visible: true,
        ownership: detectedWorktree.ownership === 'oak-managed' ? 'oak-managed' : 'unknown-legacy'
  ...
  private recordCreatedWorktreeLineage(
    ...
    if (
      ...
      warnings.push({
        ...
        message:
          'Worktree created, but Oak could not record lineage because instance identity was unavailable.',
        details: {
  ...
  async createManagedWorktree(args: {
    ...
    // Why: CLI-managed WSL worktrees live under ~/oak/workspaces inside the
    ...
    const displayNameMeta = requestedDisplayName
      ...
      : shouldSetDisplayName(effectiveRequestedName, branchName, effectiveSanitizedName)
        ...
        : {}
    const meta = this.store.setWorktreeMeta(worktreeId, {
      // Why: worktree IDs are path-derived. If a path is deleted outside Oak
    ...
    // defers to the repo's oak.yaml setupRunPolicy. runHooks === true maps
    ...
    if (shouldRunSetup && hooks?.scripts.setup) {
      ...
      warning = `oak.yaml setup hook skipped for ${worktreePath}; pass --setup run to run it.`
    ...
    if (sequencedStartup && this.ptyController?.spawn) {
      try {
        ...
        // session later, matching `oak terminal create` background semantics.
        const startupTrustAgent = effectiveDraftPaste?.agent ?? effectiveCreatedWithAgent
        if (startupTrustAgent) {
  ...
  async getOrStartRemoteFetch(
    ...
    const promise = this.enqueueRemoteFetch(key, () =>
      ...
      // create on this repo until Oak restarted (the F2 bug §3.3 pins).
  ...
  async updateManagedWorktreeMeta(
    ...
    this.store.setWorktreeMeta(worktree.id, stripOakProvenanceMetaUpdates(persistedMetaUpdates))
  ...
  private async resolveWorktreeRemovalTarget(
    ...
    try {
      ...
      // Only exact IDs with persisted Oak metadata are accepted here so
  ...
  async removeManagedWorktree(
    ...
    const removal = (async (): Promise<RemoveWorktreeResult & { warning?: string }> => {
      ...
      if (hooks?.scripts.archive && runHooks) {
        ...
        // Runtime RPC calls have no renderer trust prompt, so hooks require explicit CLI opt-in.
        warning = `oak.yaml archive hook skipped for ${canonicalWorktreePath}; pass --run-hooks to run it.`
        console.warn(`[hooks] ${warning}`)
      }
  ...
  async createTerminal(
    ...
    const shouldCreateInBackground =
      ...
      ((!requiresRendererFocus && opts.rendererBacked !== true) ||
        // Why: `oak serve` exposes the local runtime without a renderer
  ...
  private async resolveMobileSessionTerminalCommand(
    ...
    // Why: an SSH workspace runs the CLI through the relay shim (plain `oak`),
    // so the Linux-only `oak-ide` rename must not be applied.
  ...
  async prepareClaudeAgentTeamsLeader(args: {
    ...
    if (!handle) {
      throw new Error('claude_agent_teams_requires_oak_terminal')
    }
  ...
  markRendererReloading(windowId: number): void {
    ...
    // Why: handleByPtyId maps ptyId → pre-allocated CLI handle (OAK_TERMINAL_HANDLE).
  ...
  markGraphUnavailable(windowId: number): void {
    ...
    // Why: once the authoritative renderer graph disappears, Oak must fail
  ...
  private buildTerminalWorkspaceEnv(
    ...
    return {
      ...env,
      OAK_WORKSPACE_ID: scope.id,
      OAK_PROJECT_GROUP_ID: scope.folderWorkspace.projectGroupId,
      OAK_WORKSPACE_ROOT: scope.folderWorkspace.folderPath
    }
  }
  ...
  private async resolveLineageForWorktreeCreate(
    ...
    if (input.envParentWorkspace) {
      try {
        ...
        warnings.push({
          code: 'LINEAGE_PARENT_CONTEXT_MISSING',
          message: 'Worktree created, but Oak could not validate the environment parent context.',
    ...
    if (input.callerTerminalHandle) {
      try {
        ...
        warnings.push({
          code: 'LINEAGE_PARENT_CONTEXT_MISSING',
          message:
            'Worktree created, but Oak could not validate the caller terminal as a parent context.',
        ...
        warnings: [
          {
            ...
            message: 'Worktree created, but Oak could not prove which parent context caused it.',
            details: {
  ...
  private async computeResolvedWorktrees(generation: number): Promise<ResolvedWorktree[]> {
    ...
    // allowed to change outside Oak. A short TTL avoids shelling out on every
    ...
    for (const tab of snapshot.tabs) {
      if (tab.type === 'browser') {
        ...
        tabs.push({
          ...
          // active Oak tab. Preserve the renderer's app-level session focus.
  ...
  private rememberDetachedPreAllocatedLeaves(): void {
    for (const leaf of this.leaves.values()) {
      if (leaf.ptyId && this.handleByPtyId.has(leaf.ptyId)) {
        // Why: OAK_TERMINAL_HANDLE is an agent identity, so CLI control should
  ...
  async linearResolveCurrentIssue(
    ...
    if (!worktree && context?.remote !== true && context?.cwd) {
      ...
      if (!worktree) {
        throw new LinearAgentAccessError(
          'linear_issue_required',
          'Run --current from inside an Oak-managed worktree or pass an issue id.'
        )
      }
    }
      ...
      await this.runLinearAgentWrite(
        ...
        (cause) =>
          linearError(
            ...
            'Linear may have applied the state change, but Oak could not confirm it.',
            {
              nextSteps: [
                `Run \`oak linear issue ${target.issue.identifier} --workspace ${target.workspaceId} --json\` and check the current state before retrying.`
              ],
              ...(cause ? { cause } : {})
            }
          )
    ...
    if (!alreadySet) {
      await this.runLinearAgentWrite(
        ...
        (cause) =>
          linearError(
            'linear_write_unconfirmed',
            'Linear may have applied the task update, but Oak could not confirm it.',
            {
              nextSteps: [
                `Run \`oak linear issue ${target.issue.identifier} --workspace ${target.workspaceId} --json\` and check the updated field before retrying.`
              ],
              ...(cause ? { cause } : {})
            }
          )
    ...
    if (compatibleNameMatches.length > 1) {
      throw linearError(
        ...
        `Multiple Linear projects exactly matched "${trimmed}".`,
        {
          ...
          nextSteps: ['Run `oak linear project list --query <name> --json` and retry by id.']
        }
      )
    }
    ...
    const resolved = inputs.map((input) => {
      ...
      throw linearError(
          ...
          : `Multiple labels exactly matched "${input}".`,
        {
          ...
          nextSteps: ['Run `oak linear team labels --team <key-or-id> --json` and retry by id.']
        }
      )
    })
    ...
    try {
      ...
      if (!root) {
        throw linearError(
          ...
          {
            nextSteps: ['Run `oak linear issue <id> --comments --json` to list valid comment ids.']
          }
        )
      }
      return root.id
  ...
  private mapLinearReadFailure(error: unknown): LinearAgentAccessError {
    ...
    if (isLinearAuthError(error)) {
      return linearError('linear_auth_expired', 'Linear authentication expired.', {
        nextSteps: ['Reconnect Linear from Oak settings.']
      })
    }
  ...
  private async resolveLinearCreateTeam(
    ...
    if (!teamInput) {
      throw linearError('linear_team_required', 'Pass --team or create under a parent issue.', {
        nextSteps: ['Run `oak linear create --team <key> ...` or use --parent-current.']
      })
    }
    ...
    if (teams.length === 0 && (getLinearStatus().workspaces?.length ?? 0) === 0) {
      throw linearError('linear_not_connected', 'Linear is not connected.', {
        nextSteps: ['Connect Linear from Oak settings, then retry the issue create.']
      })
    }
  ...
  private linearCreateStyleUnconfirmed(
    ...
    const pinned =
      verb === 'create'
        ...
        : [
            `oak linear ${verb === 'attach' ? 'attach' : 'comment add'}`,
    ...
    return linearError(
      'linear_write_unconfirmed',
      'Linear may have applied the write, but Oak could not confirm it.',
      {
        writeId,
        workspaceId,
...
function branchSelectorMatches(branch: string, selector: string): boolean {
  ...
  // or `foo` depending on which plumbing path produced the record. Oak's
