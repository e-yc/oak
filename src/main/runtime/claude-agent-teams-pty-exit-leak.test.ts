 ...
 * called solely from `OakRuntimeService.closeTerminal` (the explicit user-close IPC).
...
function internals(runtime: OakRuntimeService): RuntimeInternals {
  return runtime as unknown as RuntimeInternals
}

function registerTeam(runtime: OakRuntimeService, ptyId: string, leaderHandle: string): void {
  const { claudeAgentTeams, handleByPtyId } = internals(runtime)
  claudeAgentTeams.createLaunchEnv({
    leaderHandle,
    baseEnv: {},
    shimDir: '/tmp/oak-shim',
    shimBin: 'oak'
  })
