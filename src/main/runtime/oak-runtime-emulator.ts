...
// Why: dedicated file for "one surface" separation (emulator), parallel to oak-runtime-browser.ts. Keeps OakRuntimeService focused; emulator routing easy to scan. No max-lines disable (split further if grows; per AGENTS + plan Phase 3).
...
export class RuntimeEmulatorCommands {
  ...
  async emulatorAttach(params: {
    ...
    if (worktreeId) {
      ...
      if (reusable) {
        ...
        serveSimStateWatcher.markOakManaged(reusable)
      ...
      if (stoppedUdid) {
        serveSimStateWatcher.unmarkOakManaged(stoppedUdid)
      }
    }
  ...
  // devices/AVDs) for the cross-platform `oak emulator devices` command.
  ...
  async emulatorKill(params: {
    ...
    serveSimStateWatcher.unmarkOakManaged(killedUdid)
  ...
  async emulatorShutdown(params: {
    ...
    if (params.managedOnly && worktreeId && !params.device && !params.emulator) {
      ...
      if (shutdownUdid) {
        serveSimStateWatcher.unmarkOakManaged(shutdownUdid)
      }
