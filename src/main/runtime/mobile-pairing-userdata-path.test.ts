...
// app.setName('Oak') changing how app.getPath('userData') resolves (e.g. from
// lowercase 'oak' to uppercase 'Oak' on a case-sensitive filesystem) — the
...
describe('mobile pairing userData path stability', () => {
  ...
  beforeEach(() => {
    root = mkdtempSync(join(tmpdir(), 'oak-pairing-path-'))
  ...
  it('keeps returning the path captured before app.setName changes resolution', async () => {
    ...
    // app.setName('Oak') happens later in startup, changing late resolution.
  ...
  it('writes DeviceRegistry + E2EE keypair under the canonical path, not the late one', async () => {
    ...
    appState.userData = lateDir // app.setName('Oak') has run by the time the runtime starts

    const { DeviceRegistry } = await import('./device-registry')
    const { loadOrCreateE2EEKeypair } = await import('./e2ee-keypair')

    // Mirrors OakRuntimeRpcServer.start(): both read from the same userDataPath.
    ...
    // Pairing credentials land beside oak-data.json so they survive restarts/updates.
