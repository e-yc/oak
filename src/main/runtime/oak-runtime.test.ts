...
vi.mock('../hooks', () => ({
  ...
  parseOakYaml: vi.fn().mockReturnValue(null)
}))
...
function resetRuntimeTestMocks(): void {
  ...
  vi.mocked(parseOakYaml).mockReset()
  ...
  vi.mocked(parseOakYaml).mockReturnValue(null)
...
function antigravityReadyScreen(model = 'Gemini 3.5 Flash (High)'): string {
  return [
    ...
    model,
    '~/oak/workspaces/oak/agy-dispatch-issue',
    '>'
  ].join('\n')
}
...
function expectStablePaneKeyEnv(env: Record<string, string>): string {
  expect(env.OAK_TAB_ID).toMatch(UUID_RE)
  const leafId = env.OAK_PANE_KEY?.slice(`${env.OAK_TAB_ID}:`.length)
  expect(leafId).toMatch(UUID_RE)
  expect(env.OAK_PANE_KEY).toBe(`${env.OAK_TAB_ID}:${leafId}`)
  return env.OAK_PANE_KEY
}

function createRuntime(): OakRuntimeService {
  return new OakRuntimeService(store)
}
...
describe('OakRuntimeService', () => {
  it('projects worktree card display settings to paired clients', () => {
    const runtime = new OakRuntimeService({
      ...store,
      getSettings: () => ({
        ...store.getSettings(),
    ...
    vi.mocked(listWorktrees).mockResolvedValueOnce([
      {
        ...
        head: 'abc',
        branch: 'refs/heads/wolfiesch/oak-skill-smoke',
        isBare: false,
        isMainWorktree: false
      }
    ])
  ...
  it('routes SSH-backed forward-slash UNC file and git paths without collapsing the root', async () => {
    ...
    const runtime = new OakRuntimeService(remoteStore as never)
  ...
  it.each([
    ...
    const folderPath = await mkdtemp(join(tmpdir(), 'oak-runtime-folder-files-'))
  ...
  it('lists persisted SSH worktrees while the git provider is unavailable', async () => {
    ...
    const runtime = new OakRuntimeService(runtimeStore as never)
  ...
  it('creates additional workspace metadata for folder-mode repos through runtime create', async () => {
    ...
    const runtime = new OakRuntimeService(runtimeStore as never, undefined, {
  ...
  it('launches SSH setup terminals for runtime task-created worktrees', async () => {
    ...
    const provider = {
      exec: vi.fn(async (args: string[]) => {
        ...
        if (args[0] === 'rev-parse' && args[1] === '--git-path') {
          return {
            stdout: '/remote/repo/.git/worktrees/mobile-setup/oak/setup-runner.sh\n',
            stderr: ''
          }
        }
        if (args[0] === 'rev-parse') {
    ...
    try {
      ...
      expect(spawn).toHaveBeenNthCalledWith(
        ...
        expect.objectContaining({
          ...
          command: expect.stringContaining(
            'bash /remote/repo/.git/worktrees/mobile-setup/oak/setup-runner.sh'
          ),
          worktreeId: result.worktree.id
        })
      )
      ...
      const markerPath = `/remote/repo/.git/worktrees/mobile-setup/oak/setup-runner.sh.${nonceMatch![1]}.done`
  ...
  it('honors split setup placement for SSH worktrees without startup agents', async () => {
    ...
    const provider = {
      exec: vi.fn(async (args: string[]) => {
        ...
        if (args[0] === 'rev-parse' && args[1] === '--git-path') {
          return {
            stdout: '/remote/repo/.git/worktrees/mobile-setup-split/oak/setup-runner.sh\n',
            stderr: ''
          }
        }
        if (args[0] === 'rev-parse') {
    ...
    try {
      ...
      expect(setupEnv.OAK_TAB_ID).toBe(initialEnv.OAK_TAB_ID)
      const initialLeafId = initialEnv.OAK_PANE_KEY!.slice(`${initialEnv.OAK_TAB_ID!}:`.length)
      expect(revealTerminalSession).toHaveBeenLastCalledWith(
        result.worktree.id,
        expect.objectContaining({
          ptyId: 'pty-remote-setup-split',
          tabId: initialEnv.OAK_TAB_ID,
          activate: false,
  ...
  it('reads SSH repo hooks through the SSH filesystem provider', async () => {
    ...
    vi.mocked(parseOakYaml).mockReturnValue({ scripts: { setup: 'pnpm install' } })
    registerSshFilesystemProvider('ssh-1', fsProvider as never)
    const runtime = new OakRuntimeService(remoteStore as never)
    ...
    expect(fsProvider.readFile).toHaveBeenCalledWith('C:\\remote\\repo\\oak.yaml')
    expect(hasHooksFile).not.toHaveBeenCalled()
    expect(getEffectiveHooks).not.toHaveBeenCalled()
  })

  it('hashes only the shared oak.yaml setup script for local run-both hooks', async () => {
  ...
  it('uses remote path joins for SSH hook checks and issue-command files', async () => {
    ...
    const fsProvider = {
      readFile: vi.fn(async (filePath: string) => ({
        content: filePath.endsWith('oak.yaml')
          ? 'scripts:\n  setup: pnpm install\n'
    ...
    try {
      ...
      await expect(runtime.readRepoIssueCommand('id:repo-1')).resolves.toMatchObject({
        ...
        localFilePath: 'C:\\remote\\repo\\.oak\\issue-command'
      })
    ...
    expect(fsProvider.readFile).toHaveBeenCalledWith('C:\\remote\\repo\\oak.yaml')
    expect(fsProvider.readFile).toHaveBeenCalledWith('C:\\remote\\repo\\.oak\\issue-command')
    expect(fsProvider.createDir).toHaveBeenCalledWith('C:\\remote\\repo\\.oak')
    expect(fsProvider.writeFile).toHaveBeenCalledWith(
      'C:\\remote\\repo\\.oak\\issue-command',
      'Ship it\n'
    )
    expect(fsProvider.writeFile).toHaveBeenCalledWith(
  ...
  it('resolves SSH issue commands from shared oak.yaml and deletes empty overrides', async () => {
    const remoteStore = {
      ...store,
      getRepos: () => [
        {
    ...
    vi.mocked(parseOakYaml).mockReturnValue({
      scripts: {},
      issueCommand: 'claude -p "Fix #{{issue}}"'
    })
    const fsProvider = {
      readFile: vi.fn(async (filePath: string) => {
        if (filePath.endsWith('.oak/issue-command')) {
          throw Object.assign(new Error('missing'), { code: 'ENOENT' })
        }
        if (filePath.endsWith('oak.yaml')) {
    ...
    try {
      await expect(runtime.readRepoIssueCommand('id:repo-1')).resolves.toMatchObject({
        ...
        localFilePath: '/remote/repo/.oak/issue-command',
        source: 'shared'
      })
    ...
    expect(fsProvider.readFile).toHaveBeenCalledWith('/remote/repo/oak.yaml')
    expect(fsProvider.deletePath).toHaveBeenCalledWith('/remote/repo/.oak/issue-command', false)
    expect(fsProvider.writeFile).not.toHaveBeenCalledWith(
      '/remote/repo/.oak/issue-command',
      expect.anything()
    )
  })
  ...
  it('routes runtime GitHub repo identity helpers through the selected WSL project runtime', async () => {
    ...
    const runtime = new OakRuntimeService(runtimeStore as never)
    getRepoSlugMock.mockResolvedValueOnce({ owner: 'acme', repo: 'oak' })
    getRepoUpstreamMock.mockResolvedValueOnce({ owner: 'stablyai', repo: 'oak' })
  ...
  it('routes runtime GitHub issue and work-item actions through the selected WSL project runtime', async () => {
    ...
    createGitHubIssueMock.mockResolvedValueOnce({
      ok: true,
      number: 12,
      url: 'https://github.com/acme/oak/issues/12'
    })
  ...
  it('routes runtime GitHub PR details and actions through the selected WSL project runtime', async () => {
    ...
    const runtime = new OakRuntimeService(runtimeStore as never)
    const localGitOptions = { wslDistro: 'Ubuntu' }
    const prRepo = { owner: 'acme', repo: 'oak' }
  ...
  it('rejects hosted review worktree selectors outside the selected repo', async () => {
    ...
    const runtime = new OakRuntimeService(multiRepoStore as never)
  ...
  it('routes local WSL project hosted review flows through runtime git options', async () => {
    ...
    const runtime = new OakRuntimeService(wslStore as never)
    getHostedReviewForBranchMock.mockResolvedValueOnce({
      provider: 'github',
      number: 76,
      title: 'Feature WSL',
      state: 'open',
      url: 'https://github.com/acme/oak/pull/76',
      status: 'success',
    ...
    createHostedReviewMock.mockResolvedValueOnce({
      ok: true,
      number: 77,
      url: 'https://github.com/acme/oak/pull/77'
    })
  ...
  it('deduplicates runtime repo paths with Windows/UNC comparison semantics', async () => {
    ...
    const runtime = new OakRuntimeService(uncStore as never)
  ...
  it('browses runtime server directories before projects are added', async () => {
    const tempRoot = await mkdtemp(join(tmpdir(), 'oak-runtime-browse-'))
    try {
  ...
  it('defaults runtime addRepo badgeColor to DEFAULT_REPO_BADGE_COLOR', async () => {
    ...
    const runtime = new OakRuntimeService(colorStore as never)
  ...
  it('sets up an existing folder on a fresh runtime after importing the repo project', async () => {
    const tempRoot = await mkdtemp(join(tmpdir(), 'oak-runtime-project-setup-'))
    const repos: Record<string, unknown>[] = []
    getRepoUpstreamMock.mockResolvedValueOnce({ owner: 'e-yc', repo: 'oak' })
    const runtimeStore = {
      ...store,
    ...
    try {
      ...
      expect(result.project.id).toBe('github:e-yc/oak')
  ...
  it('defaults runtime createRepo badgeColor to DEFAULT_REPO_BADGE_COLOR', async () => {
    ...
    const runtime = new OakRuntimeService(colorStore as never)
    const parentDir = await mkdtemp('/tmp/oak-runtime-create-')
    try {
  ...
  it('creates a missing runtime parent before creating the project directory', async () => {
    ...
    const runtime = new OakRuntimeService(createStore as never)
    const tempRoot = await mkdtemp('/tmp/oak-runtime-create-parent-')
    const parentDir = join(tempRoot, 'oak', 'projects')
    try {
  ...
  it('prepares the runtime worktree root when creating a repo', async () => {
    ...
    const runtime = new OakRuntimeService(runtimeStore as never)
    const parentDir = await mkdtemp('/tmp/oak-runtime-create-root-prep-')
    try {
  ...
  it('rejects runtime cloneRepo dot-segment URLs before spawning git', async () => {
    const spawnSpy = vi.spyOn(gitRunner, 'wslAwareSpawn')
    const runtime = createRuntime()
    const tempRoot = await mkdtemp(join(tmpdir(), 'oak-runtime-clone-'))
  ...
  it('removes an owned runtime clone target when git exits unsuccessfully', async () => {
    ...
    const runtime = createRuntime()
    const destination = await mkdtemp(join(tmpdir(), 'oak-runtime-clone-'))
  ...
  it('emits explicit OSC 9999 agent status from runtime PTY data', () => {
    ...
    const runtime = new OakRuntimeService(store, undefined, {
  ...
  it('creates visible terminal sessions without asking the renderer to focus a tab', async () => {
    ...
    // pre-allocate the tabId and stamp OAK_PANE_KEY/TAB_ID/WORKTREE_ID into
    ...
    const spawnedLeafId = spawnedEnv.OAK_PANE_KEY.slice(`${spawnedEnv.OAK_TAB_ID}:`.length)
    expect(spawnedEnv.OAK_WORKTREE_ID).toBe(TEST_WORKTREE_ID)
    expect(spawnedEnv.OAK_AGENT_LAUNCH_TOKEN).toMatch(UUID_RE)
    expect(revealTerminalSession).toHaveBeenCalledWith(TEST_WORKTREE_ID, {
      ...
      launchToken: spawnedEnv.OAK_AGENT_LAUNCH_TOKEN,
      activate: false,
      tabId: spawnedEnv.OAK_TAB_ID,
  ...
  it('injects runtime hook receiver env into terminal sessions', async () => {
    ...
    expect(spawnCall?.env).toEqual(
      expect.objectContaining({
        ...
        OAK_AGENT_HOOK_ENV: 'remote',
        OAK_AGENT_HOOK_VERSION: '1',
        OAK_PANE_KEY: expect.any(String),
        OAK_TAB_ID: expect.any(String),
        OAK_WORKTREE_ID: TEST_WORKTREE_ID
      })
    )
    expect(spawnCall?.env?.OAK_AGENT_HOOK_ENDPOINT).toBeUndefined()
  })
  ...
  it.each([
    ...
    const folderPath = await mkdtemp(join(tmpdir(), 'oak-runtime-folder-workspace-'))
    ...
    expectStablePaneKeyEnv(spawnedEnv)
    expect(spawnedEnv.OAK_WORKSPACE_ID).toBe(TEST_FOLDER_WORKSPACE_KEY)
    expect(spawnedEnv.OAK_PROJECT_GROUP_ID).toBe(TEST_FOLDER_PROJECT_GROUP_ID)
    expect(spawnedEnv.OAK_WORKSPACE_ROOT).toBe(folderPath)
    expect(spawnedEnv.OAK_WORKTREE_ID).toBe(TEST_FOLDER_WORKSPACE_KEY)
  })
  ...
  it.each([
    ...
    expect(spawnCall?.env?.OAK_WORKTREE_ID).toBe(FLOATING_TERMINAL_WORKTREE_ID)
    expect(spawnCall?.env?.OAK_WORKSPACE_ID).toBeUndefined()
    expect(spawnCall?.env?.OAK_PROJECT_GROUP_ID).toBeUndefined()
    expect(spawnCall?.env?.OAK_WORKSPACE_ROOT).toBeUndefined()
  })

  it('rejects folder workspace terminal creation when the backing path is missing', async () => {
    const missingPath = join(tmpdir(), `oak-missing-folder-workspace-${randomUUID()}`)
    const spawn = vi.fn().mockResolvedValue({ id: 'pty-folder' })
  ...
  it('rejects folder workspace folderPath updates when the new path is missing', async () => {
    const missingPath = join(tmpdir(), `oak-missing-folder-update-${randomUUID()}`)
  ...
  it('enables Claude Agent Teams only for direct Claude launches when configured in-process', async () => {
    ...
    const runtime = new OakRuntimeService(runtimeStore)
    runtime.setPtyController({
      spawn,
      write: () => true,
      kill: () => true,
  ...
  it('restores captured native Claude Agent Teams mode with fresh service env', async () => {
    ...
    await runtime.createTerminal(`path:${TEST_WORKTREE_PATH}`, {
      ...
      env: {
        CLAUDE_PROFILE: 'captured',
        OAK_AGENT_TEAMS_TEAM_ID: 'stale-team',
        OAK_AGENT_TEAMS_TOKEN: 'stale-token',
        TMUX: '/tmp/oak-claude-agent-teams/stale-team,0,1'
      },
      launchAgent: 'claude',
      launchConfig: {
        agentCommand: 'claude',
    ...
    expect(spawnCall?.env?.OAK_AGENT_TEAMS_TEAM_ID).toMatch(/^team-/)
    expect(spawnCall?.env?.OAK_AGENT_TEAMS_TEAM_ID).not.toBe('stale-team')
    expect(spawnCall?.env?.OAK_AGENT_TEAMS_TOKEN).not.toBe('stale-token')
    expect(spawnCall?.env?.TMUX).not.toBe('/tmp/oak-claude-agent-teams/stale-team,0,1')
    ...
    expect(revealedLaunchConfig?.agentEnv.OAK_AGENT_TEAMS_TEAM_ID).not.toBe('stale-team')
    expect(revealedLaunchConfig?.agentEnv.OAK_AGENT_TEAMS_TOKEN).not.toBe('stale-token')
  })
  ...
  it('adopts renderer pane identity for remote runtime terminal creates', async () => {
    ...
    const spawnedEnv =
      (spawn.mock.calls[0]?.[0] as { env?: Record<string, string> } | undefined)?.env ?? {}
    expect(spawnedEnv.OAK_TAB_ID).toBe(tabId)
    expect(spawnedEnv.OAK_PANE_KEY).toBe(`${tabId}:${leafId}`)
  })
  ...
  it('does not adopt web mirror ids as host terminal ids', async () => {
    ...
    const spawnedEnv =
      (spawn.mock.calls[0]?.[0] as { env?: Record<string, string> } | undefined)?.env ?? {}
    expect(spawnedEnv.OAK_TAB_ID).not.toBe(tabId)
    expect(spawnedEnv.OAK_TAB_ID).not.toMatch(/^web-terminal-/)
    expect(spawnedEnv.OAK_PANE_KEY).toMatch(`${spawnedEnv.OAK_TAB_ID}:`)
  })
  ...
  it('splits visible pty-backed terminal sessions through the parent renderer tab', async () => {
    ...
    const sourceEnv =
      (spawn.mock.calls[0]?.[0] as { env?: Record<string, string> } | undefined)?.env ?? {}
    const sourceLeafId = sourceEnv.OAK_PANE_KEY.slice(`${sourceEnv.OAK_TAB_ID}:`.length)

    await expect(runtime.splitTerminal(handle, { direction: 'vertical' })).resolves.toMatchObject({
      handle: expect.stringMatching(/^term_/),
      tabId: sourceEnv.OAK_TAB_ID,
      paneRuntimeId: -1
    })

    const splitEnv =
      (spawn.mock.calls[1]?.[0] as { env?: Record<string, string> } | undefined)?.env ?? {}
    const splitLeafId = splitEnv.OAK_PANE_KEY.slice(`${sourceEnv.OAK_TAB_ID}:`.length)
    expect(splitTerminal).not.toHaveBeenCalled()
    expect(splitEnv.OAK_TAB_ID).toBe(sourceEnv.OAK_TAB_ID)
    expect(splitEnv.OAK_WORKTREE_ID).toBe(TEST_WORKTREE_ID)
    ...
    const siblingSurfaces = publishedTabs.filter(
      (tab): tab is Extract<typeof tab, { type: 'terminal' }> =>
        tab.type === 'terminal' && tab.parentTabId === sourceEnv.OAK_TAB_ID
    )
  ...
  it('splits folder workspace pty-backed terminal sessions with folder cwd and env', async () => {
    const folderPath = await mkdtemp(join(tmpdir(), 'oak-runtime-folder-split-'))
    const spawn = vi
      .fn()
    ...
    expect(splitEnv.OAK_TAB_ID).toBe(sourceEnv.OAK_TAB_ID)
    expect(splitEnv.OAK_WORKSPACE_ID).toBe(TEST_FOLDER_WORKSPACE_KEY)
    expect(splitEnv.OAK_PROJECT_GROUP_ID).toBe(TEST_FOLDER_PROJECT_GROUP_ID)
    expect(splitEnv.OAK_WORKSPACE_ROOT).toBe(folderPath)
    expect(splitEnv.OAK_WORKTREE_ID).toBe(TEST_FOLDER_WORKSPACE_KEY)
  ...
  it('returns an actionable discoverability warning when default adoption fails after spawn', async () => {
    ...
    try {
      ...
      expect(created.warning).toContain(`oak terminal focus --terminal ${created.handle}`)
      const spawnCall = spawn.mock.calls[0]?.[0] as { env?: Record<string, string> } | undefined
      const spawnedEnv = spawnCall?.env ?? {}
      expectStablePaneKeyEnv(spawnedEnv)
      const spawnedLeafId = spawnedEnv.OAK_PANE_KEY.slice(`${spawnedEnv.OAK_TAB_ID}:`.length)
  ...
  it('resolves tui-idle from a Codex ready prompt preview', async () => {
    ...
    runtime.onPtyData(
      ...
      [
        ...
        ' directory:   ~/oak/workspaces/oak/cli-debug\n'
      ].join(''),
      Date.now()
    )
    ...
    runtime.onPtyData(
      ...
      [
        ...
        [
          ...
          'Run /review on my current changes gpt-5.5 high ~/oak/workspaces/oak/cli-debug',
          'Run /review on my current changes gpt-5.5 high ~/oak/workspaces/oak/cli-debug',
          'Run /review on my current changes gpt-5.5 high ~/oak/workspaces/oak/cli-debug',
          'Run /review on my current changes gpt-5.5 high ~/oak/workspaces/oak/cli-debug\n'
        ].join('')
  ...
  // Why: PR #2553 fixed Oak CLI terminal reads so older retained output stays
  ...
  it('strips retained terminal path line and hash locators before matching', () => {
    const colonPath = '/tmp/oak report/result.json'
  ...
  it('keeps tmpdir artifact paths in bounded PTY path candidates', () => {
    const artifactPath = join(tmpdir(), 'oak-runtime-retained-result.json')
    const candidates = appendRecentPtyPathCandidates(undefined, `wrote ${artifactPath}\n`)
  ...
  it('matches terminal artifact paths only when they appear in recent terminal output', () => {
    expect(
      recentTerminalOutputIncludesPath(
        'wrote /tmp/oak report/result.json:12:3',
        '/tmp/oak report/result.json',
        '/tmp/oak report/result.json'
      )
    ).toBe(true)
    expect(
      recentTerminalOutputIncludesPath(
        '\x1b]8;;file:///tmp/oak%20report/result.json\x1b\\result\x1b]8;;\x1b\\',
        '/tmp/oak report/result.json',
        '/tmp/oak report/result.json'
      )
    ).toBe(true)
    ...
    expect(
      recentTerminalOutputIncludesPath(
        'wrote /tmp/oak report/other.json',
        '/tmp/oak report/result.json',
        '/tmp/oak report/result.json'
      )
    ).toBe(false)
  ...
  it('adopts preallocated OAK_TERMINAL_HANDLE as a valid runtime handle', async () => {
    const runtime = new OakRuntimeService(store)
  ...
  it('creates mobile session terminals for folder workspaces in a headless runtime server', async () => {
    const folderPath = await mkdtemp(join(tmpdir(), 'oak-mobile-folder-workspace-'))
  ...
  it('keeps the activated headless tab active across PTY republishes (serve focus-jump regression)', async () => {
    // Why: in `oak serve`, focusTerminal has no renderer to persist the remote
  ...
  it('rejects SSH lineage updates when Oak worktree identity is missing', async () => {
    const remoteRepo = {
      id: 'remote-repo',
      path: '/home/user/repo',
      displayName: 'remote',
  ...
  it('rejects local lineage updates when Oak worktree identity is missing', async () => {
    const tempRoot = await mkdtemp(join(tmpdir(), 'oak-runtime-lineage-'))
    ...
    try {
      ...
      await expect(lstat(join(childPath, '.oak'))).rejects.toThrow()
      await expect(lstat(join(parentPath, '.oak'))).rejects.toThrow()
      expect(setWorktreeLineage).not.toHaveBeenCalled()
    } finally {
      await rm(tempRoot, { recursive: true, force: true })
    }
  ...
  it('strips Oak provenance fields from runtime metadata updates', async () => {
    const metaById: Record<string, WorktreeMeta> = {
    ...
    await runtime.updateManagedWorktreeMeta(`id:${TEST_WORKTREE_ID}`, {
      ...
      oakCreatedAt: 123,
      oakCreationSource: 'runtime',
      oakCreationWorkspaceLayout: { path: '/tmp', nestWorkspaces: false }
    })
  ...
  it('keeps cwd-inferred lineage best-effort when the cwd parent cannot be resolved', async () => {
    ...
    expect(result.warnings).toEqual([
      expect.objectContaining({
        ...
        message:
          'Worktree created, but Oak could not validate the current directory as a parent context.'
      })
    ])
  })
  ...
  it('returns a setup launch payload for CLI-created worktrees when hooks are explicitly enabled', async () => {
    ...
    vi.mocked(createSetupRunnerScript).mockReturnValue({
      ...
      envVars: {
        ...
        OAK_WORKTREE_PATH: '/tmp/workspaces/runtime-hook-test'
      }
    })
    vi.mocked(listWorktrees).mockResolvedValueOnce([
      {
  ...
  it('passes setup payloads through when explicitly activating CLI-created worktrees', async () => {
    ...
    vi.mocked(createSetupRunnerScript).mockReturnValue({
      ...
      envVars: {
        OAK_ROOT_PATH: '/tmp/repo',
        OAK_WORKTREE_PATH: '/tmp/workspaces/runtime-hook-activate'
      }
    })
    vi.mocked(listWorktrees).mockResolvedValueOnce([
      {
  ...
  it('follows normal setup policy for CLI-created worktrees without activating them', async () => {
    ...
    vi.mocked(createSetupRunnerScript).mockReturnValue({
      ...
      envVars: {
        OAK_ROOT_PATH: '/tmp/repo',
        OAK_WORKTREE_PATH: '/tmp/workspaces/runtime-hook-skip'
      }
    })
    vi.mocked(listWorktrees).mockResolvedValue([
      {
    ...
    expect(spawn).toHaveBeenNthCalledWith(
      ...
      expect.objectContaining({
        ...
        // Why: createTerminal stamps OAK_PANE_KEY/TAB_ID/WORKTREE_ID into the
        // PTY env on top of the caller-supplied env so hook-based agent status
        // can attribute hook events to a stable pane.
        env: expect.objectContaining({
          OAK_ROOT_PATH: '/tmp/repo',
          OAK_WORKTREE_PATH: '/tmp/workspaces/runtime-hook-skip',
          OAK_TAB_ID: expect.stringMatching(UUID_RE),
          OAK_PANE_KEY: expect.any(String),
          OAK_WORKTREE_ID: result.worktree.id
        }),
        worktreeId: result.worktree.id
      })
    )
    ...
    const setupLeafId = setupSpawnEnv.OAK_PANE_KEY.slice(`${setupSpawnEnv.OAK_TAB_ID}:`.length)
    expect(revealTerminalSession).toHaveBeenLastCalledWith(result.worktree.id, {
      ptyId: 'pty-setup',
      title: 'Setup',
      activate: false,
      tabId: setupSpawnEnv.OAK_TAB_ID,
      leafId: setupLeafId
    })
  })
  ...
  it('sequences setup before startup for opted-in local headless worktree creates', async () => {
    ...
    vi.mocked(createSetupRunnerScript).mockReturnValue({
      ...
      envVars: {
        OAK_ROOT_PATH: '/tmp/repo',
        OAK_WORKTREE_PATH: '/tmp/workspaces/runtime-headless-startup-setup'
      },
  ...
  it('starts setup and startup side by side by default for local headless worktree creates', async () => {
    ...
    vi.mocked(createSetupRunnerScript).mockReturnValue({
      ...
      envVars: {
        OAK_ROOT_PATH: '/tmp/repo',
        OAK_WORKTREE_PATH: '/tmp/workspaces/runtime-headless-parallel'
      }
    })
    vi.mocked(listWorktrees).mockResolvedValue([
      {
    ...
    expect(spawn).toHaveBeenNthCalledWith(
      2,
      expect.objectContaining({ command: 'bash /tmp/repo/.git/oak/setup-runner.sh' })
    )
  })
  ...
  it('creates the first terminal for CLI-created worktrees without activating them', async () => {
    ...
    const initialLeafId = initialSpawnEnv.OAK_PANE_KEY.slice(
      `${initialSpawnEnv.OAK_TAB_ID}:`.length
    )
    expect(revealTerminalSession).toHaveBeenCalledWith(result.worktree.id, {
      ptyId: 'pty-created-worktree',
      title: null,
      activate: false,
      tabId: initialSpawnEnv.OAK_TAB_ID,
  ...
  it('honors split setup placement for CLI-created worktrees without startup agents', async () => {
    ...
    vi.mocked(createSetupRunnerScript).mockReturnValue({
      ...
      envVars: {
        OAK_ROOT_PATH: '/tmp/repo',
        OAK_WORKTREE_PATH: '/tmp/workspaces/runtime-cli-setup-split'
      }
    })
    vi.mocked(listWorktrees).mockResolvedValue([
      {
    ...
    expect(setupEnv.OAK_TAB_ID).toBe(mainEnv.OAK_TAB_ID)
    const mainLeafId = mainEnv.OAK_PANE_KEY!.slice(`${mainEnv.OAK_TAB_ID!}:`.length)
    expect(revealTerminalSession).toHaveBeenLastCalledWith(
      result.worktree.id,
      expect.objectContaining({
        ptyId: 'pty-cli-setup-setup',
        tabId: mainEnv.OAK_TAB_ID,
        activate: false,
  ...
  it('uses desktop task agent selection and bracketed-pastes startup drafts for local worktrees', async () => {
    ...
    const draftUrl = 'https://github.com/e-yc/oak/issues/123'
    const result = await runtime.createManagedWorktree({
      repoSelector: 'id:repo-1',
  ...
  it('records the resolved fallback agent when the requested startup draft agent is disabled', async () => {
    ...
    const result = await runtime.createManagedWorktree({
      ...
      startupDraft: 'https://github.com/e-yc/oak/issues/456',
      createdWithAgent: 'codex',
      activate: true
    })
  ...
  it('honors split setup placement for opted-in local startup-draft worktrees', async () => {
    ...
    vi.mocked(createSetupRunnerScript).mockReturnValue({
      ...
      envVars: {
        OAK_ROOT_PATH: '/tmp/repo',
        OAK_WORKTREE_PATH: '/tmp/workspaces/runtime-startup-setup-split'
      },
      waitForAgentStartup: true
    })
    vi.mocked(listWorktrees).mockResolvedValue([
    ...
    const result = await runtime.createManagedWorktree({
      ...
      startupDraft: 'https://github.com/e-yc/oak/issues/123',
      setupDecision: 'run',
      activate: true
    })
    ...
    const nonceMatch = startupCommand.match(/if \[ "\$seen" = ([0-9a-f-]+) \]/)
    expect(nonceMatch?.[1]).toBeTruthy()
    const markerPath = `/tmp/repo/.git/oak/setup-runner.sh.${nonceMatch![1]}.done`
    ...
    const setupEnv = (spawn.mock.calls[1]![0] as { env?: Record<string, string> }).env ?? {}
    expect(result.setup).toBeUndefined()
    expect(mainEnv.OAK_TAB_ID).toBeDefined()
    expect(mainEnv.OAK_PANE_KEY).toBeDefined()
    expect(setupEnv.OAK_TAB_ID).toBe(mainEnv.OAK_TAB_ID)
    const mainLeafId = mainEnv.OAK_PANE_KEY!.slice(`${mainEnv.OAK_TAB_ID!}:`.length)
    expect(revealTerminalSession).toHaveBeenLastCalledWith(
      result.worktree.id,
  ...
  it('passes the wrapped setup command to activation when startup spawned but setup did not', async () => {
    ...
    vi.mocked(createSetupRunnerScript).mockReturnValue({
      ...
      envVars: {
        OAK_ROOT_PATH: '/tmp/repo',
        OAK_WORKTREE_PATH: '/tmp/workspaces/runtime-startup-setup-retry'
      },
      waitForAgentStartup: true
    })
    vi.mocked(listWorktrees).mockResolvedValue([
  ...
  it('lets explicit startup draft agents override the desktop default', async () => {
    ...
    const draftUrl = 'https://github.com/e-yc/oak/issues/789'
    const result = await runtime.createManagedWorktree({
      repoSelector: 'id:repo-1',
  ...
  it('detects agents on the SSH host before launching remote startup drafts', async () => {
    ...
    const draftUrl = 'https://github.com/e-yc/oak/pull/456'
    const result = await runtime.createManagedWorktree({
      repoSelector: TEST_REPO_ID,
  ...
  it('pre-marks remote Codex workspaces trusted before pasting startup drafts', async () => {
    ...
    try {
      const result = await runtime.createManagedWorktree({
        ...
        startupDraft: 'https://github.com/e-yc/oak/issues/789'
      })
  ...
  it('normalizes runtime GitLab issue list arguments like the desktop IPC path', async () => {
    const runtime = new OakRuntimeService(store as never)
  ...
  it('routes runtime worktree creation through the selected WSL project runtime', async () => {
    ...
    const gitSpy = vi.spyOn(gitRunner, 'gitExecFileAsync').mockImplementation(async (args) => {
      ...
      if (args[0] === 'remote' && args[1] === 'get-url') {
        return { stdout: 'git@github.com:e-yc/oak.git\n', stderr: '' }
      }
      return { stdout: '', stderr: '' }
    })
    ...
    try {
      ...
      expect(gitSpy).toHaveBeenCalledWith(
        [
          'fetch',
          'pr-contributor-oak',
          '+refs/heads/contributor/runtime-wsl:refs/remotes/pr-contributor-oak/contributor/runtime-wsl'
        ],
        { cwd: TEST_REPO_PATH, wslDistro: 'Ubuntu' }
      )
      expect(gitSpy).toHaveBeenCalledWith(
        [
          'branch',
          '--set-upstream-to',
          'pr-contributor-oak/contributor/runtime-wsl',
          'runtime-wsl'
        ],
  ...
  it('skips archive hooks for CLI worktree removal by default', async () => {
    ...
    expect(result.warning).toBe(
      `oak.yaml archive hook skipped for ${TEST_WORKTREE_PATH}; pass --run-hooks to run it.`
    )
  })
  ...
  it('recovers forced Windows runtime long-path removal and keeps skipped-hook warnings', async () => {
    ...
    try {
      ...
      expect(result).toEqual({
        ...
        warning: `oak.yaml archive hook skipped for ${TEST_WORKTREE_PATH}; pass --run-hooks to run it.`
      })
  ...
  it('treats forced runtime deletion of an already-missing unregistered worktree as cleanup', async () => {
    const parentDir = await mkdtemp(join(tmpdir(), 'oak-runtime-remove-'))
  ...
  it('force-removes a legacy Oak-created runtime orphaned worktree directory after Git tracking is gone', async () => {
    const parentDir = await mkdtemp(join(tmpdir(), 'oak-runtime-orphan-'))
    ...
    const runtime = new OakRuntimeService(runtimeStoreWithRepoPath as never)
    const notifier = { worktreesChanged: vi.fn() }
    runtime.setNotifier(notifier as never)
  ...
  it('prompts then force-removes an Oak-created runtime unregistered leftover directory with no git marker', async () => {
    const parentDir = await mkdtemp(join(tmpdir(), 'oak-runtime-leftover-'))
    const repoPath = join(parentDir, 'repo')
  ...
  it('rejects an Oak-created runtime unregistered local directory with a git directory', async () => {
    const parentDir = await mkdtemp(join(tmpdir(), 'oak-runtime-standalone-'))
    const repoPath = join(parentDir, 'repo')
  ...
  it('does not inspect or delete a local path when SSH runtime orphan cleanup has no filesystem provider', async () => {
    const localPath = await mkdtemp(join(tmpdir(), 'oak-runtime-ssh-missing-fs-'))
    const repo = {
  ...
  it('still rejects forced runtime unregistered delete paths that exist on disk', async () => {
    const existingWorktreePath = await mkdtemp(join(tmpdir(), 'oak-runtime-remove-existing-'))
  ...
  async function reconcileWithToken(runtime: OakRuntimeService, token: string): Promise<void> {
    await runtime.reconcileWorktreeBaseStatus({
      repoId: TEST_REPO_ID,
      repoPath: TEST_REPO_PATH,
      worktreeId: TEST_WORKTREE_ID,
  ...
  describe('removeManagedWorktree PTY teardown (design §4.3)', () => {
    ...
    it('thunk resolves the installed provider lazily, not at construction time', async () => {
      // Simulates the daemon adapter being installed AFTER OakRuntimeService
