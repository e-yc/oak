...
function writeLegacyLinearToken(token: string | Buffer, viewer: Record<string, unknown>): void {
  const oakDir = join(tempHome, '.oak')
  mkdirSync(oakDir, { recursive: true })
  writeFileSync(join(oakDir, 'linear-token.enc'), token)
  writeFileSync(join(oakDir, 'linear-viewer.json'), JSON.stringify(viewer), {
    encoding: 'utf-8'
  })
}
...
function writeMultiWorkspaceFiles(
  ...
  const oakDir = join(tempHome, '.oak')
  mkdirSync(join(oakDir, 'linear-tokens'), { recursive: true })
  writeFileSync(
    join(oakDir, 'linear-workspaces.json'),
    JSON.stringify({
      version: 1,
...
beforeEach(() => {
  tempHome = mkdtempLike('oak-linear-client-')
  fixtures = new Map([
    [
      'token-alpha',
      {
...
describe('Linear client workspace storage', () => {
  ...
  it('migrates legacy token storage to a real workspace id when explicitly tested', async () => {
    ...
    expect(existsSync(join(tempHome, '.oak', 'linear-token.enc'))).toBe(false)
    expect(readFileSync(join(tempHome, '.oak', 'linear-workspaces.json'), 'utf-8')).toContain(
      'org-alpha'
    )
  })
  ...
  it('does not pass encrypted safeStorage bytes to the Linear SDK when encryption is unavailable', async () => {
    const tokenPath = join(tempHome, '.oak', 'linear-token.enc')
