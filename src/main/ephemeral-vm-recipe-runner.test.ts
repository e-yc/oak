...
function makeRepo(): string {
  const root = mkdtempSync(join(tmpdir(), 'oak-vm-recipe-runner-'))
  tmpRoots.push(root)
  return root
}
...
describe('runEphemeralVmRecipeStart', () => {
  it('runs a recipe from the repo root and parses its JSON result', async () => {
    ...
    writeFileSync(
      ...
      [
        ...
        'console.error(`instance:${process.env.OAK_VM_INSTANCE_ID}`)',
    ...
    if (result.ok) {
      ...
      expect(result.context.instanceId).toMatch(/^oak-/)
  ...
  it('returns a parse failure when stdout is not the recipe result contract', async () => {
    ...
    const result = await runEphemeralVmRecipeStart({
      ...
      context: { instanceId: 'oak-test-instance' }
    })
...
describe('runEphemeralVmRecipeCleanup', () => {
  it('builds a copyable cleanup payload and command', () => {
    ...
    const recipe: OakVmRecipe = {
      ...
      name: 'Cloud Sandbox',
      create: 'unused',
      destroy: './scripts/oak-vm/destroy.sh'
    }
    ...
    ).toContain('| ./scripts/oak-vm/destroy.sh')
  })
  ...
  it('passes cleanup context and recipe result on stdin', async () => {
    ...
    writeFileSync(
      ...
      [
        ...
        '    envMode: process.env.OAK_VM_MODE,',
        '    envWorkspace: process.env.OAK_WORKSPACE_NAME',
        '  }))',
        '})'
      ].join('\n')
    )
  ...
  it('passes suspend context and recipe result on stdin', async () => {
    ...
    writeFileSync(
      ...
      [
        ...
        "process.stdin.on('end', () => {",
        '  const payload = JSON.parse(input)',
        '  console.log(JSON.stringify({ mode: payload.mode, envMode: process.env.OAK_VM_MODE }))',
        '})'
      ].join('\n')
    )
