...
export function initClaudeUsagePath(): void {
  _claudeUsageFile = join(app.getPath('userData'), 'oak-claude-usage.json')
}
...
export class ClaudeUsageStore {
  ...
  private load(): ClaudeUsagePersistedState {
    try {
      ...
      // preventing Oak from booting, but we leave the file on disk for debugging.
  ...
  private buildSummary(scope: ClaudeUsageScope, range: ClaudeUsageRange): ClaudeUsageSummary {
    ...
    return {
      ...
      // data here makes the Oak-only view render empty charts instead of the
      // intended "no usage for this scope" message when only off-Oak logs exist.
  ...
  async getAutomationRunUsage(input: AutomationUsageLookupInput): Promise<AutomationRunUsage> {
    ...
    return {
      ...
      // Why: Oak terminal tab ids and Claude usage session ids are different
