...
describe('BrowserSessionRegistry', () => {
  ...
  it('has a default profile on construction', () => {
    ...
    expect(defaultProfile.partition).toBe(OAK_BROWSER_PARTITION)
  })

  it('allows the default partition', () => {
    expect(browserSessionRegistry.isAllowedPartition(OAK_BROWSER_PARTITION)).toBe(true)
  })
  ...
  it('creates an isolated profile with a unique partition', () => {
    ...
    expect(profile!.partition).toMatch(/^persist:oak-browser-session-/)
    expect(profile!.partition).not.toBe(OAK_BROWSER_PARTITION)
  ...
  it('resolves default partition for null/undefined profileId', () => {
    expect(browserSessionRegistry.resolvePartition(null)).toBe(OAK_BROWSER_PARTITION)
    expect(browserSessionRegistry.resolvePartition(undefined)).toBe(OAK_BROWSER_PARTITION)
  })

  it('resolves default partition for unknown profileId', () => {
    expect(browserSessionRegistry.resolvePartition('nonexistent')).toBe(OAK_BROWSER_PARTITION)
  })
  ...
  it('hydrates profiles from persisted data', () => {
    const fakeProfile = {
      ...
      partition: 'persist:oak-browser-session-00000000-0000-0000-0000-000000000001',
      label: 'Hydrated',
  ...
  it('routes media permission requests through macOS TCC for isolated partitions', async () => {
    ...
    // after the user grants Camera/Microphone to Oak.
