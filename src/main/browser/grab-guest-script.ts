...
// itself on teardown.
//
// Why a string builder rather than a bundled file: Oak's browser guests have
...
// It stores state on window.__oakGrab so finalize/teardown can access it.
// ---------------------------------------------------------------------------
const ARM_SCRIPT = `(function() {
  'use strict';

  // Why: always tear down any pre-existing state before arming. A malicious
  // guest page could predefine window.__oakGrab with a fake extractPayload
  // function. By tearing down unconditionally we ensure our freshly installed
  // extraction logic is the only code that runs.
  if (window.__oakGrab) {
    try {
      if (typeof window.__oakGrab.cleanup === 'function') {
        window.__oakGrab.cleanup();
      }
    } catch(e) {}
    delete window.__oakGrab;
  }
...
const AWAIT_CLICK_SCRIPT = `new Promise(function(resolve, reject) {
  ...
  function onContext(e) {
    ...
    if (!payload) return;
    grab.freezeHighlight();
    resolve({ __oakContextMenu: true, payload: payload });
  }
