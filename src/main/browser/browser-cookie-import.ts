...
function getDiagLogPath(): string {
  if (!_diagLog) {
    try {
      ...
      _diagLog = join(tmpdir(), 'oak-cookie-import-diag.log')
    }
  }
  return _diagLog
}
...
async function importCookiesFromFirefox(
  ...
  const tmpDir = mkdtempSync(join(tmpdir(), 'oak-cookie-import-'))
  ...
  try {
    ...
    if (isPermError) {
      return {
        ok: false,
        reason:
          'macOS denied access to Safari cookies. Grant Full Disk Access to Oak in System Settings → Privacy & Security → Full Disk Access.'
      }
    }
...
export async function importCookiesFromBrowser(
  ...
  try {
    ...
    // cookies prevents stale cookies from a previous Oak browsing session from
