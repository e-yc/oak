...
const HIDE_BROWSER_GRAB_OVERLAY_SCRIPT = `(function(){
  var g = window.__oakGrab;
  if (g && g.host) g.host.style.display = 'none';
  document.querySelectorAll('[data-oak-browser-annotation-overlay]').forEach(function(el) {
    el.setAttribute('data-oak-previous-display', el.style.display || '');
...
const RESTORE_BROWSER_GRAB_OVERLAY_SCRIPT = `(function(){
  ...
  document.querySelectorAll('[data-oak-browser-annotation-overlay]').forEach(function(el) {
    el.style.display = el.getAttribute('data-oak-previous-display') || '';
    el.removeAttribute('data-oak-previous-display');
  });
})()`
