...
export function setupGuestContextMenu(args: {
  ...
  const handler = (_event: Electron.Event, params: Electron.ContextMenuParams): void => {
    ...
    // would otherwise expose the bearer token outside Oak.
...
export function setupGrabShortcutForwarding(args: {
  ...
  const handler = (event: Electron.Event, input: Electron.Input): void => {
    ...
    if (
      ...
      // While grab mode is actively awaiting a pick, plain C/S belong to Oak's
...
export function setupGuestShortcutForwarding(args: {
  ...
  const forwardShortcutInput = (
    ...
    if (action?.type === 'zoom') {
      // Why: keyboard zoom is Oak chrome zoom. Focused guests bypass the
    ...
    if (keybindingMatchesAction('tab.newBrowser', input, process.platform, keybindings)) {
      ...
      // (bypass cache). The guest would handle it natively, but Oak's webview
      ...
      // the guest frame, which is invisible behind Oak's chrome.
