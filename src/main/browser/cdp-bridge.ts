...
export class CdpBridge {
  ...
  getPageInfo(
    ...
    // Why: OakRuntimeService pushes navigation/title updates after commands
  ...
  private getActiveGuest(): Electron.WebContents {
    ...
    if (tabs.length === 0) {
      throw new BrowserError(
        'browser_no_tab',
        'No browser tab is open. Use the Oak UI to open a browser tab first.'
      )
    }
    if (tabs.length === 1) {
      ...
      throw new BrowserError(
        'browser_no_tab',
        "Multiple browser tabs are open. Run 'oak tab list' and 'oak tab switch --index <n>' to select one."
      )
    }
    ...
    if (!guest || guest.isDestroyed()) {
      ...
      throw new BrowserError(
        ...
        "The active browser tab was closed. Run 'oak tab list' to find remaining tabs."
      )
    }
    return guest
  }
  ...
  private async resolveRef(
    ...
    if (!state.snapshotResult) {
      throw new BrowserError(
        'browser_stale_ref',
        "No snapshot exists for this tab. Run 'oak snapshot' first."
      )
    }
    ...
    if (!entry) {
      throw new BrowserError(
        ...
        `Element ref ${ref} was not found. Run 'oak snapshot' to see available refs.`
      )
    }
    ...
    if (!entry.sessionId) {
      ...
      if (state.navigationId && currentNavId !== state.navigationId) {
        ...
        throw new BrowserError(
          'browser_stale_ref',
          "The page has navigated since the last snapshot. Run 'oak snapshot' to get fresh refs."
        )
      }
    }
    ...
    try {
      ...
      throw new BrowserError(
        'browser_stale_ref',
        `Element ${ref} no longer exists in the DOM. Run 'oak snapshot' to get fresh refs.`
      )
    }
  }
