...
const DEFAULT_COMPUTER_USE_BUNDLE_ID = 'com.stablyai.oak.computer-use'
...
async function openComputerUsePermissionsAsync(
  ...
  if (!helperAppPath) {
    throw new RuntimeClientError('accessibility_error', 'Oak Computer Use.app was not found')
  }
...
function closeExistingPermissionHelpers(): void {
  ...
  const setupHelperPatterns = [
    'oak-computer-use-macos[[:space:]]+--permission([[:space:]]|$)',
    'oak-computer-use-macos[[:space:]]+--permissions([[:space:]]|$)'
  ]
...
function nextPermissionStep(
  ...
  return `Grant ${missing.id === 'accessibility' ? 'Accessibility' : 'Screen Recording'} to Oak Computer Use, then retry get-app-state.`
}
