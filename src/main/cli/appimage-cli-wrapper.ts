const APPIMAGE_CLI_SCRIPT = [
  ...
  'if(!appDir){console.error("Oak AppImage runtime did not set APPDIR.");process.exit(1);}',
...
if [ ! -f "$APPIMAGE" ]; then
  echo "Oak AppImage not found at $APPIMAGE" >&2
  echo "If you moved the AppImage, re-run CLI registration from Oak Settings." >&2
  exit 1
fi
