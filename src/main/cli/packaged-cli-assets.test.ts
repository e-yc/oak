...
const linuxLauncherAsset = new URL('../../../resources/linux/bin/oak-ide', import.meta.url)
...
describe('packaged CLI assets', () => {
  ...
  itRunsUnixShell(
    ...
    async () => {
      const root = await mkdtemp(join(tmpdir(), 'oak-linux-cli-'))
      try {
        const appDir = join(root, 'Oak')
        const resourcesDir = join(appDir, 'resources')
        const launcherDir = join(resourcesDir, 'bin')
        const cliDir = join(resourcesDir, 'app.asar.unpacked', 'out', 'cli')
        const launcherPath = join(launcherDir, 'oak-ide')
        const electronPath = join(appDir, 'oak-ide')
...
`,
        ...
        const commandPath = join(commandDir, 'oak-ide')
        await mkdir(commandDir, { recursive: true })
        await mkdir(join(homeDir, 'oak'), { recursive: true })
  ...
  itRunsUnixShell('runs the AppImage CLI wrapper through APPDIR at runtime', async () => {
    const root = await mkdtemp(join(tmpdir(), 'oak-appimage-cli-'))
    try {
      const appDir = join(root, 'Oak.AppDir')
      const cliDir = join(appDir, 'resources', 'app.asar.unpacked', 'out', 'cli')
      const cliPath = join(cliDir, 'index.js')
      const appImagePath = join(root, "Oak's AppImage.AppImage")
      const commandPath = join(root, 'oak-ide')
      await mkdir(cliDir, { recursive: true })
      await writeFile(
        cliPath,
        `exports.main = (argv) => {
  console.log(JSON.stringify({
    ...
    oakNodeOptions: process.env.OAK_NODE_OPTIONS ?? null,
    nodeReplExternalModule: process.env.NODE_REPL_EXTERNAL_MODULE ?? null,
    oakNodeReplExternalModule: process.env.OAK_NODE_REPL_EXTERNAL_MODULE ?? null
  }))
}
`,
        'utf8'
...
`,
      ...
      const payload = JSON.parse(result.stdout) as {
        ...
        oakNodeOptions: string | null
        nodeReplExternalModule: string | null
        oakNodeReplExternalModule: string | null
      }
      ...
      expect(payload.oakNodeOptions).toBe('--trace-warnings')
      expect(payload.nodeReplExternalModule).toBeNull()
      expect(payload.oakNodeReplExternalModule).toBe('external-loader')
    } finally {
      await rm(root, { recursive: true, force: true })
    }
  })
