...
export const OAK_PI_PREFILL_EXTENSION_FILE = 'oak-prefill.ts'

// Why: prefill-without-submit needs an env-var the bundled `oak-prefill.ts`
 ...
 *  `OAK_OMP_PREFILL_ENV_VAR` instead. */
export const OAK_PI_PREFILL_ENV_VAR = PREFILL_ENV_VAR_BY_KIND.pi

/** OMP's prefill env var. Mirrors `OAK_PI_PREFILL_ENV_VAR` for OMP launches
 *  so renderer plans and shell-ready restore lines can stay agent-scoped. */
export const OAK_OMP_PREFILL_ENV_VAR = PREFILL_ENV_VAR_BY_KIND.omp
...
export function getPiPrefillExtensionSource(kind: PiAgentKind): string {
  ...
  return [
    ...
    '    if (!process.env.OAK_PANE_KEY) return',
