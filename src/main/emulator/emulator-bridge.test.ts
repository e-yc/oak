...
import { RuntimeEmulatorCommands } from '../runtime/oak-runtime-emulator'
...
describe('EmulatorBridge helper ownership', () => {
  ...
  it('stops the previous Oak-managed helper when a worktree switches devices', async () => {
  ...
  it('shuts down the previous Oak-managed device when requested', async () => {
    const bridge = new EmulatorBridge()
    bridge.registerActiveEmulator('wt-1', session('device-old'), { managed: true })
  ...
  it('only kills Oak-managed helpers during app shutdown cleanup', async () => {
    const bridge = new EmulatorBridge()
