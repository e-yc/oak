...
const WRITE_KEY: string | null =
  typeof OAK_POSTHOG_WRITE_KEY !== 'undefined'
    ? OAK_POSTHOG_WRITE_KEY
    : ((globalThis as { OAK_POSTHOG_WRITE_KEY?: string | null }).OAK_POSTHOG_WRITE_KEY ?? null)
 ...
 * OAK_TELEMETRY_DISABLED). The SDK flag does not persist across process
...
// official stable/rc builds where CI injects `OAK_BUILD_IDENTITY` and
// `OAK_POSTHOG_WRITE_KEY`.
