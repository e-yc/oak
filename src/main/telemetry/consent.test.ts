...
describe('resolveConsent', () => {
  ...
  it('returns oak_disabled when OAK_TELEMETRY_DISABLED=1', () => {
    process.env.OAK_TELEMETRY_DISABLED = '1'
    expect(
      resolveConsent(
        settingsWithTelemetry({
          optedIn: true,
  ...
  it('prefers do_not_track over oak_disabled when both are set', () => {
    process.env.DO_NOT_TRACK = '1'
    process.env.OAK_TELEMETRY_DISABLED = '1'
    expect(
      resolveConsent(
  ...
  it('env-var override wins over a stored pending_banner state', () => {
    ...
    ).toEqual({ effective: 'disabled', reason: 'oak_disabled' })
  })
})
