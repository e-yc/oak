...
function buildAndApplyMenu(options: RegisterAppMenuOptions): void {
  ...
  const featureTourItem: Electron.MenuItemConstructorOptions = {
    label: translateMain('menu.exploreOak', 'Explore Oak'),
    click: (_menuItem, window) => onOpenFeatureTour(window)
  }

  const setupGuideItem: Electron.MenuItemConstructorOptions = {
    label: translateMain('menu.gettingStarted', 'Getting Started with Oak'),
  ...
  // redundant "Oak" entry with roles that don't apply, so we omit it there
    ...
    submenu: [
      ...
      { role: 'copy' },
      {
        ...
        click: () => {
          ...
          // control, so raw Electron paste cannot know which Oak surface owns it.
  ...
  const appearanceSubmenu: Electron.MenuItemConstructorOptions = {
    ...
    submenu: [
      ...
      {
        label: translateMain('menu.showMobileButton', 'Show Oak Mobile Button'),
        type: 'checkbox',
