...
describe('createMainWindow', () => {
  ...
  it('enables renderer sandboxing and opens external links safely', () => {
    ...
    const allowBlankPrefs = { partition: 'persist:oak-browser' }
    ...
    windowHandlers['will-attach-webview'](
      ...
      { partition: 'persist:oak-browser' } as never,
  ...
  it('notifies before Oak-first captures a risky terminal-focused shortcut', () => {
    const windowHandlers: Record<string, (...args: any[]) => void> = {}
    const webContents = {
      on: vi.fn((event, handler) => {
        windowHandlers[event] = handler
    ...
    createMainWindow({
      getUI: () => ({}),
      getSettings: () => ({ terminalShortcutPolicy: 'oak-first' })
    } as never)
  ...
  it('notifies before Oak-first captures a terminal-focused double-tap shortcut', () => {
    const windowHandlers: Record<string, (...args: any[]) => void> = {}
    const webContents = {
      on: vi.fn((event, handler) => {
        windowHandlers[event] = handler
  ...
  it('keeps the headless E2E window hidden when the Windows fallback fires', () => {
    vi.useFakeTimers()
    const previousHeadless = process.env.OAK_E2E_HEADLESS
    process.env.OAK_E2E_HEADLESS = '1'
    const { browserWindowInstance } = createStartupRevealWindowFixture()
    ...
    try {
      ...
      if (previousHeadless === undefined) {
        delete process.env.OAK_E2E_HEADLESS
      } else {
        process.env.OAK_E2E_HEADLESS = previousHeadless
      }
    }
  })
