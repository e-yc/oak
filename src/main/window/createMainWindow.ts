...
import { OAK_BROWSER_GUEST_WEB_PREFERENCES } from '../../shared/browser-guest-web-preferences'
...
type CreateMainWindowOptions = {
  ...
  /** Returns true when Oak should reload after an unexpected renderer loss.
...
export function createMainWindow(
  ...
  const mainWindow = new BrowserWindow({
    ...
    title: opts?.title ?? 'Oak',
    show: false,
    // Why: macOS swallows the app-activating click by default, so clicking
    // back into Oak (e.g. the floating workspace) needed a second click.
  ...
  if (process.platform === 'darwin') {
    ...
    // visibility transitions hardens Oak against black-surface failures during
  ...
  mainWindow.webContents.on('will-attach-webview', (event, webPreferences, params) => {
    ...
    // The one allowed data URL is Oak's inert blank-tab bootstrap page; deny
    ...
    // persist:oak-browser-session-<uuid>). The registry is the sole authority
    ...
    Object.assign(webPreferences, OAK_BROWSER_GUEST_WEB_PREFERENCES)
  ...
  mainWindow.webContents.on('did-attach-webview', (_event, guest) => {
    ...
    // a race where target=_blank or early redirects can bypass Oak's intended
    // fallback behavior.
  ...
  const dispatchResolvedWindowShortcutAction = (
    ...
    const capturedTerminalActionId =
      ...
      focusedShortcutContext.terminalShortcutPolicy === 'oak-first' &&
  ...
  mainWindow.webContents.on('before-input-event', (event, input) => {
    ...
    if (isMacAppPasteInput(input)) {
      ...
      // editable controls, so route Cmd+V through Oak's paste ownership first.
      event.preventDefault()
  ...
  const hideToTrayIfEnabled = (): boolean => {
    ...
    if (store.getUI().trayMinimizeNoticeShown !== true) {
      try {
        new Notification({
          title: 'Oak',
          body: translateMain('tray.minimizeNotice.body', 'Oak is still running in the system tray')
        }).show()
      } catch {
  ...
  mainWindow.on('closed', () => {
    ...
    // here or the quit path can crash before Squirrel.Mac relaunches Oak.
