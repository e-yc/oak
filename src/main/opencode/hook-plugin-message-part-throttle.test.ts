 ...
 * coalesced and capped before POSTing to Oak's agent-hook server. The
 * un-throttled plugin re-posted the full accumulated reply per streamed
 * append — O(n²) bytes per turn — which saturated Oak's main + renderer
...
const ENV_KEYS = ['OAK_PANE_KEY', 'OAK_AGENT_HOOK_PORT', 'OAK_AGENT_HOOK_TOKEN'] as const
...
describe('OpenCode plugin MessagePart throttling', () => {
  ...
  beforeEach(() => {
    tempDir = mkdtempSync(join(tmpdir(), 'oak-opencode-plugin-test-'))
    posts = []
    savedEnv = {}
    for (const key of ENV_KEYS) {
      savedEnv[key] = process.env[key]
    }
    process.env.OAK_PANE_KEY = 'tab-1:leaf-1'
    process.env.OAK_AGENT_HOOK_PORT = '45678'
    process.env.OAK_AGENT_HOOK_TOKEN = 'test-token'
  ...
  async function loadPluginEventHandler(): Promise<PluginEventHandler> {
    const pluginPath = join(tempDir, 'oak-opencode-status.mjs')
    writeFileSync(pluginPath, _internals.getOpenCodePluginSource())
    const module = (await import(pathToFileURL(pluginPath).href)) as {
      OakOpenCodeStatusPlugin: (ctx: unknown) => Promise<{ event: PluginEventHandler }>
    }
    const client = {
      session: {
    ...
    const hooks = await module.OakOpenCodeStatusPlugin({ client })
    return hooks.event
  }
