...
describe('OpenCode hook plugin source', () => {
  ...
  it('still accepts an optional opaque plugin context instead of destructuring', () => {
    ...
    expect(source).toContain('export const OakOpenCodeStatusPlugin = async (_ctx) => {')
  ...
  it('resolves hook coords from the endpoint file before falling back to process.env', () => {
    // Why: a long-running OpenCode session was fork()ed with the prior Oak's
    // PORT/TOKEN frozen into process.env. The plugin must prefer the on-disk
    // endpoint file (rewritten on every Oak start()) over env, otherwise it
    // keeps posting to a dead port after an Oak restart.
    const source = _internals.getOpenCodePluginSource()

    expect(source).toContain('function readEndpointFile()')
    expect(source).toContain('process.env.OAK_AGENT_HOOK_ENDPOINT')
    ...
    expect(source).toContain('port: fileEnv.OAK_AGENT_HOOK_PORT || process.env.OAK_AGENT_HOOK_PORT')
    expect(source).toContain(
      'token: fileEnv.OAK_AGENT_HOOK_TOKEN || process.env.OAK_AGENT_HOOK_TOKEN'
    )
    ...
    expect(source).toContain('"X-Oak-Agent-Hook-Token": coords.token')
  })
...
describe('OpenCodeHookService buildPtyEnv / clearPty round-trip', () => {
  ...
  beforeAll(() => {
    userDataDir = mkdtempSync(join(tmpdir(), 'oak-opencode-hooks-'))
  ...
  it('writes a shared OPENCODE_CONFIG_DIR and installs the plugin file', () => {
    ...
    const pluginPath = join(env.OPENCODE_CONFIG_DIR!, 'plugins', 'oak-opencode-status.js')
    expect(existsSync(pluginPath)).toBe(true)
    // Sanity-check the file has plugin source, not a stray write.
    const pluginSource = readFileSync(pluginPath, 'utf8')
    expect(pluginSource).toContain('OakOpenCodeStatusPlugin')
  ...
  it('buildPtyEnv preserves a user-set OPENCODE_CONFIG_DIR when the id is unusable', () => {
    ...
    const service = new OpenCodeHookService()
    const userDir = mkdtempSync(join(tmpdir(), 'oak-opencode-userdir-'))
    try {
  ...
  it('works end-to-end for a plain UUID id (non-daemon path)', () => {
    ...
    expect(env.OPENCODE_CONFIG_DIR).toBe(join(userDataDir, 'opencode-hooks', 'shared'))
    expect(existsSync(join(env.OPENCODE_CONFIG_DIR!, 'plugins', 'oak-opencode-status.js'))).toBe(
      true
    )
...
describe('OpenCodeHookService overlay mode (user OPENCODE_CONFIG_DIR set)', () => {
  ...
  // Oak must mirror it into a source-scoped overlay rather than `delete` its
  // own injection or overwrite the user's value. The user's auth/models/keymap
  // and Oak's status plugin both load via a single OPENCODE_CONFIG_DIR.
  const ptyId = 'overlay-pty-1'
  let userDataDir: string
  let userConfigDir: string

  beforeAll(() => {
    userDataDir = mkdtempSync(join(tmpdir(), 'oak-opencode-overlay-userdata-'))
    getPathMock.mockImplementation((name: string) => {
      if (name === 'userData') {
        return userDataDir
      }
  ...
  beforeEach(() => {
    userConfigDir = mkdtempSync(join(tmpdir(), 'oak-opencode-overlay-userconfig-'))
  ...
  it('builds an overlay under userData and exposes user config + Oak plugin together', () => {
    const service = new OpenCodeHookService()
    ...
    // Oak's status plugin is a sibling, not a replacement.
    const oakPluginPath = join(env.OPENCODE_CONFIG_DIR!, 'plugins', 'oak-opencode-status.js')
    expect(existsSync(oakPluginPath)).toBe(true)
    expect(readFileSync(oakPluginPath, 'utf8')).toContain('OakOpenCodeStatusPlugin')
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    () => {
      // Why: only the plugins/ subtree needs entry-by-entry mirroring so Oak
      ...
      // plugins/ must be a real directory in the overlay so Oak can write
  ...
  it("does not overwrite a user plugin file with the same filename as Oak's plugin", () => {
    // Why: the failure mode this guards against — a user-owned plugin file
    // happens to be named oak-opencode-status.js. Without the per-entry
    // skip in mirrorUserConfig, the file would be linked into the overlay
    // and Oak's writeFileSync would write through the symlink, destroying
    // the user's content on their real filesystem.
    const userOakSentinel = 'USER OWNED OAK-NAMED PLUGIN — DO NOT CLOBBER'
    writeFileSync(join(userConfigDir, 'plugins', 'oak-opencode-status.js'), userOakSentinel)
    ...
    // Overlay copy is Oak's real plugin source, not the user's file.
    const overlayPlugin = readFileSync(
      join(env.OPENCODE_CONFIG_DIR!, 'plugins', 'oak-opencode-status.js'),
      'utf8'
    )
    expect(overlayPlugin).toContain('OakOpenCodeStatusPlugin')
    expect(overlayPlugin).not.toBe(userOakSentinel)
    expectUserConfigIntact()
  })
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    () => {
      // Why: if plugins/ is a symlink (common dotfiles pattern), writing Oak's
      ...
      const realPluginsDir = mkdtempSync(join(tmpdir(), 'oak-real-plugins-'))
      try {
        ...
        // The user's real filesystem must NOT receive Oak's status plugin.
        expect(existsSync(join(realPluginsDir, 'oak-opencode-status.js'))).toBe(false)
        ...
        // Oak's status plugin lands in the overlay only.
        expect(
          existsSync(join(env.OPENCODE_CONFIG_DIR!, 'plugins', 'oak-opencode-status.js'))
        ).toBe(true)
  ...
  it("preserves the user's OPENCODE_CONFIG_DIR when the path does not exist", () => {
    // Why: typoed user path — overriding it with an Oak-owned dir would let
    // Oak's status plugin "succeed" while silently hiding the user's typo.
    // The design rejects that: leave the user's value alone and let OpenCode
    // surface the typo on its own.
    const service = new OpenCodeHookService()
    const missingPath = join(tmpdir(), `oak-opencode-nope-${Date.now()}`)
  ...
  it("preserves the user's OPENCODE_CONFIG_DIR when the mirror step fails", async () => {
    ...
    // the user's plugins/auth/models keep loading; only Oak's status plugin
    // is forfeited.
  ...
  it('rebuilding the overlay for the same ptyId does not corrupt the user dir', () => {
    ...
    expect(
      readFileSync(join(env.OPENCODE_CONFIG_DIR!, 'plugins', 'oak-opencode-status.js'), 'utf8')
    ).toContain('OakOpenCodeStatusPlugin')
    expectUserConfigIntact()
  })
