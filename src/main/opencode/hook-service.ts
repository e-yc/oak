...
const OAK_OPENCODE_PLUGIN_FILE = 'oak-opencode-status.js'
...
const OPENCODE_OVERLAY_MANIFEST_FILE = '.oak-opencode-overlay-manifest.json'
...
export function getOpenCodeFamilyPluginSource(hookPathname: string): string {
  ...
  // same OAK_PANE_KEY / OAK_TAB_ID / OAK_WORKTREE_ID / OAK_AGENT_HOOK_*
  // env vars that Oak injects into every PTY, so OpenCode panes flow into
  ...
  return [
    ...
    "// OpenCode's Node process (not Oak's) and has no access to server.ts's",
    ...
    '// which reads the endpoint file. The file only changes on Oak restart',
    ...
    '  const path = process.env.OAK_AGENT_HOOK_ENDPOINT;',
    '  if (!path) return null;',
    '  try {',
    '    const fs = require("fs");',
    '    try {',
    ...
    '      // writeEndpointFile on the Oak side) allocates a fresh inode on',
    ...
    '        // digits in the key for forward-compat with future OAK_AGENT_HOOK_*',
    ...
    '      console.warn("[oak-hook] failed to parse endpoint file:", err.message);',
    '    }',
    '    return null;',
    '  }',
    '}',
    ...
    '  // frozen when OpenCode was fork()ed — stale after an Oak restart. The',
    '  // file is rewritten on every Oak start(), so sourcing it per post lets',
    '  // a long-running OpenCode session reach the current server. Falls back',
    '  // to process.env when the file is absent (first-run / pre-endpoint-file / Oak',
    '  // never started writing the file).',
    '  const fileEnv = readEndpointFile() || {};',
    '  return {',
    '    port: fileEnv.OAK_AGENT_HOOK_PORT || process.env.OAK_AGENT_HOOK_PORT,',
    '    token: fileEnv.OAK_AGENT_HOOK_TOKEN || process.env.OAK_AGENT_HOOK_TOKEN,',
    '    env: fileEnv.OAK_AGENT_HOOK_ENV || process.env.OAK_AGENT_HOOK_ENV || "",',
    '    version: fileEnv.OAK_AGENT_HOOK_VERSION || process.env.OAK_AGENT_HOOK_VERSION || "",',
    '  };',
    '}',
    '',
    ...
    '// per turn through Oak (loopback HTTP -> main JSON parse -> status compare',
    ...
    '// flip the root Oak pane to done or overwrite the parent turn preview.',
    ...
    '  // rewritten by a newer Oak since the last call. Pane/tab/worktree IDs',
    '  // stay on process.env because they are per-PTY (stable for the life of',
    '  // the OpenCode process), not per-Oak-instance.',
    ...
    '    paneKey,',
    '    launchToken: process.env.OAK_AGENT_LAUNCH_TOKEN || "",',
    '    tabId: process.env.OAK_TAB_ID || "",',
    '    worktreeId: process.env.OAK_WORKTREE_ID || "",',
    '    env: coords.env,',
    ...
    '        "X-Oak-Agent-Hook-Token": coords.token,',
    '      },',
    '      body,',
    '    });',
    '  } catch {',
    '    // Why: OpenCode session events must never fail the agent run just',
    '    // because Oak is unavailable or the local loopback request failed.',
    '  }',
    '}',
    '',
    ...
    'export const OakOpenCodeStatusPlugin = async (_ctx) => {',
...
export class OpenCodeHookService {
  ...
  buildPtyEnv(ptyId: string, existingConfigDir?: string | undefined): Record<string, string> {
    if (!isUsableId(ptyId)) {
      ...
      // user's own config — only the Oak status plugin is forfeited.
    ...
    // Oak-owned dir is the exact config-replacement failure mode documented in
  ...
  private clearManifestEntries(overlayDir: string, manifest: OpenCodeOverlayManifest): void {
    ...
    for (const entryName of manifest.pluginEntries) {
      if (entryName === OAK_OPENCODE_PLUGIN_FILE) {
        continue
      }
  ...
  // so Oak can drop a sibling file alongside the user's plugins; everything
  ...
  private mirrorUserConfig(sourceDir: string, overlayDir: string): void {
    ...
    // Oak previously mirrored, so deleted/replaced user config cannot stay
      ...
      if (entry.name === 'plugins') {
        ...
        if ((!isSymlink && entry.isDirectory()) || isLinkPointingToDir) {
          ...
          for (const pluginEntry of readdirSync(resolvedSource, { withFileTypes: true })) {
            // Why: skip a user file with the same filename as Oak's plugin —
            ...
            // semantics: Oak's status plugin runs and the user's same-named
            // plugin is shadowed for this PTY only — their source file on disk
            // is untouched.
            if (pluginEntry.name === OAK_OPENCODE_PLUGIN_FILE) {
              continue
            }
            mirrorEntry(
  ...
  // Why: write Oak's status plugin into the overlay's plugins/ dir. The
  ...
  private writePluginIntoOverlay(overlayDir: string): void {
    ...
    const pluginPath = join(pluginsDir, OAK_OPENCODE_PLUGIN_FILE)
    try {
  ...
  private writeSharedPluginConfig(): string | null {
    ...
    try {
      mkdirSync(pluginsDir, { recursive: true })
      writeFileSync(join(pluginsDir, OAK_OPENCODE_PLUGIN_FILE), getOpenCodePluginSource())
    } catch {
