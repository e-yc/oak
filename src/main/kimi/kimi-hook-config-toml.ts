...
// so Oak manages only its own marker-delimited block: install rewrites the
...
// Why: mirror the Claude-compatible events Oak normalizes for status. Kimi uses
...
const BLOCK_START = '# >>> oak-managed-kimi-hooks (managed by Oak; do not edit) >>>'
const BLOCK_END = '# <<< oak-managed-kimi-hooks <<<'
...
// Oak generates; escape both defensively anyway.
...
// Oak-managed script (by filename, so a moved userData path is still swept).
