...
const COMMAND =
  "if [ -x '/home/u/.oak/agent-hooks/kimi-hook.sh' ]; then /bin/sh '/home/u/.oak/agent-hooks/kimi-hook.sh'; fi"
...
describe('kimi managed hooks TOML block', () => {
  ...
  it('is idempotent — reinstalling does not duplicate the block', () => {
    ...
    const markerCount = (twice.match(/oak-managed-kimi-hooks \(/g) ?? []).length
