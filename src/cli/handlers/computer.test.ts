...
describe('oak computer observation CLI handlers', () => {
  beforeEach(() => {
    vi.restoreAllMocks()
    callMock.mockReset()
  ...
  it('prints targeted permission setup command help', async () => {
    ...
    const output = vi.mocked(console.log).mock.calls[0][0]
    expect(output).toContain('oak computer permissions [--id <accessibility|screenshots>] [--json]')
  ...
  it('prints provider capabilities without resolving a worktree', async () => {
    queueFixtures(
      ...
      okFixture('req_capabilities', {
        platform: 'darwin',
        provider: 'oak-computer-use-macos',
    ...
    expect(vi.mocked(console.log).mock.calls[0][0]).toContain('oak-computer-use-macos')
  })
  ...
  it('opens computer permission setup without resolving a worktree', async () => {
    ...
    const output = vi.mocked(console.log).mock.calls[0][0]
    expect(output).toContain('Opened Oak Computer Use permission setup')
    expect(output).toContain('/Applications/Oak Computer Use.app')
  })
