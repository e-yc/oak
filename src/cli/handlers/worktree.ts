...
function getEnvParentWorkspace(): string | undefined {
  const workspaceId = process.env.OAK_WORKSPACE_ID
  if (typeof workspaceId === 'string' && isWorkspaceKey(workspaceId)) {
    return workspaceId
  }
  const worktreeId = process.env.OAK_WORKTREE_ID
...
async function getCreateRepoSelector(
  ...
  throw new RuntimeClientError(
    'invalid_argument',
    'Missing repo selector. Pass --repo or run from inside an Oak-managed worktree.'
  )
}
...
export const WORKTREE_HANDLERS: Record<string, CommandHandler> = {
  ...
  'worktree create': async ({ flags, client, cwd, json }) => {
    ...
    if (
      ...
      try {
        // Why: agent shells can lose OAK_TERMINAL_HANDLE while still running
        // inside an Oak worktree. Cwd keeps CLI-created children nestable and
