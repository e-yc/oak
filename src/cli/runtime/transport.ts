  ...
  return await new Promise((resolve, reject) => {
    ...
    if (!transport) {
      reject(
        new RuntimeClientError(
          'runtime_unavailable',
          'No compatible transport found in Oak runtime metadata.'
        )
      )
      return
    }
    ...
    const timeout = setTimeout(() => {
      ...
      reject(
        new RuntimeClientError(
          'runtime_timeout',
          'Timed out waiting for the Oak runtime to respond.'
        )
      )
    }, timeoutMs)
    ...
    socket.once('error', () => {
      finish({
        ...
        error: new RuntimeClientError(
          'runtime_unavailable',
          'Could not connect to the running Oak app. Restart Oak and try again.'
        )
      })
    })
    ...
    socket.once('close', () => {
      finish({
        ...
        error: new RuntimeClientError(
          'runtime_unavailable',
          'The Oak runtime closed the connection before responding. Restart Oak and try again.'
        )
      })
    })
    socket.on('data', (chunk) => {
      ...
      while (newlineIndex !== -1 && !settled) {
        ...
        try {
          ...
          finish({
            ...
            error: new RuntimeClientError(
              ...
              'The Oak runtime returned an invalid response frame.'
            )
          })
          return
        }
        ...
        // setTimeout().refresh() is stable since Node 10 (Oak ships on
        ...
        // the decode boundary so version skew between the CLI and the Oak
        ...
        if (response.id !== requestId) {
          finish({
            ...
            error: new RuntimeClientError(
              'invalid_runtime_response',
              'The Oak runtime returned a mismatched response id.'
            )
          })
          return
        }
        if (response._meta?.runtimeId && response._meta.runtimeId !== metadata.runtimeId) {
          finish({
            ...
            error: new RuntimeClientError(
              'runtime_unavailable',
              'The Oak runtime changed while the request was in flight. Retry the command.'
            )
          })
          return
        }
