...
describe('buildSetupRunnerCommand', () => {
  it('uses bash for WSL UNC runner scripts regardless of host casing', () => {
    ...
    ).toBe('bash /home/jin/repo/.git/worktrees/feature/oak/setup-runner.sh')
  })
  ...
  it('keeps generic forward-slash UNC runner scripts on cmd.exe', () => {
    expect(
      buildSetupRunnerCommand('//server/share/repo/.git/oak/setup-runner.cmd', 'windows')
    ).toBe('cmd.exe /c "//server/share/repo/.git/oak/setup-runner.cmd"')
  })
})
...
describe('getSetupRunnerCommandPlatformForPath', () => {
  it('prefers POSIX for absolute POSIX runner paths even from Windows clients', () => {
    expect(
      getSetupRunnerCommandPlatformForPath('/remote/repo/.git/oak/setup-runner.sh', 'windows')
    ).toBe('posix')
  })

  it('prefers Windows for native Windows runner paths even from POSIX clients', () => {
    expect(
      getSetupRunnerCommandPlatformForPath('C:\\repo\\.git\\oak\\setup-runner.cmd', 'posix')
    ).toBe('windows')
  })
  ...
  it('keeps WSL UNC paths on the Windows resolver so they can be converted', () => {
    expect(
      getSetupRunnerCommandPlatformForPath(
        '\\\\wsl.localhost\\Ubuntu\\home\\jin\\repo\\.git\\oak\\setup-runner.sh',
        'posix'
      )
    ).toBe('windows')
  })
  ...
  it('keeps forward-slash UNC paths on the Windows resolver', () => {
    ...
    expect(
      getSetupRunnerCommandPlatformForPath('//server/share/repo/.git/oak/setup-runner.cmd', 'posix')
    ).toBe('windows')
  })
})
