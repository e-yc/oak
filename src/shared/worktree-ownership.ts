...
export function buildKnownOakWorkspaceLayouts(
  ...
  const layouts: OakWorkspaceLayout[] = []
...
function appendWorkspaceLayouts(
  target: OakWorkspaceLayout[],
  source: readonly OakWorkspaceLayout[]
): void {
...
function buildWslWorkspaceLayouts(
  ...
  const root = `//wsl.localhost/${parsed.distro}${linuxHome}/oak/workspaces`
...
export function classifyWorktreeOwnership(args: {
  ...
  meta?: WorktreeMeta
  settings: Pick<GlobalSettings, 'workspaceDir' | 'nestWorkspaces' | 'workspaceDirHistory'>
  knownOakLayouts: OakWorkspaceLayout[]
}): WorktreeOwnership {
  if (hasStrongOakMetadata(args.meta)) {
    return 'oak-managed'
  }

  if (isUnderFlatOrUntrustedOakRoot(args.worktree.path, args.knownOakLayouts)) {
    return 'unknown-legacy'
  }

  if (canClassifyAsExternal(args.worktree.path, args.knownOakLayouts)) {
    // Why: a plain `git worktree add` can target Oak's nested workspace
    // folder. Only metadata proves Oak created it.
    return 'external'
  }
...
function hasStrongOakMetadata(meta: WorktreeMeta | undefined): boolean {
  return Boolean(
    meta?.oakCreatedAt ||
    meta?.oakCreationWorkspaceLayout ||
    meta?.createdAt ||
...
function isUnderFlatOrUntrustedOakRoot(
  worktreePath: string,
  knownOakLayouts: OakWorkspaceLayout[]
): boolean {
  for (const layout of knownOakLayouts) {
