...
const EphemeralVmRecipeOakServerConnectionSchema = z
  .object({
    type: z.literal('oak-server'),
...
export const EphemeralVmRecipeConnectionSchema = z.discriminatedUnion('type', [
  EphemeralVmRecipeOakServerConnectionSchema,
...
export function parseEphemeralVmRecipeResult(stdout: string): EphemeralVmRecipeResultParseResult {
  ...
  if (connection.type === 'oak-server' && !parsePairingCode(connection.pairingCode)) {
    return { ok: false, error: 'Recipe result pairingCode is not a valid Oak pairing code.' }
  }
...
export function getEphemeralVmRecipeResultPairingCode(
  ...
  return connection.type === 'oak-server' ? connection.pairingCode : null
}
