...
// the relay sends to Oak. Consumed by `src/relay/agent-hook-server.ts` (which
...
// between Oak's main process and the remote relay.
//
// Per the design doc:
// - The relay normalizes; Oak routes. The envelope's `payload` field has
//   already been through `normalizeHookPayload` (which calls
//   `parseAgentStatusPayload` → `normalizeAgentStatusObject`) on the relay
//   side. Oak's `ingestRemote` re-runs the canonical payload normalizer at
...
// - The wire `connectionId` is **always `null`**: a `connectionId` is Oak's
//   local handle on an `ssh2` connection, not a wire identity. Oak stamps the
...
//   POST body so Oak's warn-once protocol diagnostics still fire. The relay
...
// with the same value so Oak can attribute the event back to the right CLI.
...
/** Wire envelope for a single hook event flowing relay → Oak. */
export type AgentHookRelayEnvelope = {
  source: AgentHookSource
  paneKey: string
  /** Ephemeral Oak launch identity stamped into the PTY env for this process. */
  launchToken?: string
  tabId?: string
  worktreeId?: string
  /** Always `null` on the wire — relay does not know Oak's local connectionId. */
  connectionId: null
  /** Preserved from the relay-side normalized hook event so Oak can
  ...
  /** True when the relay is replaying its cache after Oak reconnects. */
  isReplay?: boolean
  ...
  env?: string
  /** Forwarded verbatim from the agent CLI POST body. Lets Oak's warn-once
   ...
   *  Oak's `ingestRemote` validates it again at the SSH trust boundary. */
...
/** JSON-RPC request method Oak issues after `--connect` reattach to ask the
...
/** JSON-RPC request method Oak issues at session-ready to ship the
...
/** Feature-flag env var. Read once at process start by Oak and the relay.
 *  Remote agent hooks ship as the default SSH behavior; set "0" to opt out. */
export const OAK_FEATURE_REMOTE_AGENT_HOOKS_ENV = 'OAK_FEATURE_REMOTE_AGENT_HOOKS' as const

export function isRemoteAgentHooksEnabled(env: NodeJS.ProcessEnv = process.env): boolean {
  const raw = env[OAK_FEATURE_REMOTE_AGENT_HOOKS_ENV]
  if (raw === undefined) {
    return true
  }
