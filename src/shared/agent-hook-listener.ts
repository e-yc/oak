/* eslint-disable max-lines -- Why: this module is the canonical, transport-
   ...
   between Oak's main process and the relay. Splitting by line count would
...
// docs/design/agent-status-over-ssh.md §3 ("relay normalizes; Oak routes").
...
import { OAK_HOOK_PROTOCOL_VERSION } from './agent-hook-types'
 ...
 *  Exported so non-HTTP ingest paths (e.g. Oak's `ingestRemote`) can apply
 ...
 *  (last prompt, last tool snapshot, last status replay). Both Oak's main
...
export function warnOnHookEnvOrVersionMismatch(
  ...
  if (
    version &&
    version !== OAK_HOOK_PROTOCOL_VERSION &&
    !state.warnedVersions.has(version) &&
    state.warnedVersions.size < MAX_WARNED_KEYS
  ) {
    state.warnedVersions.add(version)
    console.warn(
      `[agent-hooks] received hook v${version}; server expects v${OAK_HOOK_PROTOCOL_VERSION}. ` +
    ...
    if (!state.warnedEnvs.has(key) && state.warnedEnvs.size < MAX_WARNED_KEYS) {
      ...
      console.warn(
        `[agent-hooks] received ${env} hook on ${expectedEnv} server. ` +
          'Likely a stale terminal from another Oak install.'
      )
    }
  }
}
...
export type AgentHookEventPayload = {
   ...
   *  Stamped only on the remote-ingest path (Oak's `ingestRemote`); the
...
// those event names into Oak's shared status states.
...
// into Oak's shared status states while attributing the status to Kimi so the
...
export function normalizeHookPayload(
  ...
  // it null; the relay forwards null on the wire and Oak's `ingestRemote`
...
export function writeEndpointFile(
  ...
  const valuesToWrite: [string, string][] = [
    ['OAK_AGENT_HOOK_PORT', String(fields.port)],
    ['OAK_AGENT_HOOK_TOKEN', fields.token],
    ['OAK_AGENT_HOOK_ENV', fields.env],
    ['OAK_AGENT_HOOK_VERSION', fields.version]
  ]
  ...
  try {
    ...
    // not leak the existence of this Oak/relay install to other local users.
