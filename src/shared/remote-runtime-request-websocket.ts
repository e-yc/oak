...
export function openRemoteRuntimeWebSocket(
  ...
  const onError = (): void => {
    callbacks.onError(
      ws,
      remoteRuntimeUnavailableError('Could not connect to the remote Oak runtime.')
    )
  }
  ...
  const onMessage = (data: WebSocket.RawData, isBinary: boolean): void => {
    if (isBinary) {
      callbacks.onError(
        ws,
        invalidRemoteRuntimeResponseError('Remote Oak runtime returned an unexpected binary frame.')
      )
      return
    }
  ...
  const cleanup = (): void => {
    ...
    // that from becoming an unhandled EventEmitter error after detaching Oak.
