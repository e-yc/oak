...
export function describeRuntimeCompatBlock(verdict: RuntimeCompatVerdict): string {
  ...
  if (verdict.reason === 'client-too-old') {
    return `This Oak client is too old for the selected server. Update Oak on this machine. Client protocol ${verdict.clientProtocolVersion}, server requires client protocol ${verdict.requiredClientProtocolVersion}.`
  }
  return `The selected Oak server is too old for this client. Update Oak on the server. Server protocol ${verdict.serverProtocolVersion}, client requires server protocol ${verdict.requiredServerProtocolVersion}.`
}
