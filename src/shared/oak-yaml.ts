...
import type { OakDefaultTabTemplate, OakHooks, OakVmRecipe, OakVmRecipeDiagnostic } from './types'
...
export const OAK_VM_RECIPE_ID_PATTERN = /^[a-z0-9][a-z0-9._-]{0,63}$/
export const OAK_VM_RECIPE_ID_RULE =
  'Use 1-64 lowercase letters, numbers, dots, underscores, or hyphens, starting with a letter or number.'

function normalizeDefaultTabs(value: unknown): OakDefaultTabTemplate[] {
  ...
  return value
    ...
    .filter((entry): entry is OakDefaultTabTemplate => entry !== null)
}

type VmRecipeParseResult = {
  recipes: OakVmRecipe[]
  diagnostics: OakVmRecipeDiagnostic[]
}

function normalizeVmRecipes(value: unknown): VmRecipeParseResult {
  const diagnostics: OakVmRecipeDiagnostic[] = []
  if (!Array.isArray(value)) {
  ...
  const recipes = value
    .map((entry, index) => {
      ...
      if (!OAK_VM_RECIPE_ID_PATTERN.test(id)) {
        diagnostics.push({
          index,
          field: 'id',
          message: `Invalid recipe id "${id}". ${OAK_VM_RECIPE_ID_RULE}`
        })
        return null
      }
      if (seenIds.has(id)) {
    ...
    .filter((entry): entry is OakVmRecipe => entry !== null)
  return { recipes, diagnostics }
}

/**
 * Parse the supported project defaults from `oak.yaml`.
 */
export function parseOakYaml(content: string): OakHooks | null {
  let root: unknown
  try {
    root = parse(content)
  } catch {
