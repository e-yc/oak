...
export function parseAgentHookEndpointFile(contents: string): AgentHookEndpoint {
  ...
  if (
    !values.OAK_AGENT_HOOK_PORT ||
    !values.OAK_AGENT_HOOK_TOKEN ||
    !values.OAK_AGENT_HOOK_ENV ||
    !values.OAK_AGENT_HOOK_VERSION
  ) {
    throw new Error('Agent hook endpoint file is missing required fields')
  }
  return {
    port: values.OAK_AGENT_HOOK_PORT,
    token: values.OAK_AGENT_HOOK_TOKEN,
    env: values.OAK_AGENT_HOOK_ENV,
    version: values.OAK_AGENT_HOOK_VERSION
  }
}
