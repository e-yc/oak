...
export function parseSharedControlFrame(
  ...
  if (!sharedKey) {
    return {
      type: 'error',
      error: invalidRemoteRuntimeResponseError('Remote Oak runtime returned a frame before E2EE.')
    }
  }
...
export function formatSharedControlCloseMessage(code: number, reason: Buffer): string {
  ...
  if (code !== 1005 && code !== 1006 && reasonText) {
    return `Remote Oak runtime closed the connection (${code}: ${reasonText}).`
  }
  if (code !== 1005 && code !== 1006) {
    return `Remote Oak runtime closed the connection (${code}).`
  }
  return 'Remote Oak runtime closed the connection.'
}
