...
export type Repo = {
  ...
  /** Controls Oak's fork-default-branch sync offer for repos with upstream metadata. */
  forkSyncMode?: ForkSyncMode
  /** Canonical identity for the repo remote Oak should use for provider-level grouping. */
  gitRemoteIdentity?: GitRemoteIdentity | null
  /** Controls whether worktrees Oak did not create appear in the sidebar. */
...
export type Worktree = {
  ...
  manualOrder?: number
  lastActivityAt: number
  /** Set once when Oak creates the worktree. Absent for worktrees discovered
  ...
  createdAt?: number
  /** Agent selected when Oak originally created the worktree. Used only to
  ...
  baseRef?: string
  /** Remote/branch Oak should publish review commits to when it created this worktree. */
...
export type GitPushTarget = {
  ...
  branchName: string
  remoteUrl?: string
  /** True when Oak added this remote while preparing a fork-PR worktree. */
...
export type WorktreeMeta = {
  ...
  manualOrder?: number
  lastActivityAt: number
  /** See {@link Worktree.createdAt}. Persisted to oak-data.json. */
  createdAt?: number
  /** See {@link Worktree.createdWithAgent}. Persisted to oak-data.json. */
  createdWithAgent?: TuiAgent
  ...
  /** Intended create base for stale-base probes. Persisted metadata, not UI drift state. */
  baseRef?: string
  /** True when Oak checked out a pre-existing local branch that delete must not prune. */
  preserveBranchOnDelete?: boolean
  /** See {@link Worktree.pushTarget}. Persisted so refreshed worktree lists keep the target. */
  pushTarget?: GitPushTarget
  /** Explicit marker stamped when Oak creates the worktree. */
  oakCreatedAt?: number
  oakCreationSource?: 'desktop' | 'runtime' | 'cli' | 'ssh'
  /** Workspace layout active when Oak created the worktree. */
  oakCreationWorkspaceLayout?: OakWorkspaceLayout
...
export type WorktreeOwnership = 'oak-managed' | 'external' | 'unknown-legacy'
...
// existing persistence layer writes them to oak-data.json automatically.
...
export type TerminalTab = {
   ...
   *  Why: agent CLIs overwrite the live title via OSC updates, but Oak still
  ...
  startupCwd?: string
  /** Why: the coding-harness agent Oak launched in this tab. Lets the tab bar
...
export type BrowserWorkspace = {
  id: string
  worktreeId: string
  /** Stable display label for the outer Oak tab ("Browser 1", "Browser 2", …).
  ...
  // Oak tab strip still renders one workspace entry. Mirror the active page's
...
export type GitHubWorkItemDetails = {
  // Why: main-process doesn't know Oak's Repo.id, so this inner item omits
...
export type OakHooks = {
  ...
  defaultTabs?: OakDefaultTabTemplate[] // Terminal tabs to create once for a new worktree
  environmentRecipes?: OakVmRecipe[] // Project-scoped per-workspace environment recipes
  environmentRecipeDiagnostics?: OakVmRecipeDiagnostic[] // Non-fatal validation issues from environmentRecipes
}

export type OakDefaultTabTemplate = {
  title?: string
  color?: string
  command?: string
}

export type OakVmRecipe = {
  id: string
  name: string
  create: string
  description?: string
...
export type OakVmRecipeDiagnostic = {
  index: number
  field?: string
  message: string
}
...
/** All AI coding agents Oak knows how to launch. Used for the agent picker in the new-workspace
 *  flow and for the default-agent setting. Extend this union as new agents are added. */
export type TuiAgent =
  | 'claude' // Claude Code
  | 'claude-agent-teams' // Claude Code Agent Teams via Oak native panes
...
export type GlobalSettings = {
  ...
  workspaceDirHistory?: OakWorkspaceLayout[]
  ...
  /** When enabled, Oak renames a workspace's auto-generated creature branch to
   ...
   *  need Oak to launch terminals and scan agents in the same chosen distro
   ...
   *  tmux-compatible shim so teammate output stays on Oak's normal PTY path. */
  ...
  /** Why: opening arbitrary links inside Oak uses an isolated guest browser surface.
  ...
  /** Whether to show the Oak app name in the titlebar. */
  ...
  /** Why: Oak Mobile remains reachable from Settings; this only controls
  ...
  /** Why: Oak-first preserves fast workspace/app control from agent TUIs.
   ...
   *  the user's home directory; markdown notes use Oak's app-owned
   ...
   *  ~/.oak/keybindings.json; main migrates this once when present. */
   ...
   *  auth here lets Oak prepare shared ~/.codex before the renderer hydrates,
  ...
  /** Why: Claude Code keeps conversations under one shared config root. Oak
   ...
   *  working directory), so Oak shows a confirmation dialog by default. Users
  ...
  /** When true, Oak requests local awake assertions while hook-reported agents are working. */
...
export type OakWorkspaceLayout = {
  path: string
  nestWorkspaces: boolean
}
...
export type PersistedUIState = {
  ...
  /** Whether Oak has already attempted to trigger the macOS notification
   ...
   *  tray, so the "Oak is still running" notification shows only on first use. */
  ...
  /** Once the user has starred Oak (from any entry point) we permanently
...
export type PersistedTrustedOakHookEntry = {
  contentHash: string
  approvedAt: number
}

export type PersistedTrustedOakHookRepo = {
  all?: {
    approvedAt: number
  }
  setup?: PersistedTrustedOakHookEntry
  archive?: PersistedTrustedOakHookEntry
  issueCommand?: PersistedTrustedOakHookEntry
  vmRecipe?: PersistedTrustedOakHookEntry
}

export type PersistedTrustedOakHooks = Record<string, PersistedTrustedOakHookRepo>
...
export type AppMemory = UsageValues & {
  ...
  other: UsageValues
  /** Oldest-first memory samples (bytes) for the whole Oak app, one per
