...
describe('pairing offer', () => {
  ...
  it('encode then decode round-trips correctly', () => {
    ...
    expect(url).toMatch(/^oak:\/\/pair\?code=/)
  ...
  it('rejects oak URLs outside the exact pairing route', () => {
    ...
    expect(parsePairingCode(`oak://pairing?code=${code}`)).toBeNull()
    expect(parsePairingCode(`oak://pair-extra?code=${code}`)).toBeNull()
    expect(() => decodePairingOffer(`oak://pairing?code=${code}`)).toThrow('Invalid pairing URL')
  })

  it('rejects URLs without a pairing code', () => {
    expect(() => decodePairingOffer('oak://pair')).toThrow('Invalid pairing URL')
  })

  it('decodes legacy hash URLs', () => {
    const url = encodePairingOffer(offer)
    const code = new URLSearchParams(url.slice(url.indexOf('?') + 1)).get('code')!
    expect(decodePairingOffer(`oak://pair#${code}`)).toEqual(offer)
  })
  ...
  it('rejects payloads with missing fields', () => {
    ...
    expect(() => decodePairingOffer(`oak://pair#${base64}`)).toThrow()
  })
