...
describe('ephemeral VM runtime store', () => {
  ...
  function makeUserDataPath(): string {
    const userDataPath = mkdtempSync(join(tmpdir(), 'oak-ephemeral-vm-store-'))
  ...
  it('updates lifecycle and cleanup state', () => {
    ...
    const failed = updateEphemeralVmRuntimeStatus(userDataPath, 'oak-instance-1', {
    ...
    const recovered = updateEphemeralVmRuntimeStatus(userDataPath, 'oak-instance-1', {
      status: 'cleaned',
