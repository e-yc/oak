...
const OAK_UNICODE_VERSION = 'oak-11-zwj'
...
class OakUnicodeProvider implements IUnicodeVersionProvider {
  public readonly version = OAK_UNICODE_VERSION
...
export function activateOakTerminalUnicodeProvider(terminal: XtermTerminalWithUnicodeCore): void {
  const { unicode } = terminal
  if (unicode.activeVersion === OAK_UNICODE_VERSION) {
    return
  }
  ...
  if (!unicode.versions.includes(OAK_UNICODE_VERSION)) {
    unicode.register(new OakUnicodeProvider(baseProvider))
  }
  unicode.activeVersion = OAK_UNICODE_VERSION
}
