...
if (typeof window !== 'undefined') {
  const guardedWindow = window as Window & {
    __oakContextualTourGlobalKeyGuardInstalled?: boolean
  }
  if (!guardedWindow.__oakContextualTourGlobalKeyGuardInstalled) {
    guardedWindow.__oakContextualTourGlobalKeyGuardInstalled = true
    window.addEventListener('keydown', handleContextualTourGlobalKeyDown, true)
  }
}

const PANEL_BASE_CLASSES =
  'oak-contextual-tour-panel rounded-lg border border-border text-popover-foreground backdrop-blur-[2px]'
  ...
  return (
    ...
    >
      {showTargetRings ? (
        <div
          aria-hidden="true"
          className="oak-contextual-tour-target-rings fixed z-[75]"
