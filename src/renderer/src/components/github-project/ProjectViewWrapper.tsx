...
const OAK_FEATURE_REQUEST_URL = 'https://github.com/e-yc/oak/issues/new'
...
export default function ProjectViewWrapper({ selectedRepoIds }: Props): React.JSX.Element {
  ...
  // simplified slug-mode dialog. `repoNotInOak` drives the fallback modal
  // from the design doc's `repo-not-in-oak` interaction state.
  ...
  // in Oak (matched repos go through the full GitHubItemDialog above), so
  // there's no `matchedRepo` to track here. The repo-not-in-oak modal —
  // owned by this parent, not the slug dialog — handles "Start work".
  const [slugDialog, setSlugDialog] = useState<{
    origin: GitHubItemDialogProjectOrigin
  } | null>(null)
  const [repoNotInOak, setRepoNotInOak] = useState<{
    owner: string
    repo: string
    url: string | null
  } | null>(null)
  ...
  if (resolvedDialogRepoItem !== dialogRepoItem) {
    ...
    // Oak; clear them before the modal tree receives stale repo ids.
  ...
  if (resolvedMissingRepoDialogs.repoNotInOak !== repoNotInOak) {
    setRepoNotInOak(resolvedMissingRepoDialogs.repoNotInOak)
  }
  ...
  return (
    <div className="flex min-h-0 min-w-0 flex-1 flex-col">
      ...
      {/* Slug-only simplified dialog for rows whose repo isn't added to Oak.
          Why: no Start-work affordance lives inside the slug dialog — the
          parent's `handleStartWork`/`repoNotInOak` modal owns that flow, so
      ...
      {/* repo-not-in-oak prompt: see design doc Interaction States. */}
      <Dialog
        open={resolvedMissingRepoDialogs.repoNotInOak !== null}
        onOpenChange={(open) => !open && setRepoNotInOak(null)}
      >
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>
              {translate(
                ...
                'Repository not in Oak'
              )}
            </DialogTitle>
            <DialogDescription>
              {resolvedMissingRepoDialogs.repoNotInOak
                ? translate(
                    'auto.components.github.project.ProjectViewWrapper.1850fceac8',
                    "{{value0}}/{{value1}} isn't added to Oak. Add it to start work, or open in GitHub.",
                    {
                      value0: resolvedMissingRepoDialogs.repoNotInOak.owner,
                      value1: resolvedMissingRepoDialogs.repoNotInOak.repo
                    }
                  )
                : null}
            </DialogDescription>
          ...
          <DialogFooter className="gap-2 sm:justify-end">
            <Button variant="ghost" onClick={() => setRepoNotInOak(null)}>
              {translate('auto.components.github.project.ProjectViewWrapper.dffa899f36', 'Cancel')}
            </Button>
            {resolvedMissingRepoDialogs.repoNotInOak?.url ? (
              <Button
                variant="outline"
                onClick={() => {
                  if (resolvedMissingRepoDialogs.repoNotInOak?.url) {
                    void window.api.shell.openUrl(resolvedMissingRepoDialogs.repoNotInOak.url)
                  }
                  setRepoNotInOak(null)
                }}
              >
                {translate(
  ...
  return (
    <div className="project-view-tab-strip flex min-h-[41px] min-w-0 flex-none items-end gap-1 overflow-x-auto overflow-y-hidden border-b border-border/50 bg-muted/20 px-3 pt-3">
      {views.map((v) => {
            ...
            title={
              supported
                ...
                : translate(
                    ...
                    "{{value0}} — Oak doesn't support {{value1}} project views yet. File a feature request at {{value2}}.",
                    { value0: v.name, value1: layoutLabel, value2: OAK_FEATURE_REQUEST_URL }
                  )
            }
            className={cn(
        ...
        const unsupportedMessage = `Oak doesn't support ${layoutLabel} project views yet.`
        return (
            ...
            <HoverCardTrigger asChild>
              <span
                ...
                aria-label={translate(
                  ...
                  { value0: v.name, value1: unsupportedMessage, value2: OAK_FEATURE_REQUEST_URL }
                )}
              ...
              <div className="space-y-2">
                <p className="text-xs leading-5 text-muted-foreground">
                  ...
                  {translate(
                    ...
                    'Switch to a Table view to work with this project in Oak.'
                  )}
                </p>
                <Button
                  type="button"
                  size="xs"
                  variant="outline"
                  onClick={() => void window.api.shell.openUrl(OAK_FEATURE_REQUEST_URL)}
                >
                  {translate(
...
function ErrorState({
  ...
  const copy =
    error.type === 'too_large'
      ? `This view has ${totalCount ?? 'many'} items — too large to render in Oak. Narrow the view's filter on GitHub.`
      : error.type === 'unsupported_layout'
        ? 'Oak only renders table views yet. This is a Board or Roadmap view.'
