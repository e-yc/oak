...
function DiagnosticsDisabledStateNote({
  ...
  const message =
    reason === 'do_not_track'
      ...
      : reason === 'oak_telemetry_disabled'
        ? translate(
            'auto.components.settings.PrivacyDiagnosticsRows.63d03261d1',
            'OAK_TELEMETRY_DISABLED=1 is set — creating and sending diagnostic files is disabled.'
          )
        : reason === 'oak_diagnostics_disabled'
          ? translate(
              'auto.components.settings.PrivacyDiagnosticsRows.d37e92a06b',
              'OAK_DIAGNOSTICS_DISABLED=1 is set — app diagnostics are off.'
            )
          : reason === 'ci'
            ? translate(
