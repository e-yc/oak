...
describe('PrivacyPane — isEnvBlocked', () => {
  ...
  it('returns true for OAK_TELEMETRY_DISABLED', () => {
    expect(isEnvBlocked({ effective: 'disabled', reason: 'oak_disabled' })).toBe(true)
  })
...
describe('PrivacyPane — envVarNameForReason', () => {
  ...
  it('maps oak_disabled to OAK_TELEMETRY_DISABLED', () => {
    expect(envVarNameForReason('oak_disabled')).toBe('OAK_TELEMETRY_DISABLED')
  })
...
describe('PrivacyPane — computeBlockedReason', () => {
  ...
  it('names OAK_TELEMETRY_DISABLED as the env reason when set', () => {
    const result = computeBlockedReason({ effective: 'disabled', reason: 'oak_disabled' })
    expect(result).toEqual({ kind: 'env', reason: 'oak_disabled' })
  })
