...
export function BrowserUseSetup({
  ...
  const browserUseInstallCommand = !activeSkillRuntime.installDisabledReason
    ? buildSkillCommandForRuntime(OAK_CLI_SKILL_INSTALL_COMMAND, activeSkillRuntime.agentRuntime)
    : OAK_CLI_SKILL_INSTALL_COMMAND
  const browserUseUpdateCommand = !activeSkillRuntime.installDisabledReason
    ? buildSkillCommandForRuntime(OAK_CLI_SKILL_UPDATE_COMMAND, activeSkillRuntime.agentRuntime)
    : OAK_CLI_SKILL_UPDATE_COMMAND
  ...
  const cliEnabled = isOakCliAvailableOnPath(cliStatus)
      ...
      {showStep2 ? (
        <SearchableSetting
          ...
          description={translate(
            ...
            "Install the Browser Use skill so agents can operate Oak's browser."
          )}
