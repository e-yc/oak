...
import { runOakContextMenuPaste, type OakContextMenuPasteDeps } from './monaco-context-menu-paste'
...
function makeDeps(
  overrides: Partial<OakContextMenuPasteDeps> & { editor?: ReturnType<typeof makeEditor> }
): OakContextMenuPasteDeps {
...
describe('runOakContextMenuPaste', () => {
  ...
  it('reads via IPC and dispatches a native paste for writable focused editors', async () => {
    ...
    const outcome = runOakContextMenuPaste(deps)
  ...
  it('applies in-app copy metadata (empty-selection + multicursor) when enabled', async () => {
    ...
    await runOakContextMenuPaste(deps)
    expect(handle.trigger).toHaveBeenCalledWith('keyboard', 'paste', {
      text: 'line text',
      pasteOnNewLine: true,
      multicursorText: ['a', 'b'],
  ...
  it('does nothing on an empty clipboard', async () => {
    ...
    await expect(runOakContextMenuPaste(deps)).resolves.toEqual({
      status: 'noop',
      reason: 'empty'
    })
  ...
  it('routes oversized pastes through the chunked inserter instead of a native trigger', async () => {
    ...
    const outcome = await runOakContextMenuPaste(deps)
