...
export function dispatchTerminalNotification(
  ...
  if (event.source === 'agent-task-complete') {
    ...
    const shouldMarkUnread = event.paneKey
      ...
      : state.activeWorktreeId !== worktreeId || !isOakWindowForegroundFocused()
    if (shouldMarkUnread) {
      // Why: activeWorktreeId is only in-app selection. If Oak is backgrounded,
