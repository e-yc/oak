...
export const EMULATOR_LOCAL_SHUTDOWN_EVENT = 'oak:emulator-shutdown'
...
export function useEmulatorPaneSessionEvents({
  ...
  useEffect(() => {
    ...
    window.addEventListener('oak:emulator-auto-attach', onAuto)
    return () => window.removeEventListener('oak:emulator-auto-attach', onAuto)
