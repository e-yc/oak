...
import { OAK_CLI_SKILL_NAME } from '@/lib/agent-feature-install-commands'
import {
...
function getCliActionLabel(status: CliInstallStatus | null, busy: boolean): string {
  ...
  if (isOakCliAvailableOnPath(status)) {
    return translate(
...
export function useMobileEmulatorAgentSetupState(enabled = true): {
  ...
  const cliEnabled = isOakCliAvailableOnPath(cliInstallStatus)
  ...
  const recheckSetup = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      const cliReady = isOakCliAvailableOnPath(cliStatus)
      if (!mountedRef.current) {
        return
      }
      ...
      if (skillInstalled) {
        toast.message(
          translate(
            ...
            'Skill is installed. Enable the Oak CLI to finish setup.'
          )
        )
        return
      }
      if (cliReady) {
        toast.message(
          translate(
            ...
            'Oak CLI is ready. Install the skill to finish setup.'
          )
        )
        return
      }
  ...
  const handleEnableCli = useCallback(async (): Promise<void> => {
    setCliBusy(true)
    try {
      const next = await ensureOakCliAvailableForAgentSkillTerminal({
        onStatusChange: setCliInstallStatus
      })
      if (mountedRef.current && isOakCliAvailableOnPath(next)) {
        toast.success(
          translate(
            'auto.components.emulator.pane.use.mobile.emulator.agent.setup.state.2b519eed94',
            'Registered the Oak CLI in PATH.'
          )
        )
      }
    } finally {
