...
import { showOakCliRegistrationPromptToast } from '@/lib/agent-skill-cli-prerequisite'
import {
...
export function createOnboardingFeatureSetupDeps(): OnboardingFeatureSetupDeps {
  ...
  return {
    ...
    showCliRegistrationPrompt: showOakCliRegistrationPromptToast,
...
export async function runOnboardingFeatureSetup(
  ...
  try {
    ...
    if (!status.supported) {
      warnings.push({
        featureId: 'cli',
        message: status.detail ?? 'Oak CLI registration is not available on this platform.'
      })
      ...
      if (next.state !== 'installed') {
        warnings.push({
          featureId: 'cli',
          message: next.detail ?? 'Oak CLI registration needs attention.'
        })
