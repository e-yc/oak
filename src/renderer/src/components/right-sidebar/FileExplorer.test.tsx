...
describe('FileExplorerToolbar', () => {
  ...
  it('adds open-in launchers to the overflow menu', () => {
    ...
    expect(openInItems.props.worktreePath).toBe('/tmp/oak')
...
describe('FileExplorerRow collapse folder action', () => {
  ...
  it('shows OS file copy for single local rows and SSH file rows on desktop', () => {
    const previous = (globalThis as { __OAK_WEB_CLIENT__?: boolean }).__OAK_WEB_CLIENT__
    try {
      ...
      ;(globalThis as { __OAK_WEB_CLIENT__?: boolean }).__OAK_WEB_CLIENT__ = previous
    }
  })
