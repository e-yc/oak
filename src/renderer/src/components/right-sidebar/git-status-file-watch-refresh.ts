...
export function useGitStatusFileWatchRefresh({
  ...
  useEffect(() => {
    ...
    window.addEventListener(OAK_WORKTREE_FILE_CHANGE_EVENT, handleFsChanged as EventListener)

    return () => {
      if (refreshTimer) {
        clearTimeout(refreshTimer)
      }
      window.removeEventListener(OAK_WORKTREE_FILE_CHANGE_EVENT, handleFsChanged as EventListener)
    }
