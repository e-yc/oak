...
import {
  OAK_TERMINAL_COMMAND_FINISHED_EVENT,
...
export function useGitStatusPushSignalRefresh({
  ...
  useEffect(() => {
    ...
    window.addEventListener(OAK_TERMINAL_COMMAND_FINISHED_EVENT, handleCommandFinished)
    return () => {
      window.removeEventListener(OAK_TERMINAL_COMMAND_FINISHED_EVENT, handleCommandFinished)
    }
