...
const OAK_STARGAZERS_URL = 'https://github.com/e-yc/oak/stargazers'
...
function GitHubStarButton({ hasRepos }: { hasRepos: boolean }): React.JSX.Element | null {
  ...
  useEffect(() => {
    let cancelled = false
    void window.api.gh.checkOakStarred().then((result) => {
      if (cancelled) {
        return
      }
      if (result === null) {
  ...
  const handleClick = async (): Promise<void> => {
    ...
    if (state === 'web-fallback') {
      await window.api.shell.openUrl(OAK_STARGAZERS_URL)
      return
    }
    ...
    const ok = await window.api.gh.starOak('landing')
    if (!ok) {
...
export default function Landing(): React.JSX.Element {
  ...
  useEffect(() => {
    ...
    // Why: users often install/authenticate gh outside Oak. Re-check when the
  ...
  useEffect(() => {
    ...
    let cancelled = false
    // Why: some users complete `gh auth login` without ever leaving the Oak
      ...
      <div className="w-full max-w-lg px-6">
        <div className="flex flex-col items-center gap-4 py-8">
          ...
          >
            <img
              src={logo}
              alt={translate('auto.components.Landing.520304a067', 'Oak logo')}
              className="size-12"
            />
          </div>
          <h1 className="text-4xl font-bold text-foreground tracking-tight">
            {translate('auto.components.Landing.6ca6ff404e', 'OAK')}
          </h1>
