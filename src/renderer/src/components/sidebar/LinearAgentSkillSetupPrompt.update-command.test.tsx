...
vi.mock('@/lib/agent-skill-cli-prerequisite', () => ({
  ...
  ensureOakCliAvailableForAgentSkillTerminal: vi.fn(async () => null),
...
function discoveredSkill(overrides: Partial<DiscoveredSkill>): DiscoveredSkill {
  return {
    ...
    directoryPath: '/Users/test/.agents/skills/oak-linear',
    skillFilePath: '/Users/test/.agents/skills/oak-linear/SKILL.md',
    installed: true,
    fileCount: 1,
    updatedAt: null,
    ...overrides
...
describe('LinearAgentSkillSetupPrompt update command', () => {
  ...
  it('uses the canonical update command when the canonical Linear skill is installed', async () => {
    mocks.skillState.skills = [discoveredSkill({ name: 'oak-linear' })]

    await renderPrompt()

    expect(mocks.panelProps.at(-1)).toEqual(
      expect.objectContaining({ installedCommand: 'npx skills update oak-linear --global' })
    )
  })
  ...
  it('prefers the canonical update command when both Linear skill names are installed', async () => {
    mocks.skillState.skills = [discoveredSkill({ name: 'oak-linear' }), legacyLinearSkillPath()]
