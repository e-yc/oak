  ...
  return {
    ...
    descriptionSuffix: args.isBatchDelete
      ? allFolderWorkspaceDeletes
        ? 'from Oak. Project folders on disk will not be deleted.'
        : mixedFolderWorkspaceDeletes
          ? 'from Oak. Git worktrees will also be removed from git and disk; folder workspaces will only remove the Oak workspace entry.'
          : 'from git and delete their workspace folders.'
      : args.isFolderWorkspaceDelete
        ? 'from Oak. The project folder on disk will not be deleted.'
