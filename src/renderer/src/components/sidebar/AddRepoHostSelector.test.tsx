...
describe('AddRepoHostSelector', () => {
  it('shows a remote host setup menu when Local Mac is the only host', () => {
    ...
    expect(html).toContain('Pair with Oak running on another computer.')
  })
  ...
  it('shows exact update guidance for incompatible runtime hosts', () => {
    ...
    expect(html).toContain('The selected Oak server is too old for this client.')
    expect(html).toContain('Update Oak on the server.')
    expect(html).toContain('aria-disabled="true"')
  })
})
