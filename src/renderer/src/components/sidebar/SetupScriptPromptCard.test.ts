  ...
  it('uses the current inspection when it belongs to the active repo', () => {
    ...
    expect(
      getRenderedSetupScriptPromptState({
        ...
        activeProjectId: 'github:e-yc/oak',
        lastVisiblePrompt: { state: prompt('repo-ssh'), projectId: 'github:e-yc/oak' }
      })
    ).toBe(current)
  })
  ...
  it('keeps the previous visible prompt during same-project host inspection refresh', () => {
    ...
    expect(
      getRenderedSetupScriptPromptState({
        ...
        activeProjectId: 'github:e-yc/oak',
        lastVisiblePrompt: { state: previous, projectId: 'github:e-yc/oak' }
      })
    ).toBe(previous)
  })
  ...
  it('does not keep a stale prompt when switching to a different project', () => {
    expect(
      getRenderedSetupScriptPromptState({
        ...
        lastVisiblePrompt: { state: prompt('repo-local'), projectId: 'github:e-yc/oak' }
      })
    ).toBeNull()
  })
})
