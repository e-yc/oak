...
describe('feature tip telemetry', () => {
  ...
  it('keeps feature tip sources low-cardinality', () => {
    expect(getOakCliFeatureTipTelemetrySource('app_open')).toBe('app_open')
    expect(getOakCliFeatureTipTelemetrySource('settings')).toBe('manual')
    expect(getOakCliFeatureTipTelemetrySource(undefined)).toBe('manual')
  })
  ...
  it('tracks CLI tip exposure once per explicit call', () => {
    ...
    expect(trackMock).toHaveBeenCalledWith('oak_cli_feature_tip_shown', {
      source: 'app_open'
    })
  })
  ...
  it('tracks setup click and result without raw CLI details', () => {
    trackOakCliFeatureTipSetupClicked('app_open')
    trackOakCliFeatureTipSetupResult('app_open', 'installed')

    expect(trackMock).toHaveBeenCalledTimes(2)
    expect(trackMock).toHaveBeenNthCalledWith(1, 'oak_cli_feature_tip_setup_clicked', {
      source: 'app_open'
    })
    expect(trackMock).toHaveBeenNthCalledWith(2, 'oak_cli_feature_tip_setup_result', {
      source: 'app_open',
      result: 'installed'
    })
  })
