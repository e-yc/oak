...
describe('repo-search', () => {
  ...
  it('matches display names case-insensitively', () => {
    const repos = [
      makeRepo({ id: '1', displayName: 'e-yc/oak', path: '/repos/oak' }),
      makeRepo({ id: '2', displayName: 'stablyai/noqa', path: '/repos/noqa' })
    ]

    expect(searchRepos(repos, 'OAK').map((repo) => repo.id)).toEqual(['1'])
  })

  it('falls back to matching repo paths', () => {
    const repos = [
      makeRepo({ id: '1', displayName: 'frontend', path: '/src/team-a/oak' }),
  ...
  it('keeps display-name matches ahead of path-only matches', () => {
    const repos = [
      makeRepo({ id: '1', displayName: 'misc', path: '/src/oak-tools/misc' }),
      makeRepo({ id: '2', displayName: 'oak', path: '/src/team-a/project' })
    ]

    expect(searchRepos(repos, 'oak').map((repo) => repo.id)).toEqual(['2', '1'])
  })
