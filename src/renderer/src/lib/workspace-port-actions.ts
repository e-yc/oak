...
export function shouldOpenWorkspacePortInOakBrowser(
...
export function resolvePortOpenInOakBrowser({
  settings,
  event,
  isMac
}: {
  ...
  return shouldOpenWorkspacePortInOakBrowser(settings)
}
...
export async function openWorkspacePortInBrowser(args: {
  ...
  if (args.openInOakBrowser === false && args.runtimeTarget.kind === 'local') {
    try {
