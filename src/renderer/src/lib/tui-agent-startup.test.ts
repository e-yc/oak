...
describe('buildAgentDraftLaunchPlan', () => {
  ...
  it('uses OAK_PI_PREFILL env var for pi (no CLI flag exists)', () => {
    // Why: pi has no `--prefill` flag, and bracketed-paste-after-ready races
    // against pi's lengthy startup output. The Oak overlay installs an
    // `oak-prefill` extension that reads OAK_PI_PREFILL on session_start
    ...
    expect(
      ...
      agent: 'pi',
      launchCommand: 'pi; unset OAK_PI_PREFILL',
      expectedProcess: 'pi',
      env: { OAK_PI_PREFILL: 'https://github.com/acme/repo/issues/42' },
