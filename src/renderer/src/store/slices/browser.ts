...
import { FLOATING_TERMINAL_WORKTREE_ID, OAK_BROWSER_BLANK_URL } from '../../../../shared/constants'
...
function normalizeBrowserTitle(title: string | null | undefined, url: string): string {
  if (
    ...
    // Why: blank pages render through Oak's inert data: URL guest. Persisting
...
function buildBrowserPage(
  ...
  return {
    ...
    loading: normalizedUrl !== 'about:blank' && normalizedUrl !== OAK_BROWSER_BLANK_URL,
    faviconUrl: null,
    canGoBack: false,
    canGoForward: false,
    loadError: null,
...
export const createBrowserSlice: StateCreator<AppState, [], [], BrowserSlice> = (set, get) => ({
    ...
    set((s) => {
      ...
      const shouldFocusAddressBar =
        ...
        (options?.focusAddressBar ??
          (page.url === 'about:blank' || page.url === OAK_BROWSER_BLANK_URL))
  ...
  setBrowserPageUrl: (pageId, url) => {
    ...
    if (nextUrl !== 'about:blank' && nextUrl !== OAK_BROWSER_BLANK_URL) {
  ...
  addBrowserHistoryEntry: (url, title) => {
    ...
    if (safeUrl === OAK_BROWSER_BLANK_URL || safeUrl === 'about:blank' || !safeUrl) {
      return
    }
