...
export type AgentStatusSlice = {
   ...
   *  carry Oak launch settings, so the renderer supplies them from startup. */
  ...
  return {
    ...
    setAgentStatus: (paneKey, payload, terminalTitle, timing, routing, metadata) => {
      ...
      set((s) => {
        ...
        // an Oak-launched one. Once the provider session changes, the old
        ...
        // Why: launch tokens can remain in a shell after an Oak-started TUI exits;
      ...
      if (completionRefreshWorktreeId) {
        ...
        // Why: agents can create a PR via `gh pr create`, bypassing Oak's
