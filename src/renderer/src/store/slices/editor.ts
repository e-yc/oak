...
export const createEditorSlice: StateCreator<AppState, [], [], EditorSlice> = (set, get) => ({
  ...
  closeFile: (fileId) => {
    ...
    set((s) => {
      ...
      return {
        ...
        // close handlers and return to the Oak landing state instead of
  ...
  beginRemoteOperation: (kind) =>
    set((s) => ({
      ...
      // so a second remote op can't be started from inside Oak. If a
  ...
  activateMarkdownLink: async (rawHref, ctx) => {
    ...
    if (target.kind === 'file') {
      ...
      if (target.relativePath === undefined) {
        ...
        // before opening them in Oak. Markdown file:// links need the same
      ...
      for (const [worktreeId, files] of Object.entries(openFilesByWorktree)) {
        ...
        for (const pf of files) {
          ...
          openFiles.push({
            ...
            worktreeId,
            // Why: sessions can contain language ids from older Oak builds.
