...
describe('composer drop upload result', () => {
  it('separates imported files and folders while counting skipped or failed paths', () => {
    const results: ComposerDropUploadImportResult[] = [
      { status: 'imported', kind: 'file', destPath: '/repo/.oak/drops/file.txt' },
      { status: 'imported', kind: 'directory', destPath: '/repo/.oak/drops/folder' },
      { status: 'skipped' },
      { status: 'failed' }
    ]

    expect(collectComposerDropUploadResult(results)).toEqual({
      filePaths: ['/repo/.oak/drops/file.txt'],
      folderPaths: ['/repo/.oak/drops/folder'],
      skippedOrFailed: 2
    })
  })
