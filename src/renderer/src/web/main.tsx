...
function WebRoot(): React.JSX.Element {
  ...
  const [hasEnvironment, setHasEnvironment] = useState(() => {
    if (startupDecision.kind === 'auto-save-runtime-offer') {
      saveStoredWebRuntimeEnvironment(
        createStoredWebRuntimeEnvironment({ name: 'Oak Server', offer: startupDecision.offer })
      )
      return true
    }
...
function WebRootBoundary(): React.JSX.Element {
  ...
  return (
    <RecoverableRenderErrorBoundary
      boundaryId="web.root"
      surface="web-root"
      title={translate('app.recoverableError.webTitle', 'Oak web hit a renderer error.')}
