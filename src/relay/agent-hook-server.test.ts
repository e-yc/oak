...
describe('RelayAgentHookServer', () => {
  ...
  it('keeps named-pipe endpoint files on a real filesystem path', () => {
    const endpointDir = endpointDirForRelaySocket('\\\\.\\pipe\\oak-relay-abc123')

    expect(endpointDir).toBe(join(homedir(), '.oak-relay', 'agent-hooks', 'oak-relay-abc123'))
  ...
  it('forwards a parsed Claude UserPromptSubmit POST as a normalized envelope', async () => {
    ...
    try {
      ...
      // Why: the relay forwards body env/version so Oak's warn-once
  ...
  // the wire, even though Oak main re-validates at the SSH trust boundary.
  ...
  it('exposes OAK_AGENT_HOOK_* env vars after start', async () => {
    const forward = vi.fn()
    const server = new RelayAgentHookServer({ endpointDir: dir, forward })
    await server.start()
    try {
      const env = server.buildPtyEnv()
      expect(env.OAK_AGENT_HOOK_PORT).toMatch(/^\d+$/)
      expect(env.OAK_AGENT_HOOK_TOKEN).toBeTruthy()
      expect(env.OAK_AGENT_HOOK_ENV).toBe('remote')
      expect(env.OAK_AGENT_HOOK_VERSION).toBe('1')
      expect(env.OAK_AGENT_HOOK_ENDPOINT).toBeTruthy()
    } finally {
      server.stop()
    }
  })
  ...
  it('can defer endpoint file publication until relay socket ownership is proven', async () => {
    ...
    try {
      ...
      expect(server.buildPtyEnv().OAK_AGENT_HOOK_ENDPOINT).toBeTruthy()
    } finally {
      server.stop()
    }
  })
