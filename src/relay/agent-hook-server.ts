...
// a loopback HTTP server (same shape as Oak's main-process server: bind
...
const RELAY_HOOKS_DIR_NAME = '.oak-relay'
...
export type RelayHookServerOptions = {
  /** Where to put endpoint.env / endpoint.cmd. Defaults to `$HOME/.oak-relay/agent-hooks`. */
  endpointDir?: string
...
export class RelayAgentHookServer {
  ...
  // the original `source`/`env`/`version` so Oak's warn-once diagnostics fire
   ...
   *  forwards each entry as a fresh notification. Called after Oak has
  ...
  buildPtyEnv(): Record<string, string> {
    ...
    if (this.endpointFileWritten) {
      env.OAK_AGENT_HOOK_ENDPOINT = this.endpointFilePath
    }
    return env
  }
