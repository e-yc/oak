...
describe('getRelayShellLaunchConfig', () => {
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    () => {
      ...
      const zshRoot = join(homeDir, '.oak-relay', 'shell-ready', 'zsh')
      ...
      expect(readFileSync(join(zshRoot, '.zprofile'), 'utf8')).toContain(
        '_oak_home="${OAK_USER_ZDOTDIR:-${OAK_ORIG_ZDOTDIR:-$HOME}}"'
      )
      ...
      expect(zshrc).toContain('_oak_home="${OAK_USER_ZDOTDIR:-${OAK_ORIG_ZDOTDIR:-$HOME}}"')
      expect(zlogin).toContain('_oak_home="${OAK_USER_ZDOTDIR:-${OAK_ORIG_ZDOTDIR:-$HOME}}"')
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    () => {
      ...
      expect(config.args).toEqual(['-l'])
      expect(config.env.ZDOTDIR).toBe(zshRoot)
      expect(config.env.OAK_SHELL_READY_MARKER).toBe('1')
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    () => {
      ...
      expect(config.env.OAK_SHELL_READY_MARKER).toBe('1')
      expect(bashRc).toContain('__oak_append_prompt_command "__oak_prompt_mark"')
      expect(bashRc).toContain('printf "\\033]777;oak-shell-ready\\007"')
    }
  )
