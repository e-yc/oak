// Why: relay-side equivalent of Oak's local agent integration installers.
// OpenCode still needs a config overlay, while Pi/OMP now get Oak-managed
...
// relay is versioned independently from Oak and the plugin source changes
...
// directly: those modules import `electron` and ride on Oak's userData
// path. The relay's electron-free constraint forces a thin parallel
// implementation rooted at $HOME/.oak-relay/ for OpenCode and at the remote
...
const RELAY_HOOKS_DIR = '.oak-relay'
const OPENCODE_OVERLAY_SUBDIR = 'opencode-overlays'
const PI_OVERLAY_SUBDIR_BY_KIND: Record<PiAgentKind, string> = {
  pi: 'pi-overlays',
  omp: 'omp-overlays'
}
const OPENCODE_PLUGIN_FILE = 'oak-opencode-status.js'
const PI_EXTENSION_FILE = 'oak-agent-status.ts'
const PI_AGENT_SUBDIR = 'agent'
const OAK_MANAGED_EXTENSION_MARKER = '@oak-managed-pi-extension'

function withOakManagedPiExtensionMarker(source: string): string {
...
function safeDirName(input: string): string {
  ...
  // filesystem-unsafe characters in some Oak builds. Hash to a fixed-width
...
export type PluginSources = {
  /** Source body of `oak-opencode-status.js` to drop into <overlay>/plugins/. */
  opencodePluginSource?: string
  /** Source body of Pi's `oak-agent-status.ts` to drop into <overlay>/extensions/. */
  piExtensionSource?: string
  /** Source body of OMP's `oak-agent-status.ts` to drop into <overlay>/extensions/. */
...
export class PluginOverlayManager {
  ...
  /** Replace the cached source bodies. Called from relay.ts when Oak sends
   *  `agent_hook.installPlugins`. The first install enables the augmenter
   *  output; subsequent installs (e.g. Oak version upgrade in flight) refresh
  ...
  setSources(sources: PluginSources): void {
    ...
    if (typeof sources.piExtensionSource === 'string') {
      this.piExtensionSources.pi = withOakManagedPiExtensionMarker(sources.piExtensionSource)
    }
    if (typeof sources.ompExtensionSource === 'string') {
      this.piExtensionSources.omp = withOakManagedPiExtensionMarker(sources.ompExtensionSource)
    }
  }
  ...
  materializeOpenCode(id: string, existingConfigDir?: string): string | null {
    ...
    try {
      ...
      if (existingConfigDir) {
        ...
        // remote root into the overlay before adding Oak's plugin so status
