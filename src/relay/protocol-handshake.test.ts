...
describe('handshake framing', () => {
  it('round-trips an oak-relay-handshake envelope through the existing framing', () => {
    const sent = encodeHandshakeFrame({
      type: 'oak-relay-handshake',
    ...
    expect(msg).toEqual({ type: 'oak-relay-handshake', version: '0.1.0+deadbeef' })
  })

  it('round-trips an oak-relay-handshake-ok reply', () => {
    const sent = encodeHandshakeFrame({
      type: 'oak-relay-handshake-ok',
      version: '0.1.0+deadbeef'
    })
    const frames: DecodedFrame[] = []
    const decoder = new FrameDecoder((f) => frames.push(f))
    decoder.feed(sent)
    const msg = parseHandshakeMessage(frames[0].payload)
    expect(msg).toEqual({ type: 'oak-relay-handshake-ok', version: '0.1.0+deadbeef' })
  })

  it('round-trips an oak-relay-handshake-mismatch reply', () => {
    const sent = encodeHandshakeFrame({
  ...
  it('rejects payloads with unknown type', () => {
    const bogus = Buffer.from(JSON.stringify({ type: 'oak-something-else', version: 'x' }))
