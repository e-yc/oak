 ...
 *
 * Wires Oak's main-side SshChannelMultiplexer to the relay-side
...
describe('Integration: relay hook server → mux → AgentHookServer.ingestRemote', () => {
  ...
  beforeEach(async () => {
    ...
    oakServer = new AgentHookServer()
    // Why: Oak-side never starts an HTTP server in this test — `ingestRemote`
  ...
  it('clears remote Claude permission when the approved tool starts with matching identity', async () => {
    ...
    const start = Date.now()
    while (oakServer.getStatusSnapshot()[0]?.state !== 'working' && Date.now() - start < 1500) {
      await new Promise((r) => setImmediate(r))
    }
    expect(oakServer.getStatusSnapshot()).toEqual([
