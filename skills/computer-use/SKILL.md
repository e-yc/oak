...
name: computer-use
description: >-
  Use Oak's computer-use CLI to inspect and operate local desktop app windows
  ...
  accessibility actions. Also use for browser windows, webviews, Oak app UI,
  or other desktop UI. Triggers include "computer use", "oak computer", "read
...
Use this skill for desktop UI through `oak computer`. When the requested target is a website or web app, operate the desktop browser app/window that contains the page.

## Preconditions

- Prefer `oak computer ...`; on Linux, use `oak-ide computer ...` if `oak` is unavailable. In this Oak worktree, use `./config/scripts/oak-dev computer ...` only when testing the local dev runtime.
...
```bash
oak status --json
oak computer capabilities --json
```

## Core Loop

```bash
oak computer list-apps --json
oak computer get-app-state --app com.spotify.client --json
oak computer click --app com.spotify.client --element-index 42 --json
```
...
```bash
oak computer get-app-state --app com.microsoft.edgemac --json
oak computer get-app-state --app Spotify --json
oak computer get-app-state --app pid:12345 --json
```
...
```bash
oak computer permissions --json
oak computer capabilities --json
oak computer list-apps --json
oak computer list-windows --app <app> --json
oak computer get-app-state --app <app> --json
oak computer get-app-state --app <app> --restore-window --json
oak computer click --app <app> --element-index <index> --json
oak computer click --app <app> --x 100 --y 100 --json
oak computer perform-secondary-action --app <app> --element-index <index> --action <name> --json
oak computer set-value --app <app> --element-index <index> --value "text" --json
oak computer type-text --app <app> --text "text" --json
oak computer press-key --app <app> --key Return --json
oak computer hotkey --app <app> --key CmdOrCtrl+A --json
oak computer paste-text --app <app> --text "text" --json
oak computer scroll --app <app> (--element-index <index> | --x <x> --y <y>) --direction down --json
oak computer drag --app <app> --from-element-index <index> --to-element-index <index> --json
oak computer drag --app <app> --from-x 100 --from-y 100 --to-x 300 --to-y 300 --json
```
...
```bash
printf '%s' "$TEXT" | oak computer set-value --app <app> --element-index <index> --value-stdin --json
```
...
```bash
oak computer get-app-state --app com.microsoft.edgemac --restore-window --json
oak computer set-value --app com.microsoft.edgemac --element-index <addressBarIndex> --value "test123" --json
oak computer press-key --app com.microsoft.edgemac --key Return --json
```
...
- `app_not_found`: run `list-apps` and retry with the bundle ID. If the target is a web app such as Gmail, choose the desktop browser app/window that contains it; do not retry `oak computer ... --app Gmail` unchanged because `oak computer` app selectors refer to desktop apps, not website names.
...
- `screenshot_failed`: use `--no-screenshot` if tree state is enough; if the message names Screen Recording or screenshots permission, run `oak computer permissions --id screenshots --json`.
- `accessibility_error`: run `oak computer capabilities --json`; if the message names Accessibility permission, run `oak computer permissions --id accessibility --json`.
- Empty tree or no screenshot: app may have no visible window, be minimized, or need permissions.
- Permission errors: run `oak computer permissions --json`, or `oak computer permissions --id accessibility --json` / `--id screenshots --json` when the message names one permission, use the setup UI, then retry.

## Next Action

Confirm Oak status unless already checked, then run `oak computer capabilities --json`. For website or web-app targets such as Gmail, identify the desktop browser app/window that contains the page, then get that target app state with `oak computer get-app-state --app <app> --json`.
