---
name: oak-per-workspace-env
description: >-
  Set up, review, debug, or validate Oak per-workspace environment recipes —
  ...
  per-workspace environments, fix a `environmentRecipes` entry in `oak.yaml`, scaffold
  provider lifecycle scripts, or resolve an `oak vm recipe doctor` failure.
---
...
Oak is a **thin wrapper**: you guide, detect, and scaffold; you never own the user's cloud account,
...
**Quick-start (happy path):** interview the user (connection mode Oak-server vs SSH, provider, agent CLI,
git auth — §1.2) + read the provider's CLI docs → scaffold `scripts/oak-vm/` from §7 → run the
base-snapshot script, then the auth script (you invoke these by hand; not via `oak.yaml`) → wire
`environmentRecipes` in `oak.yaml` → `oak vm recipe doctor <id> --json` (free) → then the `--provision`
...
a long time, or need the user at the keyboard. Never create an Oak workspace or commit unless asked.

1. **Inspect the repo** for an existing `environmentRecipes` entry, `scripts/oak-vm/`, a state file, or setup
...
2. **Interview the user up front** — gather these choices and confirm them back before scaffolding
   ...
   - **Connection mode:** how Oak attaches to the environment — an **Oak server** (the VM runs
     `oak serve` and Oak pairs over its pairing URL; worked example §7f) or **SSH** (Oak connects to
   ...
   - **Provider:** Vercel Sandbox, Fly, Modal, an existing SSH host, … For non-obvious providers, also
     ...
     (host/port/user/key or proxy command) or only a provider-mediated interactive shell; Oak SSH mode
     needs the former.
...
7. **Wire the recipe** so `oak.yaml` points create/suspend/resume/destroy at the scripts (§8). The
   workspace composer reads `environmentRecipes` from the project's primary checkout of `oak.yaml`, **not** from
   a feature branch or worktree. So a recipe added only on a branch won't appear as a "Run on" option
   until that `oak.yaml` change is committed and merged to the project's primary branch. Tell the user
...
8. **Dry-run doctor** — `oak vm recipe doctor <recipe-id> --repo-path <repo> --json` (free, static; §9).
   Fix every failure before going live.
9. **[CHECKPOINT] Live self-test** — get the user's OK once, then run
   `oak vm recipe doctor <recipe-id> --provision --json` as a loop: it runs create → validates →
...
- **Connection mode** (Oak server vs SSH) confirmed with the user — see §1 step 2; it shapes the recipe.
...
A repo-local JSON file (e.g. `scripts/oak-vm/<provider>-state.json`) threads non-secret values between
...
Scaffold under `scripts/oak-vm/`. These are **shapes** — fill in the provider's real commands. All
...
- **Local-side** (`create`/`suspend`/`resume`/`destroy` + the base-snapshot/auth scripts the user
  ...
  or require WSL/Git-Bash and point `oak.yaml` at the right launcher.
...
Worked Vercel commands for this phase are in §7f. You run this script by hand (not via `oak.yaml`),
...
# name = oak-${OAK_VM_RECIPE_ID}-${OAK_VM_INSTANCE_ID} (sanitized, length-capped)
...
# 3. remote exec: start oak serve in the background and read the recipe JSON it writes (see below)
...
**The exact `oak serve` invocation and its output (verified — do not improvise the flags).** Inside the
VM, run:
...
**Binary name:** in a VM built from source (the Phase-2 flow), run it as `pnpm exec oak-dev serve …`
from the repo root — `oak-dev` is the in-repo entrypoint and is what the §7f example uses. Plain
`oak serve …` is the same command when the built CLI is installed on the VM's PATH. The flags/output
...
```json
{ "schemaVersion": 1, "pairingCode": "<oak pairing URL>", "projectRoot": "<the --project-root you passed>" }
```
...
#!/usr/bin/env bash
set -euo pipefail
payload="$(cat)"                       # Oak passes lifecycle JSON on stdin
...
# destroy: provider remove "$resource_id"   (or set destroy: none in oak.yaml)
```
...
name="oak-${OAK_VM_RECIPE_ID:-vercel-sandbox}-${OAK_VM_INSTANCE_ID:-$(date +%s)}"  # sanitize+cap to 63 chars
...
vercel sandbox exec "$name" "${vercel_args[@]}" --timeout 20m \
  --env "GH_TOKEN=$gh_token" --env "OAK_PROJECT_ROOT=$project_root" \
  --env "OAK_REPO_URL=$repo_url" --env "OAK_REPO_REF=$repo_ref" \
  -- bash -lc 'set -euo pipefail; cd "$OAK_PROJECT_ROOT"; \
    ...
    git fetch origin "$OAK_REPO_REF"; \
    git checkout -B "$OAK_REPO_REF" FETCH_HEAD; \
    c="$(git rev-parse HEAD)"; [ -f .oak-built ] && [ "$(cat .oak-built)" = "$c" ] || { \
      ...
      printf "%s" "$c" > .oak-built; }' >&2

# 3. (remote) start oak serve in the background, writing recipe JSON to a file; poll until it parses
recipe_json="$(vercel sandbox exec "$name" "${vercel_args[@]}" --timeout 60s \
  --env "OAK_PORT=$port" --env "OAK_PROJECT_ROOT=$project_root" --env "OAK_PAIRING_ADDRESS=$pairing_ws" \
  -- bash -lc 'set -euo pipefail; cd "$OAK_PROJECT_ROOT"; rm -f /tmp/oak-recipe.json /tmp/oak-serve.log; \
    nohup pnpm exec oak-dev serve --port "$OAK_PORT" --project-root "$OAK_PROJECT_ROOT" \
      --pairing-address "$OAK_PAIRING_ADDRESS" --recipe-json >/tmp/oak-recipe.json 2>/tmp/oak-serve.log </dev/null & \
    pid=$!; for _ in $(seq 1 80); do \
      node -e "JSON.parse(require(\"node:fs\").readFileSync(\"/tmp/oak-recipe.json\",\"utf8\"))" >/dev/null 2>&1 && { cat /tmp/oak-recipe.json; exit 0; }; \
      kill -0 "$pid" 2>/dev/null || { cat /tmp/oak-serve.log >&2; exit 1; }; sleep 0.25; \
    done; cat /tmp/oak-serve.log >&2; echo "serve recipe JSON timed out" >&2; exit 1')"
...
`userData.resourceId` from stdin (§7d). This is the **Oak-server** connection mode (the recipe emits a
...
- **`create` does NOT run `oak serve` and does NOT emit a `pairingCode`.** Oak itself connects to the
  host over its SSH relay, brings up the git + filesystem providers, and imports the repo. The script's
  only job is to make the host ready and **print SSH connection details** Oak will dial.
- The result uses a `connection` block with `type: "ssh"` and a `target`, **not** the flat
  `pairingCode`/`projectRoot` shape. Exact shape (Oak rejects anything else):
...
- `relayGracePeriodSeconds` (optional): how long Oak keeps the SSH relay alive after the workspace
...
# 1. ensure the repo is present and at the right commit on the host (NO oak serve here)
...
# 2. print the SSH connection block (NO pairingCode, NO oak serve). host/port/username tell Oak's
...
`destroy: none` and omit suspend/resume. (Oak still disconnects/reconnects its own SSH relay on
...
`connection.type:"ssh"` block above instead of starting `oak serve`.
...
- If committing from an interactive shell, force the runtime entrypoint back to `sshd`:
  `docker commit --change='ENTRYPOINT ["/usr/local/bin/oak-docker-ssh-entrypoint"]' …`.
...
docker run -d --name "$name" -p 127.0.0.1::22 -e "OAK_SSH_PUBLIC_KEY=$pubkey" "$auth_image"
...
require WSL/Git-Bash (and point `oak.yaml` at e.g. `bash ./scripts/oak-vm/<name>.sh` via a `.cmd`
...
# Oak-server mode: @{ schemaVersion=1; pairingCode=$pairingCode; projectRoot=$projectRoot; userData=@{...} }
...
environmentRecipes:
  - id: cloud-sandbox
    name: Cloud Sandbox
    create: ./scripts/oak-vm/cloud-sandbox-create.sh
    suspend: ./scripts/oak-vm/cloud-sandbox-suspend.sh
    resume: ./scripts/oak-vm/cloud-sandbox-resume.sh
    destroy: ./scripts/oak-vm/cloud-sandbox-destroy.sh
```
...
**Oak-server mode** — boot the env, start `oak serve` in it, and print serve's result:

```json
{
  "schemaVersion": 1,
  "pairingCode": "oak-pairing-code-or-url",
  "projectRoot": "/absolute/path/to/repo/on/remote",
  "userData": { "provider": "example", "resourceId": "provider-resource-id" }
}
```

Here `pairingCode` (from `oak serve --recipe-json`) and `projectRoot` are required; `schemaVersion` (`1`)
and `userData` are optional.

**SSH mode** — do **not** run `oak serve`; print the `connection.type:"ssh"` block instead (full shape +
...
Start Oak remotely with `oak serve --port "$PORT" --project-root "$ABS_ROOT" --pairing-address
...
`oak vm recipe doctor <recipe-id> --repo-path <repo> --json` validates **static wiring only** — it does
...
`oak vm recipe doctor <recipe-id> --repo-path <repo> --provision --json` actually runs the recipe end
...
- Don't make Oak own provider lifecycle beyond invoking the configured scripts.
- Don't commit or create an Oak workspace unless asked.
