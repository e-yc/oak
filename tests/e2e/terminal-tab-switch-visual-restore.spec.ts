...
test.describe('Terminal tab switch visual restore', () => {
  ...
  test('keeps full-width geometry after switching away and back', async ({ oakPage }, testInfo) => {
    ...
    const { firstTabId, secondTabId } = await ensureTwoTerminalTabs(oakPage)
    await forceWebglOnActiveTab(oakPage)

    const runId = `${Date.now()}`
    const marker = `${TAB_SWITCH_MARKER_PREFIX}_${runId}`
    const firstPtyId = await waitForPanePtyIdOnTab(oakPage, firstTabId)
    await writeStaticTabContent(oakPage, firstTabId, marker, TAB_A_GLYPH_ROW)

    const baseline = await readTabTerminalGeometry(oakPage, firstTabId, runId)
    ...
    await resetTerminalOutputSchedulerDebug(oakPage)
    await startHiddenPtyOutputBurst(oakPage, firstPtyId, runId)

    for (let cycle = 0; cycle < 12; cycle += 1) {
      await activateTerminalTab(oakPage, secondTabId)
      await injectHiddenStreamingBurst(oakPage, firstTabId, runId)
      // Why: rapid back-to-back switches mirror the user's leave/return pattern
      // and race the overlay's rAF/50ms refit retries.
      await activateTerminalTab(oakPage, firstTabId)
      if (cycle % 3 === 0) {
        await activateTerminalTab(oakPage, secondTabId)
        await activateTerminalTab(oakPage, firstTabId)
      }
      ...
      const immediate = await readTabTerminalGeometry(oakPage, firstTabId, runId)
      ...
      await oakPage.waitForTimeout(60)
      const settled = await readTabTerminalGeometry(oakPage, firstTabId, runId)
    ...
    const schedulerActivity = await waitForHiddenOutputSchedulerActivity(oakPage)
  ...
  test('keeps geometry after hidden alt-screen TUI redraws during tab switches', async ({
    ...
    for (let cycle = 0; cycle < 6; cycle += 1) {
      ...
      const geometry = await readTabTerminalGeometry(oakPage, firstTabId, `${runId}_ALT`)
  ...
  test('restores skipped hidden agent output on light tab resume', async ({ oakPage }) => {
    await waitForSessionReady(oakPage)
    await waitForActiveWorktree(oakPage)
    await ensureTerminalVisible(oakPage)
    await waitForActiveTerminalManager(oakPage, 30_000)

    const shellTabId = (await getActiveTabId(oakPage))!
    const agentTabId = await createCodexMarkedTerminalTab(oakPage)
    await waitForActiveTerminalManager(oakPage, 30_000)
    await waitForPanePtyIdOnTab(oakPage, agentTabId)
    const paneIdentity = await readPaneIdentityOnTab(oakPage, agentTabId)
    const paneKey = `${agentTabId}:${paneIdentity.leafId}`

    await activateTerminalTab(oakPage, shellTabId)
    const runId = `${Date.now()}`
    ...
    await resetHiddenOutputDebug(oakPage)
    await injectPaneData(oakPage, paneKey, hiddenFrame, {
      seq: hiddenFrame.length,
      rawLength: hiddenFrame.length
    })

    await expect
      .poll(async () => (await readHiddenOutputDebug(oakPage))?.hiddenRendererSkipCount ?? 0, {
        timeout: 5_000,
        message: 'Codex-marked hidden output did not take the skipped renderer path'
      })
      .toBeGreaterThan(0)
    await setHiddenSnapshotOverride(oakPage, paneIdentity.ptyId, {
    ...
    await activateTerminalTab(oakPage, agentTabId)

    await expect
      .poll(() => getTerminalContent(oakPage, 8_000), {
        timeout: 10_000,
  ...
  test('restores skipped hidden Grok output on light tab resume', async ({ oakPage }) => {
    await waitForSessionReady(oakPage)
    await waitForActiveWorktree(oakPage)
    await ensureTerminalVisible(oakPage)
    await waitForActiveTerminalManager(oakPage, 30_000)
    ...
    const shellTabId = (await getActiveTabId(oakPage))!
    const grokTabId = await createGrokMarkedTerminalTab(oakPage)
    await waitForActiveTerminalManager(oakPage, 30_000)
    await waitForPanePtyIdOnTab(oakPage, grokTabId)
    const paneIdentity = await readPaneIdentityOnTab(oakPage, grokTabId)
    ...
    await activateTerminalTab(oakPage, grokTabId)
  ...
  test('keeps returned tab glyphs intact across tab switches', async ({ oakPage }, testInfo) => {
    ...
    await forceWebglOnActiveTab(oakPage)
    await activateTerminalTab(oakPage, firstTabId)
    const firstWebgl = await waitForWebglOnTab(oakPage, firstTabId)
    await activateTerminalTab(oakPage, secondTabId)
    await oakPage.evaluate((id) => {
      window.__paneManagers?.get(id)?.setTerminalGpuAcceleration?.('on')
    }, secondTabId)
    const secondWebgl = await waitForWebglOnTab(oakPage, secondTabId)
    ...
    const firstPtyId = await waitForPanePtyIdOnTab(oakPage, firstTabId)
    const secondPtyId = await waitForPanePtyIdOnTab(oakPage, secondTabId)
    await sendToTerminal(oakPage, firstPtyId, SILENT_FOREGROUND_COMMAND)
    await sendToTerminal(oakPage, secondPtyId, SILENT_FOREGROUND_COMMAND)
    ...
    await writeStaticTabContent(oakPage, firstTabId, markerA, TAB_A_GLYPH_ROW)
    await activateTerminalTab(oakPage, secondTabId)
    await writeStaticTabContent(oakPage, secondTabId, markerB, TAB_B_GLYPH_ROW)

    await activateTerminalTab(oakPage, firstTabId)
    await resetAtlasOnTab(oakPage, firstTabId)
    await oakPage.waitForTimeout(800)
    const baseline = await captureStableTabScreenshot(oakPage, firstTabId)
    ...
    for (let cycle = 0; cycle < 8; cycle += 1) {
      ...
      const afterReturn = await captureStableTabScreenshot(oakPage, firstTabId)
