...
test.describe('Setup script import prompt', () => {
  ...
  test('imports Superset local overlays through the prompt UI', async ({ oakPage }, testInfo) => {
    const repoPath = createSupersetSetupRepo(testInfo.outputPath('superset-setup-repo'))
    const repoId = await addAndActivateRepo(oakPage, repoPath)
    ...
    await oakPage.getByRole('button', { name: 'Save local setup' }).click()

    await expect(
      oakPage.getByText('2 unsupported fields skipped. Saved the setup command.')
    ).toBeVisible()

    const localCommands = await openImportedSetupSettingsFromToast(oakPage, repoId)
  ...
  test('imports cmux setup commands through the prompt UI', async ({ oakPage }, testInfo) => {
    ...
    await expect(
      oakPage.getByRole('button', { name: "project's settings", exact: true })
    ).toBeVisible()

    const repoSettings = await openRepoSettings(oakPage, repoId)
