...
async function installPowerSaveBlockerProbe(electronApp: ElectronApplication): Promise<void> {
  await electronApp.evaluate(({ powerSaveBlocker }) => {
    ...
    if (root.__oakAwakePowerProbe) {
      root.__oakAwakePowerProbe.starts = []
      root.__oakAwakePowerProbe.stops = []
      return
    }
    ...
    powerSaveBlocker.start = ((type) => {
      ...
      root.__oakAwakePowerProbe?.starts.push({ type, id })
      return id
    }) as typeof powerSaveBlocker.start

    powerSaveBlocker.stop = ((id) => {
      root.__oakAwakePowerProbe?.stops.push({ id })
      originalStop(id)
...
async function postCodexHookEvent(
  ...
  const response = await fetch(`http://127.0.0.1:${endpoint.port}/hook/codex`, {
    ...
    headers: {
      ...
      'X-Oak-Agent-Hook-Token': endpoint.token
    },
    body: JSON.stringify({
...
test.describe('Agent awake setting', () => {
  ...
  test('can be toggled from Agents settings and persists through IPC', async ({ oakPage }) => {
    await openSettings(oakPage)
    await dismissTransientAnnouncement(oakPage)
    await oakPage.getByPlaceholder('Search settings').fill('awake')

    await expect(
      oakPage.getByText('Keep computer awake while agents are working').first()
    ).toBeVisible()

    const keepAwakeSwitch = oakPage.getByRole('switch', {
    ...
    await expect
      .poll(async () => (await getSettings(oakPage)).keepComputerAwakeWhileAgentsRun, {
        timeout: 5_000,
  ...
  test('keeps the OS awake only while a hook-reported agent is working', async ({
    ...
    await setKeepAwake(oakPage, true)
