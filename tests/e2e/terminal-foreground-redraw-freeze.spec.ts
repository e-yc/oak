...
test.describe('Terminal foreground redraw freeze repro', () => {
  test('Codex-style line rewrites request a visible row refresh', async ({ oakPage }) => {
    ...
    const { paneKey } = await waitForActivePaneHookDescriptor(oakPage)
    await waitForTerminalPtyDataInjector(oakPage, paneKey)
    await installActivePaneRefreshProbe(oakPage)
    try {
      const refreshBaseline = await readRefreshProbeCount(oakPage)
      await resetSchedulerDebug(oakPage)
      const measurement = await measureRendererDuringRewriteBurst(oakPage, paneKey)
      const scheduler = await readSchedulerDebug(oakPage)
      ...
      await expect
        .poll(async () => (await readRefreshProbeCount(oakPage)) - refreshBaseline, {
          timeout: 5_000,
          message: 'Codex-style terminal rewrites did not request an xterm refresh'
        })
        .toBeGreaterThan(0)
    } finally {
      await disposeActivePaneRefreshProbe(oakPage)
    }
  })
  ...
  test('active OpenTUI-style redraw bursts do not monopolize the renderer', async ({
    ...
    await waitForTerminalPtyDataInjector(oakPage, paneKey)
    await resetSchedulerDebug(oakPage)
    const measurement = await measureRendererDuringBurst(oakPage, paneKey)
    const scheduler = await readSchedulerDebug(oakPage)
  ...
  test('captured OpenCode/OpenTUI redraw bytes do not monopolize foreground writes', async ({
    ...
    await waitForTerminalPtyDataInjector(oakPage, paneKey)
    await resetSchedulerDebug(oakPage)
    const measurement = await measureRendererDuringFrames(oakPage, paneKey, frames)
    const scheduler = await readSchedulerDebug(oakPage)
    annotateMeasurement(testInfo, measurement, scheduler)
