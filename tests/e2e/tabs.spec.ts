...
test.describe('Tabs', () => {
   ...
   */
  test('clicking "+" then "New Terminal" creates a new terminal tab', async ({ oakPage }) => {
    const tabsBefore = await countRenderedTabs(oakPage)
    ...
    await oakPage.getByRole('button', { name: 'New tab' }).click({ force: true })
    // Why: the "+" dropdown uses Radix <DropdownMenuItem>, which exposes the
    // label text as the accessible name once the menu is open.
    const newTerminalMenuItem = oakPage.getByRole('menuitem', { name: /New Terminal/i }).first()
    ...
    await expect
      .poll(() => countRenderedTabs(oakPage), {
        timeout: 5_000,
    ...
    const storeActiveId = await getActiveTabId(oakPage)
    expect(storeActiveId).not.toBeNull()
    await expect(tabLocator(oakPage, storeActiveId!)).toBeVisible()
    await expect.poll(() => getDomActiveTabId(oakPage), { timeout: 3_000 }).toBe(storeActiveId)
    await expect
      .poll(() => getFocusedTerminalTabId(oakPage), {
        timeout: 5_000,
   ...
   * - New tab works
   */
  test('Cmd/Ctrl+T creates a new terminal tab', async ({ oakPage }) => {
    ...
    await oakPage.evaluate(() => document.body.focus())
    await oakPage.keyboard.press(`${mod}+t`)
   ...
   * right tab element.
   */
  test('Cmd/Ctrl+Shift+] and Cmd/Ctrl+Shift+[ switch between tabs', async ({ oakPage }) => {
    ...
    if ((await countRenderedTabs(oakPage)) < 2) {
      await oakPage.getByRole('button', { name: 'New tab' }).click()
      await oakPage
        .getByRole('menuitem', { name: /New Terminal/i })
        .first()
        .click()
      await expect
        .poll(() => countRenderedTabs(oakPage), { timeout: 5_000 })
        .toBeGreaterThanOrEqual(2)
    }

    const firstTabId = await getActiveTabId(oakPage)
    const orderedTabs = await getWorktreeTabs(oakPage, worktreeId)
    ...
    await expect.poll(() => getDomActiveTabId(oakPage), { timeout: 3_000 }).toBe(secondTabId)

    // Switch back.
    await oakPage.evaluate((tabId) => {
      window.__store?.getState().setActiveTab(tabId)
    }, firstTabId)
    await expect.poll(() => getDomActiveTabId(oakPage), { timeout: 3_000 }).toBe(firstTabId)
  })
   ...
   */
  test('dragging a tab to a new position reorders it', async ({ oakPage }) => {
    const worktreeId = (await getActiveWorktreeId(oakPage))!
    ...
    const domOrderBefore = await oakPage.$$eval(SORTABLE_TAB, (nodes) =>
      nodes.map((n) => (n as HTMLElement).dataset.tabId ?? '')
    )
    expect(domOrderBefore.length).toBeGreaterThanOrEqual(2)

    await oakPage.evaluate((targetWorktreeId) => {
    ...
    await expect
      .poll(
        async () =>
          oakPage.$$eval(SORTABLE_TAB, (nodes) =>
            nodes.map((n) => (n as HTMLElement).dataset.tabId ?? '')
          ),
  ...
  test('clicking tabs still switches after dragging a terminal tab to reorder', async ({
    ...
    await tabLocator(oakPage, firstTabId).click({ force: true })
    await expect.poll(() => getDomActiveTabId(oakPage), { timeout: 3_000 }).toBe(firstTabId)

    const firstTabBox = await tabLocator(oakPage, firstTabId).boundingBox()
    const secondTabBox = await tabLocator(oakPage, secondTabId).boundingBox()
    ...
    await oakPage.mouse.move(endX, endY, { steps: 8 })
    ...
    await tabLocator(oakPage, firstTabId).click({ force: true })
    await expect.poll(() => getDomActiveTabId(oakPage), { timeout: 3_000 }).toBe(firstTabId)
    await tabLocator(oakPage, secondTabId).click({ force: true })
    await expect
      .poll(() => getDomActiveTabId(oakPage), {
        timeout: 5_000,
   ...
   */
  test('Cmd/Ctrl+Shift+[ walks tabs in drag-reordered order', async ({ oakPage }) => {
    const isMac = process.platform === 'darwin'
    const mod = isMac ? 'Meta' : 'Control'
    const worktreeId = (await getActiveWorktreeId(oakPage))!
    ...
    const initialOrder = await getTabBarOrder(oakPage, worktreeId)
    ...
    await expect
      .poll(async () => getTabBarOrder(oakPage, worktreeId), { timeout: 3_000 })
      .toEqual([b, c, a])
    ...
    await expect.poll(() => getDomActiveTabId(oakPage), { timeout: 3_000 }).toBe(a)

    await oakPage.keyboard.press(`${mod}+Shift+BracketLeft`)
    await expect.poll(() => getDomActiveTabId(oakPage), { timeout: 3_000 }).toBe(c)
    await expect(tabLocator(oakPage, c)).toHaveAttribute('data-active', 'true')

    await oakPage.keyboard.press(`${mod}+Shift+BracketLeft`)
    await expect.poll(() => getDomActiveTabId(oakPage), { timeout: 3_000 }).toBe(b)
    await expect(tabLocator(oakPage, b)).toHaveAttribute('data-active', 'true')
  })
   ...
   * re-render.
   */
  test('closing a tab removes it from the tab bar', async ({ oakPage }) => {
    const worktreeId = (await getActiveWorktreeId(oakPage))!
    ...
    const tabsBefore = await countRenderedTabs(oakPage)
    const activeId = await getActiveTabId(oakPage)
    expect(activeId).not.toBeNull()
    const activeTab = tabLocator(oakPage, activeId!)
   ...
   */
  test('closing the active tab activates a neighbor tab', async ({ oakPage }) => {
    const worktreeId = (await getActiveWorktreeId(oakPage))!
    ...
    const activeTabBefore = await getActiveTabId(oakPage)
    expect(activeTabBefore).not.toBeNull()

    const activeTab = tabLocator(oakPage, activeTabBefore!)
    await activeTab.hover()
    await activeTab.getByRole('button', { name: /^Close tab /i }).click()
    ...
    await expect.poll(() => getDomActiveTabId(oakPage), { timeout: 5_000 }).not.toBeNull()
  })
})
