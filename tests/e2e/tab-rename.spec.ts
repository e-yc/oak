...
test.describe('Tab Rename (Inline)', () => {
  ...
  test('double-clicking a tab opens an inline rename input and Enter commits', async ({
    ...
    const originalTitle = await getActiveTabTitle(oakPage, worktreeId)
    expect(originalTitle.length).toBeGreaterThan(0)

    const tabLocator = tabLocatorByTitle(oakPage, originalTitle)
    await tabLocator.dblclick()

    const renameInput = oakPage.getByRole('textbox', {
    ...
    await expect
      .poll(async () => getActiveCustomTitle(oakPage, worktreeId), { timeout: 3_000 })
      .toBe('My Custom Title')
    await expect(renameInput).toBeHidden()
    await expect(tabLocatorByTitle(oakPage, 'My Custom Title')).toBeVisible()
  })

  test('context-menu Change Title opens a focused select-all rename input', async ({ oakPage }) => {
    const worktreeId = (await getActiveWorktreeId(oakPage))!
    const originalTitle = await getActiveTabTitle(oakPage, worktreeId)
    expect(originalTitle.length).toBeGreaterThan(0)

    await tabLocatorByTitle(oakPage, originalTitle).click({ button: 'right' })
    await oakPage.getByRole('menuitem', { name: 'Change Title', exact: true }).click()
    ...
    await expect(tabLocatorByTitle(oakPage, 'Context Menu Title')).toBeVisible()
  })

  test('Escape during inline rename discards the edit', async ({ oakPage }) => {
    const worktreeId = (await getActiveWorktreeId(oakPage))!
    const originalTitle = await getActiveTabTitle(oakPage, worktreeId)
    ...
    await expect(tabLocatorByTitle(oakPage, originalTitle)).toBeVisible()
    await expect
      .poll(async () => getActiveCustomTitle(oakPage, worktreeId), { timeout: 3_000 })
      .toBe(null)
  })

  test('renaming to an empty string resets the tab to its default title', async ({ oakPage }) => {
    const worktreeId = (await getActiveWorktreeId(oakPage))!
    ...
    const defaultTitle = await getActiveTabTitle(oakPage, worktreeId)
    ...
    const tabLocator = tabLocatorByTitle(oakPage, 'Seeded Custom')
    await tabLocator.dblclick()
    ...
    await expect(tabLocatorByTitle(oakPage, defaultTitle)).toBeVisible()
    await expect
      .poll(async () => getActiveCustomTitle(oakPage, worktreeId), { timeout: 3_000 })
      .toBe(null)
  })

  test('clicking away (blur) commits the rename', async ({ oakPage }) => {
    const worktreeId = (await getActiveWorktreeId(oakPage))!
    ...
    await expect
      .poll(async () => (await getWorktreeTabs(oakPage, worktreeId)).length, { timeout: 3_000 })
      .toBeGreaterThanOrEqual(2)

    const tabs = await getWorktreeTabs(oakPage, worktreeId)
    const activeId = await getActiveTabId(oakPage)
    const activeTab = tabs.find((t) => t.id === activeId)!
    const otherTab = tabs.find((t) => t.id !== activeId)!

    const tabLocator = tabLocatorByTitle(oakPage, activeTab.title!)
    await tabLocator.dblclick()
    ...
    await tabLocatorByTitle(oakPage, otherTab.title!).click()

    await expect(renameInput).toBeHidden()
    await expect(tabLocatorByTitle(oakPage, 'Committed By Blur')).toBeVisible()
    expect(
      await oakPage.evaluate(
  ...
  test('rename input stays at a usable width when many tabs are open', async ({ oakPage }) => {
    const worktreeId = (await getActiveWorktreeId(oakPage))!
    const targetTabId = await getActiveTabId(oakPage)
    ...
    const tabLocator = tabLocatorByTitle(oakPage, targetTitle)
  ...
  test('middle-clicking inside the rename input does not close the tab', async ({ oakPage }) => {
    const worktreeId = (await getActiveWorktreeId(oakPage))!
    const tabsBefore = (await getWorktreeTabs(oakPage, worktreeId)).length
    const originalTitle = await getActiveTabTitle(oakPage, worktreeId)
    ...
    await expect(renameInput).toBeVisible()
    await expect(tabLocatorByTitle(oakPage, originalTitle)).toBeVisible()
    expect((await getWorktreeTabs(oakPage, worktreeId)).length).toBe(tabsBefore)
  })
})
