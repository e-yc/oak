...
test.describe('Codex terminal cursor jitter repro', () => {
  test('keeps queued-message cursor out of the Working status row in native Windows Codex @headful', async ({
    ...
    await installPtyWriteDiagnostics(oakPage)
    rmSync(ARTIFACT_DIR, { recursive: true, force: true })
    mkdirSync(ARTIFACT_DIR, { recursive: true })

    const shellCase = SHELL_CASES[0]!
    const { tabId, ptyId } = await prepareCodexTerminal(oakPage, shellCase)
    await installPtyOutputDiagnostics(oakPage)
    await oakPage.keyboard.type(CODEX_REPO_PROMPT)
    await oakPage.waitForTimeout(250)
    await oakPage.keyboard.press('Enter')
    await oakPage.waitForTimeout(1_000)
    if (!CODEX_WORKING_STATUS_RE.test(await getTerminalContentForTab(oakPage, tabId, 8_000))) {
      await oakPage.keyboard.press('Enter')
    }
    await oakPage.waitForTimeout(3_000)
    const submittedContent = await getTerminalContentForTab(oakPage, tabId, 8_000)
    writeFileSync(path.join(ARTIFACT_DIR, 'queued-message-after-submit.txt'), submittedContent)
    await expect
      .poll(
        async () =>
          CODEX_WORKING_STATUS_RE.test(await getTerminalContentForTab(oakPage, tabId, 8_000)),
        {
          timeout: 30_000,
      ...
      .toBe(true)
    await applyCursorProbeTheme(oakPage, tabId)
    ...
    await oakPage.keyboard.insertText('s')
    await expect
      .poll(
        async () =>
          (await readScreenLines(oakPage, tabId)).some((line) => isQueuedInputLine(line.text)),
        {
          timeout: 5_000,
    ...
    const snapshot = await readScreenSnapshot(oakPage, shellCase.label, tabId, ptyId)
    const rawChunks = await readPtyOutputDiagnostics(oakPage)
