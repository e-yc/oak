...
const REPO_STEP_HEADING = /Point Oak at some code/i
...
test.describe('Existing-user golden core flow', () => {
  test('adds project, creates workspace, opens a terminal tab, and splits a pane', async ({
    ...
    const repoPath = await createGitRepo('oak-e2e-golden-existing-', 'golden-existing-project')

    await addProjectFromSidebar(oakPage, electronApp, repoPath)
    const workspaceName = `golden-existing-${Date.now()}`
    await createWorkspace(oakPage, workspaceName)
    await expectActiveWorkspaceBelongsToRepo(oakPage, workspaceName, repoPath)
    await ensureTerminalVisible(oakPage)
    await expectTerminalSurface(oakPage)
    await waitForTerminalPaneManager(oakPage)

    await createTerminalTabThroughMenu(oakPage)
    await splitTerminalPaneAndAssertIdentity(oakPage)
  })
})
...
test.describe('New-user golden core flow', () => {
  ...
  test('completes onboarding, adds a project, and follows the workspace tour handoff', async ({
    ...
    await selectCodexAgent(oakPage)
    await continueOnboarding(oakPage)
    await expect(oakPage.getByRole('heading', { name: /Make it feel like home/i })).toBeVisible()
    await chooseOppositeTheme(oakPage)
    await continueOnboarding(oakPage)
    await continueThroughOptionalSetupToNotifications(oakPage)
    await expect(oakPage.getByRole('button', { name: /Send Test Notification/i })).toBeVisible()
    await chooseNotificationSound(oakPage)
    await continueFromNotificationsToRepo(oakPage)

    const repoPath = await createGitRepo('oak-e2e-golden-new-', 'golden-new-project')
    await chooseFolderInNativeDialog(electronApp, repoPath)
    await oakPage
      .getByRole('button', { name: /Browse for a folder|Open a folder|Browse folder/i })
      .click()
    await expect(oakPage.getByRole('heading', { name: REPO_STEP_HEADING })).toHaveCount(0, {
      timeout: 30_000
    })
    await waitForRepoLoaded(oakPage, repoPath)
    await expectProjectVisible(oakPage, repoPath)
    await waitForActiveWorktree(oakPage)
    await ensureTerminalVisible(oakPage)
    await expectTerminalSurface(oakPage)
    await waitForTerminalPaneManager(oakPage)

    await requestAgentSessionsTour(oakPage)
    const paneCountBeforeTourSplit = await countVisibleTerminalPanes(oakPage)
    await oakPage.getByRole('button', { name: /^Split terminal$/ }).click()
    await waitForPaneCount(oakPage, paneCountBeforeTourSplit + 1)
    await waitForPaneIdentitySnapshot(oakPage, paneCountBeforeTourSplit + 1)

    await expect(
      oakPage.getByRole('dialog', { name: /Start another task in parallel/i })
    ).toBeVisible()
    const createControl = oakPage
    ...
    await completeWorkspaceCreationTour(oakPage, workspaceName)
    await expectActiveWorkspaceBelongsToRepo(oakPage, workspaceName, repoPath)
  })
})
