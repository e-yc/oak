...
async function createShallowPriorityTruncationFixture(): Promise<{
  ...
  const parentPath = realpathSync(
    await mkdtemp(path.join(os.tmpdir(), 'oak-e2e-shallow-priority-'))
  )
...
async function createCancellableScanFixture(): Promise<{
  ...
  const parentPath = realpathSync(
    await mkdtemp(path.join(os.tmpdir(), 'oak-e2e-cancellable-scan-'))
  )
  tempRoots.push(parentPath)
...
test('prioritizes shallow sibling repositories in a bounded nested scan', async ({
  ...
  await oakPage
    ...
    .first()
    .click()
  const dialog = oakPage.getByRole('dialog', { name: /Add a project/i })
  await expect(dialog).toBeVisible()
  await dialog.getByRole('button', { name: /Browse folder/i }).click()

  const importDialog = oakPage.getByRole('dialog', {
  ...
  await expect(oakPage.getByText(fixture.groupName)).toBeVisible()
  await expect(oakPage.getByText('z-web-client').first()).toBeVisible()
})
