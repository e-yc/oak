...
test.describe('Source Control AI PR generation worktree switching', () => {
  ...
  test('keeps checks-panel PR generation running after switching worktrees', async ({
    ...
    const { primaryWorktreeId, prWorktreeId, prWorktreePath, primaryBranch } =
      await seedCreatePrComposer(oakPage)
    ...
    await installDelayedPrGenerator(oakPage, generatorScriptPath, callLogPath, primaryBranch)

    await openChecks(oakPage, prWorktreeId)
    const generate = oakPage.getByRole('button', {
    ...
    await expect(
      oakPage.getByRole('button', { name: 'Stop generating pull request details' })
    ).toBeVisible()
    await expect.poll(() => readLog(callLogPath)).toContain('start')
    await oakPage.screenshot({
      path: path.join(screenshotDir, '01-checks-pr-generation-pending-on-a.png')
    })

    await openChecks(oakPage, primaryWorktreeId)
    await expect(oakPage.getByText('Generated PR title after switch')).toHaveCount(0)
    await oakPage.screenshot({
    ...
    await openChecks(oakPage, prWorktreeId)
    await expect(oakPage.getByRole('textbox', { name: 'Pull request title' })).toHaveValue(
      'Generated PR title after switch',
      { timeout: 10_000 }
    )
    await expect(oakPage.getByRole('textbox', { name: 'Pull request description' })).toHaveValue(
  ...
  test('keeps pending PR generation attached to its original worktree', async ({
    ...
    await openSourceControl(oakPage, prWorktreeId)
    const generate = oakPage.getByRole('button', {
      name: 'Generate pull request details with AI'
    })
    await expect(generate).toBeVisible({ timeout: 10_000 })
    ...
    await expect
      ...
      .toContain('start')
    const pendingEvidence = await oakPage.evaluate(() => {
    ...
    await openSourceControl(oakPage, primaryWorktreeId)
    await expect(oakPage.getByText('Generated PR title after switch')).toHaveCount(0)
    const switchedEvidence = await oakPage.evaluate(() => {
      const state = window.__store?.getState()
      return {
        activeWorktreeId: state?.activeWorktreeId,
    ...
    await expect
      ...
      .toContain('finish')
    await waitForPrGenerationStored(oakPage, prWorktreeId)
    await openSourceControl(oakPage, prWorktreeId)
    await waitForPrGenerationHydrated(oakPage, prWorktreeId)
    await expect(oakPage.getByRole('textbox', { name: 'Pull request title' })).toHaveValue(
      'Generated PR title after switch',
      { timeout: 10_000 }
    )
    ...
    const finalEvidence = await oakPage.evaluate(() => {
      const state = window.__store?.getState()
      return {
        activeWorktreeId: state?.activeWorktreeId,
  ...
  test('keeps Create PR intent running after switching worktrees', async ({
        ...
        store.setState((current) => ({
          ...
          createHostedReview: async (repoPath, input) => {
            ...
            return {
              ok: true as const,
              number: 74,
              url: 'https://github.com/acme/oak/pull/74'
            }
          },
          gitStatusByWorktree: {
    ...
    await openSourceControl(oakPage, prWorktreeId)
    const createPr = oakPage.getByRole('button', { name: 'Create PR' }).first()
    ...
    const completedWhileSwitchedEvidence = await oakPage.evaluate(() => {
      const state = window.__store?.getState()
      return {
        activeWorktreeId: state?.activeWorktreeId,
        rightSidebarTab: state?.rightSidebarTab
    ...
    await openSourceControl(oakPage, prWorktreeId)
    const payloads = await oakPage.evaluate(
      () =>
        (
  ...
  test('hydrates pending PR generation after Source Control remounts', async ({
    ...
    await waitForSessionReady(oakPage)
    await waitForActiveWorktree(oakPage)
    const { prWorktreeId, prWorktreePath, primaryBranch } = await seedCreatePrComposer(oakPage)
    createBranchCommit(prWorktreePath)
  ...
  test('keeps pending commit message generation attached to its original worktree', async ({
    ...
    const { primaryWorktreeId, commitWorktreeId, commitWorktreePath } =
      await seedCommitMessageComposer(oakPage)
    ...
    await installDelayedCommitMessageGenerator(oakPage, generatorScriptPath, callLogPath)

    await openSourceControl(oakPage, commitWorktreeId)
    await expect(oakPage.getByText('e2e-commit-message-generation.txt')).toBeVisible({
      timeout: 10_000
    })
    const generate = oakPage.getByRole('button', {
    ...
    await generate.click()
    await expect(
      oakPage.getByRole('button', { name: 'Stop generating commit message' })
    ).toBeVisible()
    await expect
      .poll(() => {
        return readLog(callLogPath)
    ...
    await openSourceControl(oakPage, primaryWorktreeId)
    await expect(oakPage.getByText('Generated commit message after switch')).toHaveCount(0)
    await expect(
      oakPage.getByRole('button', { name: 'Stop generating commit message' })
    ).toHaveCount(0)
    const switchedEvidence = await oakPage.evaluate(() => {
    ...
    await expect
      .poll(() => readFileSync(callLogPath, 'utf8'), { timeout: 10_000 })
      .toContain('finish')
    await waitForCommitGenerationStored(oakPage, commitWorktreeId)
    await openSourceControl(oakPage, commitWorktreeId)
    await waitForCommitGenerationHydrated(oakPage, commitWorktreeId)
    await expect(oakPage.getByRole('textbox', { name: 'Commit message' })).toHaveValue(
      'Generated commit message after switch\n\nGenerated from staged e2e-commit-message-generation.txt after switching worktrees',
      { timeout: 10_000 }
    )
    const finalEvidence = await oakPage.evaluate(() => {
  ...
  test('hydrates pending commit message generation after Source Control remounts', async ({
    ...
    await waitForSessionReady(oakPage)
    await waitForActiveWorktree(oakPage)
    const { commitWorktreeId, commitWorktreePath } = await seedCommitMessageComposer(oakPage)
    createStagedCommitMessageChange(commitWorktreePath)
  ...
  test('hides the commit AI composer on a clean branch empty state', async ({
    ...
    await waitForSessionReady(oakPage)
    await waitForActiveWorktree(oakPage)
    const primaryWorktreeId = await seedCleanBranchEmptyState(oakPage)
    ...
    await expect
      .poll(
        async () => {
          ...
          await seedCleanBranchEmptyState(oakPage, primaryWorktreeId)
          return oakPage.evaluate(() => {
