...
const EMOJI_TABLE_MARKER = 'OAK_EMOJI_TABLE_RENDER_DONE'
...
test.describe('OpenCode emoji table terminal rendering', () => {
  test('keeps emoji table output visually sane and restores the cursor', async ({
    ...
    await waitForPtyShellEcho(oakPage, ptyId, 20_000)
    const runId = randomUUID()
    const marker = `${EMOJI_TABLE_MARKER}_${runId}`
    const scriptPath = path.join(testRepoPath, `.oak-opencode-emoji-table-${runId}.mjs`)
    writeFileSync(scriptPath, emojiTableScript(marker))
    try {
      await sendToTerminal(oakPage, ptyId, `${nodeTerminalCommand([scriptPath])}\r`)
      await waitForTerminalOutput(oakPage, marker, 10_000)
      await oakPage.waitForTimeout(250)
      await forceCursorProbeTheme(oakPage)
      await oakPage.waitForTimeout(50)

      const renderState = await readActiveTerminalRenderState(oakPage)
      const blinkSamples = await sampleCursorBlink(oakPage)
  ...
  test('local real OpenCode demo keeps table rendering and cursor visible', async ({
    ...
    test.skip(
      process.env.OAK_E2E_REAL_OPENCODE !== '1',
      'Set OAK_E2E_REAL_OPENCODE=1 to exercise the locally installed OpenCode TUI'
    )
    ...
    try {
      await waitForTerminalOutput(oakPage, 'Give me markdown table', 15_000)
      await waitForTerminalOutput(oakPage, 'Emoji', 60_000)
      await waitForTerminalOutput(oakPage, 'Alice', 60_000)
      await oakPage.waitForTimeout(1_500)

      await testInfo.attach('real-opencode-demo-table', {
        body: await oakPage.screenshot({ fullPage: true }),
