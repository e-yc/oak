...
test.describe('Droid notifications', () => {
  test('Codex hook completion dispatches while its worktree is inactive', async ({
    ...
    await sendToTerminal(oakPage, ptyId, `printf '${readyMarker}\\n'\r`)
    await waitForTerminalOutput(oakPage, readyMarker)

    const { paneKey, worktreeId } = await waitForActivePaneHookDescriptor(oakPage)
    ...
    await expect
      .poll(
        async () =>
          (await getRendererOrCachedAgentStatuses(oakPage)).some(
            (status) =>
    ...
    await switchToOtherExistingWorktree(oakPage)
    ...
    await expect
      .poll(
        async () =>
          (await getAgentStatuses(oakPage)).some(
            (status) =>
    ...
    await expect
      .poll(async () => isWorktreeUnread(oakPage, worktreeId), {
        timeout: 10_000,
  ...
  test('recognized agent title completion dispatches one task-complete notification', async ({
    ...
    await sendToTerminal(oakPage, ptyId, `printf '${marker}\\n'\r`)
    await waitForTerminalOutput(oakPage, marker)

    await emitOscTitle(oakPage, ptyId, 'Codex working')
    await expect
      .poll(async () => (await getRendererTitleLog(oakPage)).includes('Codex working'), {
        timeout: 10_000,
        message: 'Codex working title did not reach the renderer before completion'
      })
      .toBe(true)

    await emitOscTitle(oakPage, ptyId, 'Codex done')
    await expect
      .poll(async () => (await getRendererTitleLog(oakPage)).includes('Codex done'), {
        timeout: 10_000,
  ...
  test('Factory Droid needs-input native title does not dispatch a task-complete notification', async ({
    ...
    await emitOscTitle(oakPage, ptyId, '⠋ Droid')
    await emitOscTitle(oakPage, ptyId, 'Factory Droid needs input')

    await expect
      .poll(
        async () => (await getRendererTitleLog(oakPage)).includes('Factory Droid needs input'),
        {
          timeout: 10_000,
