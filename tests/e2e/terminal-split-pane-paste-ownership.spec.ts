...
test.describe('split terminal pane paste ownership', () => {
  test('keyboard paste writes only to the active split pane PTY', async ({
    ...
    const scriptPath = path.join(testRepoPath, `.oak-split-paste-${runId}.mjs`)
    writeFileSync(scriptPath, pasteEchoScript(runId))
    let scriptStarted = false

    try {
      await sendToTerminal(oakPage, activePane.ptyId, `node ${JSON.stringify(scriptPath)}\r`)
      scriptStarted = true
      await waitForTerminalOutput(oakPage, `SPLIT_PASTE_READY_${runId}`, 10_000)

      const payload = `OAK_E2E_SPLIT_PASTE_${runId}`
      ...
      await oakPage.keyboard.press(keyboardPasteChord())
      ...
      if (scriptStarted) {
        await sendToTerminal(oakPage, activePane.ptyId, '\x03').catch(() => undefined)
      }
  ...
  test('internal file drop writes only to the pane under the drop target', async ({
    ...
    await oakPage.evaluate(
      ({ leafId, pathValue }) => {
        ...
        dataTransfer.setData('text/x-oak-file-path', pathValue)
