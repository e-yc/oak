...
test.describe('Browser Tab', () => {
   ...
   * - Browser works and also retains state when switching tabs etc.
   */
  test('creating a browser tab adds it and activates browser view', async ({ oakPage }) => {
    const worktreeId = (await getActiveWorktreeId(oakPage))!
    const browserTabsBefore = await getBrowserTabs(oakPage, worktreeId)

    await createBrowserTab(oakPage, worktreeId)

    // Wait for the browser tab to appear in the store
    await expect
      .poll(async () => (await getBrowserTabs(oakPage, worktreeId)).length, { timeout: 5_000 })
      .toBe(browserTabsBefore.length + 1)

    // The active tab type should switch to 'browser'
    await expect.poll(async () => getActiveTabType(oakPage), { timeout: 3_000 }).toBe('browser')
  })
   ...
   * - Browser works and also retains state when switching tabs etc.
   */
  test('browser tab is created and active in the store', async ({ oakPage }) => {
    const worktreeId = (await getActiveWorktreeId(oakPage))!

    await createBrowserTab(oakPage, worktreeId)
    await expect.poll(async () => getActiveTabType(oakPage), { timeout: 5_000 }).toBe('browser')

    // Verify the browser tab exists in the store
    const browserTabs = await getBrowserTabs(oakPage, worktreeId)
    ...
    const activeBrowserTabId = await oakPage.evaluate(() => {
      const store = window.__store
   ...
   * - Browser works and also retains state when switching tabs etc.
   */
  test('browser tab retains state when switching to terminal and back', async ({ oakPage }) => {
    const worktreeId = (await getActiveWorktreeId(oakPage))!
    ...
    await switchToTerminalTab(oakPage, worktreeId)
    await expect.poll(async () => getActiveTabType(oakPage), { timeout: 3_000 }).toBe('terminal')

    // Switch back to browser tab
    await switchToBrowserTab(oakPage, worktreeId, browserTabId!)
    await expect.poll(async () => getActiveTabType(oakPage), { timeout: 3_000 }).toBe('browser')

    // The browser tab should still exist with the same ID
    const browserTabsAfter = await getBrowserTabs(oakPage, worktreeId)
  ...
  test('browser webview form state survives switching between browser tabs', async ({
    ...
    try {
      ...
      await writeBrowserInputValue(oakPage, firstTab!.id, 'first typed value')
      ...
      await writeBrowserInputValue(oakPage, secondTab!.id, 'second typed value')
      ...
      await switchToBrowserTab(oakPage, worktreeId, firstTab!.id)
      await expect
        .poll(async () => readBrowserInputValue(oakPage, firstTab!.id), { timeout: 5_000 })
        .toBe('first typed value')

      await switchToBrowserTab(oakPage, worktreeId, secondTab!.id)
      await expect
        .poll(async () => readBrowserInputValue(oakPage, secondTab!.id), { timeout: 5_000 })
   ...
   * - Browser works and also retains state when switching tabs etc.
   */
  test('browser tab retains state when switching worktrees and back', async ({ oakPage }) => {
