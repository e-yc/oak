...
test('floating Mobile Emulator tab renders content and closes from the tab strip', async ({
  ...
  const tab = await seedFloatingSimulatorTab(oakPage)
  await openFloatingPanelIfNeeded(oakPage)

  const openPanel = oakPage.locator(OPEN_PANEL_SELECTOR).first()
  ...
  await attachPanelScreenshot(oakPage, testInfo)
