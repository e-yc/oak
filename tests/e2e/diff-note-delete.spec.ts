...
test.describe('Diff note delete', () => {
  ...
  test('clicking the trash button removes the saved note', async ({ oakPage }) => {
    ...
    const deleteButton = oakPage.getByTitle('Delete note').first()
    ...
    await expect(oakPage.getByTitle('Delete note')).toHaveCount(0, {
      timeout: 5_000
    })
  })
})
