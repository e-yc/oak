...
async function seedCreatePREligibleBranch(
  ...
  return page.evaluate(async ({ createResult }) => {
    ...
    const pr = {
      ...
      state: 'open' as const,
      url: 'https://github.com/acme/oak/pull/73',
...
test.describe('Source Control create pull request', () => {
  ...
  test('creates the pull request from the Source Control primary action', async ({ oakPage }) => {
    const { branch, worktreeId } = await seedCreatePREligibleBranch(oakPage)
    await openSourceControl(oakPage, worktreeId)
    await forceCreatePREligibleStatus(oakPage, worktreeId, branch)

    const titleInput = oakPage.getByRole('textbox', { name: 'Pull request title' })
    const descriptionInput = oakPage.getByRole('textbox', {
      name: 'Pull request description'
    })
    const createButton = getCreatePRComposerSubmitButton(oakPage)
    await expect(createButton).toBeVisible({ timeout: 10_000 })
    await expect(createButton).toBeEnabled()
    await expect(titleInput).toHaveValue('E2e secondary')
    await expect(oakPage.getByRole('textbox', { name: 'Pull request base branch' })).toHaveValue(
      'main'
    )
  ...
  test('surfaces create failures without clearing the pull request composer', async ({
    ...
    await expect(oakPage.getByText(failureMessage)).toBeVisible()
