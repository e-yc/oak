...
test.describe('Workspace Back/Forward Navigation', () => {
  ...
  test('buttons are hidden outside the terminal view', async ({ oakPage }) => {
    await expect(await getBackButton(oakPage)).toBeVisible()
    await expect(await getForwardButton(oakPage)).toBeVisible()
    ...
    await expect(await getBackButton(oakPage)).toHaveCount(0)
    await expect(await getForwardButton(oakPage)).toHaveCount(0)
  ...
  test('both buttons disabled at cold start with a single history entry', async ({ oakPage }) => {
    ...
    const activeId = await getActiveWorktreeId(oakPage)
    expect(activeId).not.toBeNull()

    await resetNavHistory(oakPage)
    await seedVisit(oakPage, activeId!)

    const back = await getBackButton(oakPage)
    const forward = await getForwardButton(oakPage)
    await expect(back).toBeDisabled()
    await expect(forward).toBeDisabled()
  })

  test('clicking Back and Forward walks the history stack', async ({ oakPage }) => {
    const worktreeIds = await getAllWorktreeIds(oakPage)
    ...
    await resetNavHistory(oakPage)
    await seedVisit(oakPage, primaryId)
    await seedVisit(oakPage, secondaryId)
    ...
    const primaryRow = worktreeRow(oakPage, primaryId)
    const secondaryRow = worktreeRow(oakPage, secondaryId)

    await back.click()
    await expect
      .poll(async () => getActiveWorktreeId(oakPage), {
  ...
  test('re-activating the current worktree is a no-op (dedupe)', async ({ oakPage }) => {
    const activeId = await getActiveWorktreeId(oakPage)
    expect(activeId).not.toBeNull()
    ...
    const snapshot = await getNavHistorySnapshot(oakPage)
    expect(snapshot.history).toEqual([activeId])
    expect(snapshot.index).toBe(0)
    await expect(await getBackButton(oakPage)).toBeDisabled()
  })

  test('new navigation after going back truncates the forward stack', async ({ oakPage }) => {
    const worktreeIds = await getAllWorktreeIds(oakPage)
    ...
    await seedVisit(oakPage, primaryId)
    await seedVisit(oakPage, secondaryId)
    await (await getBackButton(oakPage)).click()
    await expect.poll(() => getActiveWorktreeId(oakPage)).toBe(primaryId)

    // Forward button is live — a forward entry exists.
    await expect(await getForwardButton(oakPage)).toBeEnabled()
    ...
    await expect(await getForwardButton(oakPage)).toBeDisabled()
  })

  test(`${isMac ? 'Cmd' : 'Ctrl'}+Alt+Left/Right shortcuts walk history`, async ({ oakPage }) => {
    const worktreeIds = await getAllWorktreeIds(oakPage)
    ...
    await oakPage.keyboard.press(`${mod}+Alt+ArrowLeft`)
    await expect
      .poll(async () => getActiveWorktreeId(oakPage), {
    ...
    await oakPage.keyboard.press(`${mod}+Alt+ArrowRight`)
    await expect
      .poll(async () => getActiveWorktreeId(oakPage), {
  ...
  test('shortcut is a no-op in settings view', async ({ oakPage }) => {
    const worktreeIds = await getAllWorktreeIds(oakPage)
    ...
    await expect
      .poll(async () => oakPage.evaluate(() => window.__store!.getState().activeView))
      .toBe('settings')

    const idBefore = await getActiveWorktreeId(oakPage)
    await oakPage.evaluate(() => document.body.focus())
    await oakPage.keyboard.press(`${mod}+Alt+ArrowLeft`)

    // Give any erroneous nav a beat to land, then assert the active worktree
    // and the slice index both stayed put.
    await oakPage.waitForTimeout(150)
    expect(await getActiveWorktreeId(oakPage)).toBe(idBefore)
    const snapshot = await getNavHistorySnapshot(oakPage)
    expect(snapshot.index).toBe(1)
  })
})
