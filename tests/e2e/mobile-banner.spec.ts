...
test('mobile subscribe mounts overlay; collapse → chip; Take back dismisses', async ({
  ...
  const overlay = oakPage.locator('.mobile-driver-banner')
  await expect(overlay).toHaveCount(0)

  // Fire the IPC events main emits when a mobile client subscribes in 'auto'
  // mode (handleMobileSubscribe in src/main/runtime/oak-runtime.ts). The
  ...
  await expectExpandedOverlayLeavesPaneReadable(oakPage, ptyId)
  ...
  await captureAttachment(oakPage, testInfo, 'overlay-loud.png')
  ...
  await expectChipIsCompactInPane(oakPage, ptyId)

  await captureAttachment(oakPage, testInfo, 'overlay-collapsed.png')
...
test('held phone-fit state mounts restore overlay without collapse', async ({
  ...
  await captureAttachment(oakPage, testInfo, 'overlay-held-fit.png')
...
test('restore this terminal refits the active restored pane', async ({ oakPage, electronApp }) => {
  await waitForSessionReady(oakPage)
  await waitForActiveWorktree(oakPage)
  await ensureTerminalVisible(oakPage)
  await waitForActiveTerminalManager(oakPage)
  ...
  await expect(oakPage.locator('.mobile-driver-banner')).toHaveCount(1, { timeout: 15_000 })
  await expect
    .poll(() => getPaneTerminalCols(oakPage, ptyId), {
...
test('restore all refits non-focused restored terminal panes', async ({ oakPage, electronApp }) => {
  await waitForSessionReady(oakPage)
  await waitForActiveWorktree(oakPage)
  await ensureTerminalVisible(oakPage)
  await waitForActiveTerminalManager(oakPage)
  await splitActiveTerminalPane(oakPage, 'vertical')
  const ptyIds = await waitForVisiblePanePtyIds(oakPage, 2)
  const focusPtyId = await waitForActivePanePtyId(oakPage)
  ...
  await expect(oakPage.locator('.mobile-driver-banner')).toHaveCount(2, { timeout: 15_000 })

  await forcePaneToOneColumn(oakPage, inactivePtyId)
  await expect
    .poll(() => getPaneTerminalCols(oakPage, inactivePtyId), {
...
test('restore all recovers a hidden workspace held at narrow terminal geometry', async ({
  ...
  const secondWorktreeId = (await getAllWorktreeIds(oakPage)).find(
  ...
  await ensureTerminalVisible(oakPage)
  await waitForActiveTerminalManager(oakPage)
  const hiddenWorkspacePtyId = await waitForActivePanePtyId(oakPage)
  await sendHeldPhoneFitIpc(electronApp, { ptyId: hiddenWorkspacePtyId, cols: 45, rows: 20 })
  await expect(oakPage.locator('.mobile-driver-banner')).toHaveCount(1, { timeout: 15_000 })

  await forcePaneToOneColumnAndSwitchWorktree(oakPage, hiddenWorkspacePtyId, secondWorktreeId)
  await expect
  ...
  await expect
    .poll(() => getPaneTerminalCols(oakPage, hiddenWorkspacePtyId), {
      message: 'test harness should hold workspace 1 in the bad narrow state'
    })
    .toBeLessThanOrEqual(2)
  await ensureTerminalVisible(oakPage)
  await waitForActiveTerminalManager(oakPage)
  const activeWorkspacePtyId = await waitForActivePanePtyId(oakPage)
  await installRestoreTerminalFitAutoRestoreRecorder(electronApp)
  await sendHeldPhoneFitIpc(electronApp, { ptyId: activeWorkspacePtyId, cols: 45, rows: 20 })
  await expect(
    oakPage.locator(`[data-pty-id="${activeWorkspacePtyId}"] .mobile-driver-banner`)
