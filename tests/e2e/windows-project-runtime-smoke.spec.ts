...
test.describe('Windows project runtime smoke', () => {
  test('keeps a Windows-host project and WSL project available side by side', async ({
    ...
    const wsl = await oakPage.evaluate(async () => ({
    ...
    const wslRepoPath = await createGitRepo('oak-e2e-project-runtime-', 'wsl-runtime-project')

    const smoke = await oakPage.evaluate(
    ...
    await openRepoSettings(oakPage, smoke.hostRepoId)
    const hostSection = oakPage.locator(`[data-settings-section="repo-${smoke.hostRepoId}"]`)
    ...
    await chooseProjectRuntime(oakPage, smoke.hostRepoId, 'WSL')
    await expect(
      hostSection.getByText(`This project runs in ${smoke.wslDistro} via WSL.`)
    ).toBeVisible()
    const hostAfterWslUiSwitch = await oakPage.evaluate((hostRepoId) => {
      const state = window.__store!.getState()
      const hostProject = state.projects.find((project) =>
    ...
    await chooseProjectRuntime(oakPage, smoke.hostRepoId, 'Windows')
    await expect(hostSection.getByText('This project runs on Windows.')).toBeVisible()
    const hostAfterWindowsUiSwitch = await oakPage.evaluate((hostRepoId) => {
      const state = window.__store!.getState()
      const hostProject = state.projects.find((project) =>
        project.sourceRepoIds.includes(hostRepoId)
      )
    ...
    await openRepoSettings(oakPage, smoke.wslRepoId)
    const wslSection = oakPage.locator(`[data-settings-section="repo-${smoke.wslRepoId}"]`)
    ...
    await oakPage.evaluate(async (repoId) => {
