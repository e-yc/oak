...
async function addAndActivateRepo(oakPage: Page, repoPath: string): Promise<string> {
  const repoId = await oakPage.evaluate(async (pathToRepo: string) => {
  ...
  await expect
    .poll(
      () =>
        oakPage.evaluate(async (targetRepoId: string) => {
          const store = window.__store
          if (!store) {
            return 0
          }
  ...
  const worktreeId = await oakPage.evaluate(
...
test.describe('Large diff freeze repro', () => {
  ...
  test('opening a large single-file diff keeps the renderer responsive', async ({ oakPage }) => {
    await waitForSessionReady(oakPage)
    const fixture = createIsolatedLargeDiffRepo()
    const lineCount = Number(process.env.OAK_LARGE_DIFF_REPRO_LINES ?? '60000')
    if (!Number.isFinite(lineCount) || lineCount < 0) {
      throw new Error(
        `Invalid OAK_LARGE_DIFF_REPRO_LINES: ${process.env.OAK_LARGE_DIFF_REPRO_LINES}`
      )
    }
    ...
    try {
      ...
      const measurement = await oakPage.evaluate(
