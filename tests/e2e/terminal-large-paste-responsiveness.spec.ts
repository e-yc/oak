...
function largePastePayload(runId: string): string {
  return `OAK_LARGE_PASTE_${runId}_0123456789abcdef`.repeat(4096)
}
...
test.describe('large terminal paste responsiveness', () => {
  test('chunked keyboard paste keeps the renderer responsive while PTY writes are pending', async ({
    ...
    const scriptPath = path.join(testRepoPath, `.oak-large-paste-${runId}.cjs`)
    ...
    try {
      ...
      scriptStarted = true
      await waitForTerminalOutput(oakPage, `LARGE_PASTE_READY_${runId}`, 10_000)
      ...
      await installRendererHeartbeat(oakPage)
      await focusActiveTerminalInput(oakPage)

      const pasteKey = oakPage.keyboard.press(keyboardPasteChord())
      await expect
        .poll(
          async () =>
      ...
      const heartbeatBefore = await readRendererHeartbeat(oakPage)
      await oakPage.waitForTimeout(150)
      const heartbeatAfter = await readRendererHeartbeat(oakPage)
      expect(heartbeatAfter).toBeGreaterThan(heartbeatBefore)

      await pasteKey
      await waitForTerminalOutput(oakPage, doneLine, 20_000, 12_000)
      ...
      await stopRendererHeartbeat(oakPage).catch(() => undefined)
      if (scriptStarted) {
