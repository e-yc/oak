 ...
 * - PR #677 (`return to Oak landing screen after closing last terminal`)
...
test.describe('Tab Close Navigation', () => {
   ...
   */
  test('closing the active editor tab activates its visual neighbor', async ({ oakPage }) => {
    const worktreeId = await waitForActiveWorktree(oakPage)

    const fileIds = await openSeededEditorTabs(oakPage, worktreeId, [
      'package.json',
      'README.md',
      'tsconfig.json'
    ])
    ...
    await setActiveFile(oakPage, fileIds[1])
    await expect.poll(async () => getActiveFileId(oakPage), { timeout: 3_000 }).toBe(fileIds[1])

    await closeFile(oakPage, fileIds[1])
    ...
    await expect
      .poll(async () => getActiveFileId(oakPage), {
        timeout: 5_000,
   ...
   */
  test('closing the active diff tab activates a still-open neighbor', async ({ oakPage }) => {
    const worktreeId = await waitForActiveWorktree(oakPage)

    // Seed two editor tabs + one diff tab in the same worktree.
    const editorIds = await openSeededEditorTabs(oakPage, worktreeId, ['package.json', 'README.md'])
    expect(editorIds.length).toBe(2)

    const diffId = await oakPage.evaluate((wId) => {
      const store = window.__store
      if (!store) {
        return null
      }
    ...
    await expect.poll(async () => getActiveFileId(oakPage), { timeout: 3_000 }).toBe(diffId)

    await closeFile(oakPage, diffId!)
   ...
   */
  test('closing the last visible surface returns the app to Landing', async ({ oakPage }) => {
    const worktreeId = await waitForActiveWorktree(oakPage)
    ...
    const editorIds = await openSeededEditorTabs(oakPage, worktreeId, ['package.json'])
    expect(editorIds.length).toBe(1)

    await setActiveFile(oakPage, editorIds[0])
    await expect.poll(async () => getActiveFileId(oakPage), { timeout: 3_000 }).toBe(editorIds[0])
    ...
    await closeFile(oakPage, editorIds[0])
