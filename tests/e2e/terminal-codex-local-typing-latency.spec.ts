...
test.describe('local Codex terminal typing latency', () => {
  test('keeps Codex prompt typing responsive @local-real-codex', async ({ oakPage }, testInfo) => {
    test.skip(
      process.env.OAK_E2E_REAL_CODEX !== '1',
      'Set OAK_E2E_REAL_CODEX=1 to exercise the locally installed Codex TUI'
    )
    ...
    try {
      await sendToTerminal(oakPage, ptyId, launchCommand)
      await dismissCodexPromptsIfPresent(oakPage)
      await waitForCodexReady(oakPage)
      ...
      const prompt = `oak_codex_latency_${runId}`
      const latencies: number[] = []
      let typed = ''
      for (const char of prompt) {
        typed += char
        const start = performance.now()
        await oakPage.keyboard.type(char)
        await waitForPromptText(oakPage, typed)
