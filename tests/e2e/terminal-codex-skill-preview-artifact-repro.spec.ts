...
const RUN_REPRO = process.env.OAK_E2E_CODEX_SKILL_PREVIEW_REPRO === '1'
const EXPECT_NO_ARTIFACTS = process.env.OAK_E2E_EXPECT_NO_CODEX_SKILL_PREVIEW_ARTIFACTS === '1'
const FULLSCREEN_MIN_SIZE = { width: 1200, height: 760 }
const MIN_REPRO_DIFF_RATIO = 0.006
const MAX_CLEAN_DIFF_RATIO = 0.0015
const OAK_REPO_PATH = realpathSync(process.cwd())
...
const CODEX_SKILL_PREVIEW_RE = /Press enter to insert|esc to close|electron|oak-cli|oak-emulator/i
const SETUP_PANE_ACTIVITY_RE = /install-oak-skills|pnpm|Progress:|Packages:|Lockfile/i
...
async function addRealOakRepo(page: Page, repoPath: string): Promise<string> {
  return page.evaluate(async (repoPath) => {
    ...
    if (!repo) {
      throw new Error(`Real Oak repo did not load: ${repoPath}`)
    }
    ...
    if (!worktree) {
      throw new Error(`Real Oak worktree did not load: ${repoPath}`)
    }
...
async function createWorkspaceThroughComposer(page: Page, workspaceName: string): Promise<string> {
  ...
  await expect
    .poll(
      ...
      {
        timeout: 60_000,
        message: `Workspace ${workspaceName} did not appear in the real Oak repo`
      }
    )
    .not.toBeNull()
  ...
  await expect
    .poll(() => getActiveWorktreeId(page), {
      timeout: 30_000,
      message: 'Created real Oak workspace did not become active'
    })
    .toBe(createdId)
...
test.describe('Codex skill preview terminal artifact repro @headful', () => {
  ...
  test.skip(!RUN_REPRO, 'Set OAK_E2E_CODEX_SKILL_PREVIEW_REPRO=1 to run this repro.')
  ...
  test('captures the real Oak repo setup-split Codex skill preview overpaint before any click', async ({
    electronApp,
    oakPage
  }, testInfo) => {
    ...
    await setStableFullscreenWindow(electronApp, oakPage)
    await waitForSessionReady(oakPage)
    await addRealOakRepo(oakPage, OAK_REPO_PATH)
    ...
    const worktreeId = await createWorkspaceThroughComposer(oakPage, workspaceName)
    createdWorktreeIds.push(worktreeId)

    await ensureTerminalVisible(oakPage, 30_000)
    await waitForActiveTerminalManager(oakPage, 30_000)
    await waitForPaneCount(oakPage, 2, 60_000)
    await waitForPaneIdentitySnapshot(oakPage, 2)
    const webglActive = await forceTerminalWebgl(oakPage)
    test.skip(!webglActive, 'Codex skill preview artifact repro needs WebGL rendering')

    const leftPane = await focusLeftTerminalPane(oakPage)
    const rightPane = await getRightTerminalPane(oakPage)
    ...
    await dismissCodexPromptsIfPresent(oakPage, leftPane)
    await waitForPaneContent(oakPage, leftPane.tabId, leftPane.ptyId, CODEX_READY_RE, 60_000)
    await waitForPaneContent(
      oakPage,
      leftPane.tabId,
      leftPane.ptyId,
    ...
    await oakPage.waitForTimeout(CODEX_READY_SETTLE_MS)
    await waitForPaneVisibleContentChanges(oakPage, rightPane, 1, 8_000)

    await focusLeftTerminalPane(oakPage)
    await oakPage.keyboard.type('test $e', { delay: 70 })
    await waitForPaneContent(
      oakPage,
      leftPane.tabId,
      leftPane.ptyId,
    ...
    const evidence = await captureClickEvidence(oakPage, leftPane, testInfo)
