/**
 * E2E tests for the "Create Workspace" flow in Oak.
 *
...
test.describe('Create Workspace', () => {
  ...
  test('creates a worktree through the composer UI and activates it', async ({ oakPage }) => {
    const worktreeIdBefore = await getActiveWorktreeId(oakPage)
    ...
    try {
      ...
      await oakPage.getByRole('button', { name: 'New workspace', exact: true }).click()

      const dialog = oakPage.getByRole('dialog', { name: /Create (Workspace|Worktree)/i })
      ...
      await expect
        .poll(async () => worktreeExists(oakPage, workspaceName), {
          timeout: 10_000,
      ...
      await expect
        .poll(
          async () => {
            const id = await getActiveWorktreeId(oakPage)
  ...
  test('shows a failed workspace entry when worktree creation fails', async ({ oakPage }) => {
    ...
    try {
      ...
      const failedWorkspace = oakPage.getByRole('button', {
        name: new RegExp(`${workspaceName} No base branch found`)
      })
      await expect(failedWorkspace).toBeVisible()
      await expect(oakPage.getByText('Couldn’t create worktree')).toBeVisible()
      await expect(failedWorkspace).toContainText('No base branch found')
      await expect(oakPage.getByRole('button', { name: 'Retry' })).toBeVisible()
    } finally {
      await oakPage
        .evaluate(() => {
          ;(
  ...
  test('reuses a resolved pasted GitHub URL when quick create submits', async ({
    ...
    const url = 'https://github.com/e-yc/oak/pull/2049'
    ...
    try {
      ...
      await expect(dialog).toBeHidden({ timeout: 15_000 })
      await expect(oakPage.getByRole('option', { name: linkedWorkspacePattern })).toBeVisible({
        timeout: 10_000
      })
      await expect(oakPage.getByRole('option', { name: url })).toHaveCount(0)
      await expect(oakPage.getByText('Linked PR #2049')).toBeVisible()
      await expect
        .poll(() =>
          electronApp.evaluate(() => {
            const counters = globalThis as unknown as {
  ...
  test('names the workspace after the PR title when the pasted URL suggestion is selected', async ({
    ...
    const url = 'https://github.com/e-yc/oak/pull/2050'
    const linkedWorkspacePattern = new RegExp(title.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'))
    ...
    try {
      ...
      const suggestion = oakPage.getByRole('option', { name: linkedWorkspacePattern })
      ...
      await expect(oakPage.getByRole('option', { name: /https-github/i })).toHaveCount(0)
      await expect(oakPage.getByText('Linked PR #2050')).toBeVisible()
    } finally {
      await oakPage
        .evaluate(() => {
          window.__store?.getState().closeModal()
