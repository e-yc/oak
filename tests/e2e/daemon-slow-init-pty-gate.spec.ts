...
import { attachRepoAndOpenTerminal, createRestartSession } from './helpers/oak-restart'
...
{}, testInfo) => {
  ...
  try {
    ...
    process.env.OAK_E2E_DAEMON_INIT_DELAY_MS = String(DAEMON_INIT_DELAY_MS)
    try {
      ...
      delete process.env.OAK_E2E_DAEMON_INIT_DELAY_MS
    }
  } finally {
    if (secondApp) {
