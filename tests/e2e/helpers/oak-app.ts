/**
 * Shared Electron fixture for Oak E2E tests.
 *
...
import { getOakElectronLaunchArgs } from './electron-launch-args'
import { getE2ECompletedOnboardingProfile } from './e2e-completed-onboarding-profile'

type OakTestFixtures = {
  ...
  // developer's shell or already-running Oak instance.
  launchEnv: NodeJS.ProcessEnv
}

type OakWorkerFixtures = {
  /** Absolute path to the test git repo created by globalSetup. */
  testRepoPath: string
}

// Why: parse + warn at module scope so a bad OAK_E2E_SLOWMO_MS value logs once
// per worker instead of once per test (otherwise hundreds of lines per CI run).
const OAK_E2E_SLOWMO_MS_RAW = process.env.OAK_E2E_SLOWMO_MS
const OAK_E2E_SLOWMO_MS = ((): number => {
  if (OAK_E2E_SLOWMO_MS_RAW === undefined) {
    return 0
  }
  const parsed = Number(OAK_E2E_SLOWMO_MS_RAW)
  if (!Number.isFinite(parsed)) {
    console.warn(
      `[oak-e2e] OAK_E2E_SLOWMO_MS="${OAK_E2E_SLOWMO_MS_RAW}" is not a number; ignoring (using 0).`
    )
    return 0
  }
...
function shouldLaunchHeadful(testInfo: TestInfo): boolean {
  // Why: OAK_E2E_FORCE_HEADFUL lets a developer watch any spec in a real
  // window without retagging it `@headful` or switching projects.
  if (process.env.OAK_E2E_FORCE_HEADFUL === '1') {
    return true
  }
  return testInfo.project.metadata.oakHeadful === true
}

function forwardElectronProcessLogs(app: ElectronApplication, testInfo: TestInfo): void {
  if (process.env.OAK_E2E_FORWARD_APP_LOGS !== '1') {
    return
  }
...
function createSeededTestRepo(): string {
  ...
  const testRepoDir = realpathSync(mkdtempSync(path.join(os.tmpdir(), 'oak-e2e-repo-')))
  ...
  writeFileSync(
    ...
    `${JSON.stringify({ name: 'oak-e2e-test', version: '0.0.0', private: true }, null, 2)}\n`
  )
  ...
  const worktreeDir = path.join(testRepoDir, '..', `oak-e2e-worktree-${randomUUID()}`)
...
/**
 * Extended Playwright test with Oak-specific fixtures.
 *
 * `oakPage` — the main Oak renderer window.
 *
 ...
 */
export const test = base.extend<OakTestFixtures, OakWorkerFixtures>({
  ...
  electronApp: async ({ dismissOnboarding, launchEnv }, provideFixture, testInfo) => {
    ...
    const userDataDir = mkdtempSync(path.join(os.tmpdir(), 'oak-e2e-userdata-'))
    ...
    void _unused
    // Why: OAK_E2E_SLOWMO_MS adds a pause between every Playwright action so a
    // developer running with OAK_E2E_FORCE_HEADFUL=1 can actually watch what
    // the test does. Defaults to 0 (no slowdown) for normal runs.
    const slowMo = OAK_E2E_SLOWMO_MS
    // Why: OAK_E2E_RECORD_VIDEO=1 captures a webm of the renderer so a
    ...
    const recordVideoDir = process.env.OAK_E2E_RECORD_VIDEO === '1' ? testInfo.outputDir : null
    if (recordVideoDir) {
      mkdirSync(recordVideoDir, { recursive: true })
    }
    const app = await electron.launch({
      args: getOakElectronLaunchArgs(mainPath, headful),
      ...(slowMo > 0 ? { slowMo } : {}),
      ...(recordVideoDir ? { recordVideo: { dir: recordVideoDir } } : {}),
      // Why: keep NODE_ENV=development so window.__store is exposed and
      // dev-only helpers activate. OAK_E2E_USER_DATA_DIR overrides the usual
      // shared dev profile so every spec gets a clean persistence root.
      // Why: OAK_E2E_HEADLESS suppresses mainWindow.show() so the app
      // window stays hidden during test runs, avoiding focus stealing and
      // screen clutter. Playwright interacts via CDP regardless.
      // Why: OAK_E2E_HEADLESS suppresses mainWindow.show() for CI/headless
      // runs. OAK_E2E_HEADFUL overrides this for tests that need a visible
      ...
      env: {
        ...
        OAK_E2E_USER_DATA_DIR: userDataDir,
        ...((process.env.OAK_E2E_SSH_LOCALHOST === '1' || process.env.OAK_E2E_SSH_DOCKER === '1') &&
        !cleanEnv.OAK_RELAY_PATH
          ? { OAK_RELAY_PATH: path.join(process.cwd(), 'out', 'relay') }
          : {}),
        ...(headful ? { OAK_E2E_HEADFUL: '1' } : { OAK_E2E_HEADLESS: '1' })
      }
    })
  ...
  oakPage: async ({ sharedPage }, provideFixture) => {
