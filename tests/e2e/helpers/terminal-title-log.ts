...
export async function installRendererTitleLog(page: Page): Promise<void> {
  await page.evaluate(() => {
    const w = window as unknown as {
      __oakE2eTitleLog?: string[]
      __oakE2eTitleUnsubscribe?: () => void
    }
    ...
    recordTitles()
    w.__oakE2eTitleUnsubscribe = store.subscribe(recordTitles)
  })
}

export async function getRendererTitleLog(page: Page): Promise<string[]> {
  return page.evaluate(() => {
    const w = window as unknown as { __oakE2eTitleLog?: string[] }
    return w.__oakE2eTitleLog ?? []
  })
}
