...
test.describe('Terminal attention', () => {
  ...
  test('background terminal attention marks a tab unread and clears on focus', async ({
    ...
    const secondTabId = await createTerminalTab(oakPage)
    await waitForActiveTerminalManager(oakPage, 30_000)
    ...
    await expect
      .poll(async () => (await getUnreadTerminalTabIds(oakPage)).includes(secondTabId), {
        timeout: 10_000,
        message: 'Background tab did not become unread after BEL'
      })
      .toBe(true)

    const secondTabBell = oakPage
      .locator(
  ...
  test('a BEL on the focused tab raises, then clears on click', async ({ oakPage }) => {
    await waitForSessionReady(oakPage)
    await waitForActiveWorktree(oakPage)
    await ensureTerminalVisible(oakPage)
    await waitForActiveTerminalManager(oakPage, 30_000)

    const activeTabId = await getActiveTabId(oakPage)
    if (!activeTabId) {
      throw new Error('Expected an active terminal tab')
    }
    const activePtyId = await waitForActivePanePtyId(oakPage)
    await installRendererTitleLog(oakPage)
    ...
    expect((await getUnreadTerminalTabIds(oakPage)).includes(activeTabId)).toBe(true)
    const activeTabBell = oakPage
      .locator(
    ...
    await expect
      .poll(async () => (await getUnreadTerminalTabIds(oakPage)).includes(activeTabId), {
        timeout: 5_000,
  ...
  test('a BEL on the focused tab raises, then clears on plain Escape', async ({ oakPage }) => {
    await waitForSessionReady(oakPage)
    await waitForActiveWorktree(oakPage)
    await ensureTerminalVisible(oakPage)
    await waitForActiveTerminalManager(oakPage, 30_000)
    ...
    const activePaneKey = await getActivePaneKey(oakPage, activeTabId)
    const activePtyId = await waitForActivePanePtyId(oakPage)
    await installRendererTitleLog(oakPage)
    ...
    await expect
      .poll(async () => (await getUnreadTerminalPaneKeys(oakPage)).includes(activePaneKey), {
        timeout: 5_000,
    ...
    await focusActiveXterm(oakPage, activeTabId)
  ...
  // after restarting Oak with a Claude Code session open, clicking between
  ...
  test('mode bits replayed into xterm do not leak focus escapes to the shell', async ({
    ...
    try {
      ...
      // shell-startup BELs.
      const emittedFromXterm = await oakPage.evaluate(
        () =>
