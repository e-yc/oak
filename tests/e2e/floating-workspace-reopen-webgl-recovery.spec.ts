...
const TOGGLE_EVENT = 'oak-toggle-floating-terminal'
...
test.describe('floating workspace reopen WebGL recovery @headful', () => {
  test('reopening the floating workspace recovers a corrupted glyph atlas', async ({
    ...
    const shots = await setUpCorruptedFloatingTerminal(oakPage, 'REOPEN')
    ...
    expect(await instrumentRecoveryCounters(oakPage)).toBe(true)

    await toggleFloatingPanel(oakPage, false)
    ...
    const webglAttachedWhileClosed = await oakPage.evaluate((worktreeId) => {
      const state = window.__store?.getState()
      const tab = (state?.tabsByWorktree?.[worktreeId] ?? [])[0]
      const manager = tab ? window.__paneManagers?.get(tab.id) : null
      const diagnostics = manager?.getRenderingDiagnostics?.() ?? []
    ...
    await toggleFloatingPanel(oakPage, true)
    await settleRecoveryWindows(oakPage)

    const counters = await readRecoveryCounters(oakPage)
    const afterReopen = await screenshotFloatingTerminal(oakPage)
  ...
  test('window focus regain recovers the corrupted atlas (harness control)', async ({
    ...
    const shots = await setUpCorruptedFloatingTerminal(oakPage, 'CONTROL')
    test.skip(!shots, 'WebGL was not active or atlas corruption could not be injected')
    const { baseline, corrupted } = shots!
    expect(corrupted.equals(baseline)).toBe(false)
    ...
    const afterFocus = await screenshotFloatingTerminal(oakPage)
