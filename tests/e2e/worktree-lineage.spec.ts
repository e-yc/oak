...
test.describe('Worktree Lineage', () => {
  ...
  test('renders existing child lineage in the sidebar', async ({ oakPage }) => {
    const { parentId, childId } = await seedLineageScenario(oakPage)
    const parentRow = worktreeOption(oakPage, parentId)
    const childRow = worktreeOption(oakPage, childId)
    ...
    const positions = await oakPage.evaluate(
      ({ parentId, childId }) => {
    ...
    await oakPage.evaluate(async (childId) => {
  ...
  test('updates nested child preview status when the child terminal sleeps', async ({
    ...
    const childTabId = await seedWorkspaceLiveTerminal(oakPage, childId)
    await expect(childRow).toContainText('Active')

    await markWorkspaceTerminalSlept(oakPage, { worktreeId: childId, tabId: childTabId })
  ...
  test('shows parent and child agent rows while the parent workspace is active', async ({
    ...
    const parentAgentPrompt = await seedWorkspaceAgentStatus(oakPage, parentId, 'PARENT')
    const childAgentPrompt = await seedWorkspaceAgentStatus(oakPage, childId, 'CHILD')
