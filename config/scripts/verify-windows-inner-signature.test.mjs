...
function withTempFile(callback) {
  const dir = mkdtempSync(join(tmpdir(), 'oak-inner-signature-'))
  const filePath = join(dir, 'Oak.exe')
...
describe('verify-windows-inner-signature', () => {
  const originalExpectedSigners = process.env.OAK_WINDOWS_EXPECTED_SIGNERS
  const originalExpectedThumbprints = process.env.OAK_WINDOWS_EXPECTED_THUMBPRINTS

  beforeEach(() => {
    delete process.env.OAK_WINDOWS_EXPECTED_SIGNERS
    delete process.env.OAK_WINDOWS_EXPECTED_THUMBPRINTS
  })
  ...
  afterEach(() => {
    if (originalExpectedSigners === undefined) {
      ...
      process.env.OAK_WINDOWS_EXPECTED_SIGNERS = originalExpectedSigners
    }

    if (originalExpectedThumbprints === undefined) {
      delete process.env.OAK_WINDOWS_EXPECTED_THUMBPRINTS
    } else {
      process.env.OAK_WINDOWS_EXPECTED_THUMBPRINTS = originalExpectedThumbprints
    }
  })
  ...
  it('rejects missing, nonexistent, and directory executable paths before PowerShell', () => {
    ...
    expect(() => validateExecutablePath(join(tmpdir(), 'missing-Oak.exe'))).toThrow(
      /does not exist/
    )
  ...
  it('runs PowerShell with an argument array and fails on stderr or nonzero exit', () => {
    ...
    expect(getPowerShellSignatureJson('C:\\Path With Spaces\\Oak.exe', spawnSyncImpl)).toBe(
      JSON.stringify(validSignature)
    )
    expect(calls[0].command).toBe('pwsh')
    expect(calls[0].args).toContain('-Command')
    expect(calls[0].args.at(-1)).not.toBe('C:\\Path With Spaces\\Oak.exe')
    expect(calls[0].options).toEqual(
      expect.objectContaining({
        encoding: 'utf8',
        env: expect.objectContaining({
          OAK_WINDOWS_INNER_EXECUTABLE: 'C:\\Path With Spaces\\Oak.exe'
        })
      })
    )

    expect(() =>
      getPowerShellSignatureJson('Oak.exe', () => ({ status: 0, stdout: '{}', stderr: 'warning' }))
    ).toThrow(/stderr/)
    expect(() =>
      getPowerShellSignatureJson('Oak.exe', () => ({ status: 7, stdout: '', stderr: '' }))
    ).toThrow(/exit code 7/)
  })
