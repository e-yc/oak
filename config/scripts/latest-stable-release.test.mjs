...
describe('fetchReleases', () => {
  it('fetches all release pages', async () => {
    ...
    const releases = await fetchReleases('e-yc/oak', 'token', fetchImpl)

    expect(releases).toHaveLength(101)
    expect(fetchImpl).toHaveBeenNthCalledWith(
      1,
      'https://api.github.com/repos/e-yc/oak/releases?per_page=100&page=1',
      expect.any(Object)
    )
    expect(fetchImpl).toHaveBeenNthCalledWith(
      2,
      'https://api.github.com/repos/e-yc/oak/releases?per_page=100&page=2',
      expect.any(Object)
    )
  })
})
