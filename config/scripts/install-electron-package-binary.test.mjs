...
function mkTempProject() {
  const projectDir = mkdtempSync(join(tmpdir(), 'oak-install-electron-'))
...
function runInstallScript(projectDir, extraEnv = {}) {
  return spawnSync(process.execPath, ['config/scripts/install-electron-package-binary.mjs'], {
    ...
    env: {
      ...
      npm_config_platform: 'linux',
      npm_config_arch: 'x64',
      OAK_ELECTRON_PACKAGE_EXTRACTOR: join(projectDir, 'fake-extractor.cjs'),
      ...extraEnv
    }
  })
}
