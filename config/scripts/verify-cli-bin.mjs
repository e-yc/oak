...
export function verifyPackageCliBin({
  ...
  const binTarget = packageJson.bin?.oak
  if (typeof binTarget !== 'string' || binTarget.length === 0) {
    throw new Error('package.json must declare bin.oak')
  }

  const binPath = path.resolve(projectDir, binTarget)
  const stats = statSync(binPath)
  if (!stats.isFile()) {
    throw new Error(`bin.oak target is not a file: ${binTarget}`)
  }
  if (stats.size === 0) {
    throw new Error(`bin.oak target is empty: ${binTarget}`)
  }

  const content = readFileSync(binPath, 'utf8')
  if (!content.startsWith('#!/usr/bin/env node\n')) {
    throw new Error(`bin.oak target must start with a Node shebang: ${binTarget}`)
  }
  ...
  if (process.platform !== 'win32' && (stats.mode & 0o111) === 0) {
    if (!fixExecutable) {
      throw new Error(`bin.oak target is not executable: ${binTarget}`)
    }
