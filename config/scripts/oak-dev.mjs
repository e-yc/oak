...
const cliEntry = process.env.OAK_DEV_CLI_ENTRY_PATH ?? path.join(repoRoot, 'out', 'cli', 'index.js')

if (!existsSync(cliEntry)) {
  console.error("oak-dev: CLI not built yet. Run 'pnpm run build:cli' first.")
  process.exit(1)
}

process.env.OAK_USER_DATA_PATH = process.env.OAK_DEV_USER_DATA_PATH ?? getDefaultDevUserDataPath()

const electronExecutable = getElectronExecutable()
if (!process.env.OAK_APP_EXECUTABLE && isRunnableFile(electronExecutable)) {
  process.env.OAK_APP_EXECUTABLE = electronExecutable
