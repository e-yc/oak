...
const skillPath = join(projectDir, 'skills', 'oak-cli', 'SKILL.md')

function readSkill() {
  return readFileSync(skillPath, 'utf8')
}

describe('oak CLI skill guidance', () => {
  it('keeps independent worktree lineage separate from Git base selection', () => {
    const skill = readSkill()

    expect(skill).toContain('`--no-parent` only controls Oak lineage')
    expect(skill).toContain('omit `--base-branch` so Oak uses the repo default base')
  ...
  it('keeps browser injection guidance narrow and avoids literal secret examples', () => {
    ...
    expect(skill).toContain('`oak eval` expressions, or `oak exec` commands')
