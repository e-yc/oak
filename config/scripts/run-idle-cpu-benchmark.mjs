...
function createIdleRepo(worktreeCount) {
  const repoDir = mkdtempSync(path.join(os.tmpdir(), 'oak-idle-cpu-repo-'))
  ...
  writeFileSync(path.join(repoDir, 'README.md'), '# Oak idle CPU benchmark\n')
  writeFileSync(
  ...
  for (let i = 2; i <= worktreeCount; i += 1) {
    const worktreeDir = path.join(path.dirname(repoDir), `oak-idle-cpu-worktree-${i}-${Date.now()}`)
...
async function main() {
  ...
  const userDataDir = mkdtempSync(path.join(os.tmpdir(), 'oak-idle-cpu-userdata-'))
  ...
  const app = await electron.launch({
    ...
    env: {
      ...
      ...(options.headful ? { OAK_E2E_HEADFUL: '1' } : { OAK_E2E_HEADLESS: '1' })
    }
  })
