...
async function installElectronPackageBinary() {
  ...
  const tempDir = mkdtempSync(resolve(tmpdir(), 'oak-electron-'))
...
function getExtractorCommand(zipPath, extractDir) {
  if (process.env.OAK_ELECTRON_PACKAGE_EXTRACTOR) {
    return {
      file: process.execPath,
      args: [process.env.OAK_ELECTRON_PACKAGE_EXTRACTOR, zipPath, extractDir],
      label: `node ${process.env.OAK_ELECTRON_PACKAGE_EXTRACTOR}`
    }
  }

  if (osPlatform() === 'win32') {
    return {
      file: process.env.OAK_POWERSHELL_BIN || 'powershell',
      args: [
        '-NoProfile',
        '-NonInteractive',
        '-ExecutionPolicy',
  ...
  return {
    file: process.env.OAK_UNZIP_BIN || 'unzip',
