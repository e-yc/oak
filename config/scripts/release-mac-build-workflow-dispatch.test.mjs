...
describe('release mac build workflow dispatch', () => {
  it('dispatches the mac workflow and waits for the returned run id', async () => {
    const { fetch, requests } = createGitHubFetch([
      jsonResponse(200, {
        html_url: 'https://github.test/e-yc/oak/actions/runs/123',
        workflow_run_id: 123
      }),
      jsonResponse(200, {
        conclusion: 'success',
    ...
    expect(requests[0].path).toBe(
      '/repos/e-yc/oak/actions/workflows/release-mac-build.yml/dispatches'
    )
    ...
    expect(requests[1].path).toBe('/repos/e-yc/oak/actions/runs/123')
  })
  ...
  it('finds the dispatched run by release tag and parent run id when GitHub returns no body', async () => {
      ...
      jsonResponse(200, {
        workflow_runs: [
          ...
          },
          {
            ...
            display_title: runTitle,
            html_url: 'https://github.test/e-yc/oak/actions/runs/124',
            id: 124
          }
        ]
      }),
    ...
    expect(requests[1].path).toBe('/repos/e-yc/oak/actions/workflows/release-mac-build.yml/runs')
  ...
  it('fails when the isolated mac workflow does not succeed', async () => {
    const { fetch } = createGitHubFetch([
      jsonResponse(200, {
        html_url: 'https://github.test/e-yc/oak/actions/runs/125',
        workflow_run_id: 125
      }),
      jsonResponse(200, {
        conclusion: 'failure',
