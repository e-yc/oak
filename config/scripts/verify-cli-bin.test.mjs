...
/** Builds a temporary Oak-style project fixture with a compiled CLI entrypoint. */
function makeProjectWithCli(
  content,
  { mode = 0o755, rootPackageType, writeOutPackageJson = true } = {}
) {
  const projectDir = mkdtempSync(path.join(tmpdir(), 'oak-cli-bin-'))
  ...
  writeFileSync(
    ...
    JSON.stringify({ bin: { oak: './out/cli/index.js' }, type: rootPackageType }),
    'utf8'
  )
...
describe('verifyPackageCliBin', () => {
  ...
  it('rejects an empty package bin target', () => {
    ...
    expect(() => verifyPackageCliBin({ projectDir })).toThrow('bin.oak target is empty')
  })

  it('rejects package bin targets without a Node shebang', () => {
    const { projectDir } = makeProjectWithCli('console.log("oak")\n')
