...
export function shouldPreserveEnglishValue(enValue, key = '') {
  ...
  if (/^https?:\/\//.test(enValue) || enValue.startsWith('oak://')) {
    return true
  }
...
function applyCjkLatinTermSpacing(localeValue, locale) {
  ...
  if (locale === 'ko') {
    ...
    // boundary — re-glue "Oak 에"/"PR 을"/"에서는" but keep "Jira 이슈"/"Oak 로고"/"agent 에뮬레이터".
...
export function repairTranslatedValue({ key, enValue, localeValue, locale }) {
  ...
  if (enValue.includes('oak://')) {
    result = result.replace(/虎鲸:\/\//g, 'oak://')
  }

  if (enValue === 'Oak' || enValue.startsWith('Oak ')) {
    result = result
      .replaceAll('虎鲸', 'Oak')
      .replaceAll('逆戟鲸', 'Oak')
      .replaceAll('シャチ', 'Oak')
  ...
  if (enValue.includes('oak://')) {
    result = result.replace(/シャチ:\/\//g, 'oak://')
  }
...
export function repairCatalog(enCatalog, localeCatalog, locale) {
  ...
  if (localeCatalog.menu) {
    if (locale === 'zh') {
      if (localeCatalog.menu.exploreOak !== '探索 Oak') {
        localeCatalog.menu.exploreOak = '探索 Oak'
        repaired += 1
      }
      if (localeCatalog.menu.gettingStarted !== 'Oak 入门') {
        localeCatalog.menu.gettingStarted = 'Oak 入门'
        repaired += 1
      }
    }
    if (locale === 'ko') {
      if (localeCatalog.menu.exploreOak !== 'Oak 둘러보기') {
        localeCatalog.menu.exploreOak = 'Oak 둘러보기'
        repaired += 1
      }
      if (localeCatalog.menu.gettingStarted !== 'Oak 시작하기') {
        localeCatalog.menu.gettingStarted = 'Oak 시작하기'
        repaired += 1
      }
    }
  }
