  ...
  it('fixes Korean round 5 review, integration, and search keyword regressions', () => {
    ...
    expect(
      repairTranslatedValue({
        ...
        enValue: 'Install the Oak skill so agents know to use the Oak CLI.',
        localeValue: '에이전트가 Oak CLI 사용 방법을 알 수 있도록 Oak 기술을 설치합니다.',
        locale: 'ko'
      })
    ).toBe('agents가 Oak CLI를 사용하도록 Oak 스킬을 설치하세요.')
    expect(
      repairTranslatedValue({
