  ...
  it('includes both mac updater ZIP names for the tag version', () => {
    expect(getRequiredReleaseAssetNames('v1.4.27')).toEqual(
      expect.arrayContaining([
        'Oak-1.4.27-mac.zip',
        'Oak-1.4.27-mac.zip.blockmap',
        'Oak-1.4.27-arm64-mac.zip',
        'Oak-1.4.27-arm64-mac.zip.blockmap'
      ])
    )
  })
  ...
  it('includes x64 and arm64 Linux assets', () => {
    expect(getRequiredReleaseAssetNames('v1.4.27')).toEqual(
      expect.arrayContaining([
        ...
        'oak-linux.AppImage',
        'oak-linux-arm64.AppImage',
        'oak-ide_1.4.27_amd64.deb',
        'oak-ide_1.4.27_arm64.deb',
        'oak-ide-1.4.27.x86_64.rpm',
        'oak-ide-1.4.27.aarch64.rpm'
      ])
    )
  })
})
  ...
  it('extracts relative and absolute manifest asset names', () => {
    expect(
      extractManifestAssetNames(
        [
          'files:',
          '  - url: Oak-1.4.27-arm64-mac.zip',
          '  - url: https://example.com/downloads/oak-windows-setup.exe',
          'path: oak-linux.AppImage'
        ].join('\n')
      )
    ).toEqual(['Oak-1.4.27-arm64-mac.zip', 'oak-windows-setup.exe', 'oak-linux.AppImage'])
  })
})
...
describe('verifyRequiredReleaseAssets', () => {
  it('fails when a manifest-referenced asset has not been uploaded', async () => {
    ...
    const assets = required.filter((name) => name !== 'Oak-1.4.27-arm64-mac.zip')
    ...
    await expect(
      verifyRequiredReleaseAssets({ repo: 'e-yc/oak', tag, token: 'token' })
    ).rejects.toThrow('Missing: Oak-1.4.27-arm64-mac.zip')
  ...
  it('checks assets referenced by the Linux arm64 updater manifest', async () => {
      ...
      .mockResolvedValueOnce(
        jsonResponse(
          [
            'version: 1.4.27',
            'files:',
            '  - url: oak-linux-arm64.AppImage.blockmap',
            'path: oak-linux-arm64.AppImage'
          ].join('\n')
        )
      )
      .mockResolvedValue(jsonResponse('version: 1.4.27\n'))
    ...
    await expect(
      verifyRequiredReleaseAssets({ repo: 'e-yc/oak', tag, token: 'token' })
    ).rejects.toThrow('Missing: oak-linux-arm64.AppImage.blockmap')
