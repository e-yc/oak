...
function createGitRepo() {
  const repoDir = mkdtempSync(path.join(tmpdir(), 'oak-wayland-gpu-repo-'))
...
async function runValidation(mode) {
  ...
  const userDataPath = mkdtempSync(path.join(tmpdir(), 'oak-wayland-gpu-userdata-'))
  ...
  try {
    ...
    app.process().stderr?.on('data', (chunk) => {
      ...
      if (process.env.OAK_WAYLAND_GPU_VERBOSE === '1') {
