...
const binaryPath = path.join(packagePath, '.build', 'release', 'oak-computer-use-macos')
const appPath = path.join(packagePath, '.build', 'release', 'Oak Computer Use.app')
const appExecutablePath = path.join(appPath, 'Contents', 'MacOS', 'oak-computer-use-macos')
...
const bundleId = process.env.OAK_COMPUTER_MACOS_BUNDLE_ID ?? 'com.stablyai.oak.computer-use'
const displayName = 'Oak Computer Use'
...
function buildUniversalBinary() {
  const builtBinaries = universalTriples.map((triple) => {
    ...
    return path.join(packagePath, '.build', triple, 'release', 'oak-computer-use-macos')
  })
...
function codesignArgs(identity, targetPath) {
  ...
  if (process.env.OAK_MAC_RELEASE === '1' && identity !== '-') {
...
function resolveSigningIdentity() {
  const explicitIdentity = process.env.OAK_COMPUTER_MACOS_SIGN_IDENTITY ?? process.env.CSC_NAME
  if (explicitIdentity) {
  ...
  if (process.env.OAK_MAC_RELEASE !== '1' && developmentMatch) {
...
<dict>
  ...
  <string>oak-computer-use-macos</string>
  ...
  <string>Oak Computer Use needs Accessibility permission to read and interact with app interfaces when you ask Oak to use apps.</string>
  <key>NSScreenCaptureUsageDescription</key>
  <string>Oak Computer Use needs Screen Recording permission to capture app windows when you ask Oak to inspect your screen.</string>
</dict>
</plist>
`
}
