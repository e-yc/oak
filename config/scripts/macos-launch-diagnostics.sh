#!/usr/bin/env bash
# Capture one-shot macOS launch diagnostics for a published Oak release.
#
# Usage:
#   OAK_DIAGNOSTIC_TAG=v1.4.42-rc.1 bash config/scripts/macos-launch-diagnostics.sh
#   bash config/scripts/macos-launch-diagnostics.sh --tag v1.4.42-rc.1
set -euo pipefail

REPO="${OAK_DIAGNOSTIC_REPO:-e-yc/oak}"
TAG="${OAK_DIAGNOSTIC_TAG:-}"
KEEP=0
...
if [[ -z "$TAG" ]]; then
  echo "Set OAK_DIAGNOSTIC_TAG or pass --tag vX.Y.Z-rc.N." >&2
  exit 2
fi

ARCH="$(uname -m)"
case "$ARCH" in
  arm64) ASSET="oak-macos-arm64.dmg" ;;
  x86_64) ASSET="oak-macos-x64.dmg" ;;
  *)
...
WORK_DIR="$(mktemp -d "${TMPDIR:-/tmp}/oak-launch-diagnostics.XXXXXXXX")"
OUT_DIR="$WORK_DIR/output"
MOUNT_DIR="$WORK_DIR/mount"
APP_DIR="$WORK_DIR/Oak.app"
DMG_PATH="$WORK_DIR/$ASSET"
mkdir -p "$OUT_DIR" "$MOUNT_DIR"

if [[ -d "$HOME/Desktop" ]]; then
  ZIP_PATH="$HOME/Desktop/oak-launch-diagnostics-${TAG}-${TIMESTAMP}.zip"
else
  ZIP_PATH="$PWD/oak-launch-diagnostics-${TAG}-${TIMESTAMP}.zip"
fi

diag_log() {
  printf '[oak-diagnostics] %s\n' "$*"
}
...
write_environment_report() {
  {
    ...
    echo "$SHELL"
    echo
    echo "## current Oak processes before diagnostics"
    pgrep -fl 'Oak|oak' || true
  } >"$OUT_DIR/environment.txt"
}

ensure_no_existing_oak() {
  local existing
  existing="$(pgrep -x Oak || true)"
  if [[ -z "$existing" ]]; then
    return 0
  fi

  {
    echo "An Oak process is already running. Close Oak and run this script again."
    echo
    ps -p "$(printf '%s' "$existing" | paste -sd, -)" -o pid=,ppid=,command= || true
  } | tee "$OUT_DIR/existing-oak-process.txt" >&2
  exit 2
}
...
start_log_stream() {
  local file="$1"
  local predicate='process == "Oak" OR eventMessage CONTAINS[c] "Oak" OR eventMessage CONTAINS[c] "com.stablyai.oak"'
...
latest_oak_pid_for_app() {
  pgrep -nf "$APP_DIR/Contents/MacOS/Oak" || true
}
...
wait_for_probe() {
  ...
  for i in $(seq 1 200); do
    local app_pid
    app_pid="$(latest_oak_pid_for_app)"
...
run_launchservices_probe() {
  ...
  if [[ -n "$extra_env_name" ]]; then
    open -n -W \
      ...
      --env OAK_STARTUP_DIAGNOSTICS=trace \
      --env OAK_STARTUP_DIAGNOSTICS_TRACE_LIMIT=30000 \
      --env OAK_STARTUP_DIAGNOSTICS_FILE="$bootstrap_file" \
...
run_direct_exec_probe() {
  ...
  ELECTRON_ENABLE_LOGGING=1 \
    OAK_STARTUP_DIAGNOSTICS=trace \
    OAK_STARTUP_DIAGNOSTICS_TRACE_LIMIT=30000 \
    OAK_STARTUP_DIAGNOSTICS_FILE="$bootstrap_file" \
    "$APP_DIR/Contents/MacOS/Oak" >"$stdout_file" 2>"$stderr_file" &
  local runner_pid="$!"
  wait_for_probe "$runner_pid" "$label"
  echo "ended_utc=$(date -u '+%Y-%m-%dT%H:%M:%SZ')" >>"$OUT_DIR/$label.meta"
  stop_log_stream "$stream_pid"
...
run_launchservices_probe "launchservices-bypass-lock" "OAK_BYPASS_SINGLE_INSTANCE_LOCK" "1"
run_direct_exec_probe
write_system_log_snapshot
package_results
