#!/usr/bin/env node
/**
 * Terminal E2E helpers for agent-browser + CDP testing against a running Oak
 ...
 *
 * Usage:
 *   import { OakTerminal } from './terminal-e2e-helpers.mjs'
 *
 *   const term = new OakTerminal(9444)       // CDP port
 *   await term.connect()
...
export class OakTerminal {
  /** @param {number} cdpPort — the --remote-debugging-port used when launching Oak */
  constructor(cdpPort) {
    this.port = cdpPort
  }
   ...
   */
  probePtyIdWithScreenshot(maxId = 10, screenshotPath = tempScreenshotPath('oak-pty-probe.png')) {
    for (let i = 1; i <= maxId; i++) {
  ...
  /**
   * Take a screenshot of the Oak window.
   *
   * @param {string} path — output file path
   * @returns {string} the screenshot path
   */
  screenshot(path = tempScreenshotPath('oak-terminal.png')) {
    ab(this.port, ['screenshot', path])
    return path
  }

  /**
   * Open a new terminal tab in Oak.
   * @returns {void}
   */
  newTerminal() {
  ...
  readLang(ptyId) {
    ...
    return this.screenshot(tempScreenshotPath('oak-lang-check.png'))
  }
}
...
if (process.argv[1]?.endsWith('terminal-e2e-helpers.mjs')) {
  ...
  const term = new OakTerminal(port)
  console.log('Connecting to Oak on CDP port', port, '...')
  term.connect()
