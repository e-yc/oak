...
function getPackagedCliPath(appDir) {
  if (process.platform === 'darwin' || appDir.endsWith('.app')) {
    return join(appDir, 'Contents', 'Resources', 'bin', 'oak')
  }
  if (process.platform === 'win32') {
    return join(appDir, 'resources', 'bin', 'oak.cmd')
  }
  return join(appDir, 'resources', 'bin', 'oak-ide')
}

const appDir = resolve(readAppDirArg(process.argv.slice(2)))
const tempRoot = await mkdtemp(join(tmpdir(), 'oak-packaged-cli-smoke-'))
