...
const wrapperPath = path.join(projectDir, 'config', 'scripts', 'oak-dev.mjs')

describe('oak-dev package bin', () => {
  it('uses a Node entrypoint for cross-platform package installs', () => {
    expect(packageJson.bin['oak-dev']).toBe('./config/scripts/oak-dev.mjs')
    expect(readFileSync(wrapperPath, 'utf8')).toMatch(/^#!\/usr\/bin\/env node\n/)
  })

  it('runs the dev CLI through Node without requiring Bash', () => {
    const root = mkdtempSync(path.join(tmpdir(), 'oak-dev-bin-'))
    ...
    writeFileSync(
      ...
      [
        ...
        '  userDataPath: process.env.OAK_USER_DATA_PATH,',
        '  appExecutable: process.env.OAK_APP_EXECUTABLE',
        '}));'
      ].join('\n'),
      'utf8'
    )
    ...
    execFileSync(process.execPath, [wrapperPath, '--help'], {
      env: {
        ...process.env,
        OAK_DEV_CLI_ENTRY_PATH: cliEntry,
        OAK_DEV_USER_DATA_PATH: path.join(root, 'user-data'),
        OAK_APP_EXECUTABLE: path.join(root, 'Electron')
      },
      stdio: 'ignore'
    })
