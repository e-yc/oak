...
set -euo pipefail
base="$(mktemp -d /tmp/oak-apphang-repro.XXXXXX)"
repo="$base/repo"
mkdir -p "$repo"
cd "$repo"
git init -q
...
mkdir -p src docs
printf '# Oak Windows AppHang repro\n' > README.md
...
printf 'plain folder for Oak AppHang repro\n' > "$plain/README.txt"
...
export function createCompletedOnboardingProfile(userDataDir) {
  ...
  writeFileSync(path.join(userDataDir, 'oak-data.json'), `${JSON.stringify(profile, null, 2)}\n`)
}
