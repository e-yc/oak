...
export function createGpuUserDataDirectory(gpuMode) {
  const userDataDir = mkdtempSync(path.join(os.tmpdir(), `oak-apphang-${gpuMode}-userdata-`))
...
export function launchDevApp({ cdpPort, userDataDir }) {
  ...
  Object.assign(env, {
    ...
    OAK_DEV_USER_DATA_PATH: userDataDir,
...
export async function installRendererProbe(page) {
  await page.evaluate(() => {
    if (globalThis.__oakApphangProbe) {
      return
    }
    const probe = {
      intervalMs: 50,
    ...
    globalThis.__oakApphangProbe = { probe, timer }
  })
}
      ...
      () =>
        page.evaluate(async () => {
          ...
          return {
            ...
            rendererProbe: globalThis.__oakApphangProbe?.probe ?? null,
