...
describe('computer-use skill guidance', () => {
  it('keeps web-app targeting on the computer-use surface', () => {
    ...
    expect(skill).toContain('Use this skill for desktop UI through `oak computer`')
    expect(skill).toContain('operate the desktop browser app/window that contains the page')
    expect(skill).not.toContain('oak goto')
    expect(skill).not.toContain('oak snapshot')
    expect(skill).not.toContain('oak click')
    expect(skill).not.toContain('oak fill')
