  ...
  it.skipIf(process.platform === 'win32')(
    ...
    () => {
      ...
      try {
        ...
        writeFakeElectronRebuild(projectDir, { logPathEnv: 'OAK_REBUILD_TEST_LOG' })
        writeFakeLoadableNodePty(projectDir)
        writeNodePtyPatchFile(projectDir)

        const result = runRebuildScript(projectDir, {
          OAK_REBUILD_TEST_LOG: rebuildLogPath
        })
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    () => {
      ...
      try {
        ...
        expect(result.stdout).toContain("expected build/Release so Oak's node-pty patch is active")
...
function mkTempProject() {
  const projectDir = mkdtempSync(join(tmpdir(), 'oak-rebuild-native-deps-'))
...
function runRebuildScript(projectDir, extraEnv = {}) {
  ...
  for (const key of Object.keys(env)) {
    if (
      key.toLowerCase() === 'oak_strict_electron_install' ||
