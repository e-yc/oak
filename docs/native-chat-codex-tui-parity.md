...
This note maps Codex TUI behavior to Oak native chat on branch
`inspect/pr-5824-native-chat`. It is intentionally concrete: the current Oak
...
## Current Oak Architecture

Oak native chat currently sends through the hosted terminal PTY. The composer
...
- Native chat discovers skills through Oak's skills IPC with the active
...
- Native chat has interactive cards sourced from Oak's existing agent status
  hooks. This is good for common question/approval flows, but it is not the full
  Codex approval overlay model.

Recommended route:

- Keep PTY fallback for anything not represented in Oak hooks.
