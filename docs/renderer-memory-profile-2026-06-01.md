...
This profile investigated high Oak renderer memory while working in
`/Users/nwparker/oak/workspaces/oak/goal`. The user suspected the browser
...
- Oak runtime was reachable through the packaged CLI fallback. The public
  `/usr/local/bin/oak` shim pointed at a removed development app path, so it
  failed before contacting the runtime.
- `oak tab list --worktree all --json` returned `tabs: []`. There were no live
  Oak browser tabs in the measured session.
...
- `vmmap -summary` on the renderer showed about 218 MB physical footprint and a
  ...
  library mappings.
- `oak terminal list --worktree active --json` showed the active Codex terminal
...
2. The browser-pane retention fix is still useful: inactive worktree browser
   ...
   state remains in Oak, and automation-visible webviews stay mounted so
   agent-browser can keep driving them.
3. Resource Usage was using `app.getAppMetrics().memory.workingSetSize` for
   Oak app buckets. On macOS this can count large shared Electron/Chromium
...
- Browser panes now mount their backing webview only when the pane is active or
  ...
  without sleeping the main Oak renderer.
...
The current packaged Oak app was not running this worktree's patched code
...
`/usr/local/bin/oak` command was still broken because it was a regular
...
conflicts. That meant Settings could not replace an Oak-owned stale launcher,
forcing profiling to use the packaged CLI fallback.

The follow-up fix teaches the installer to recognize only generated Oak Unix
...
and host process output by hand. This branch adds `oak diagnostics memory`,
...
run with `--json`, including host memory, Oak app process buckets, worktree
