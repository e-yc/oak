...
Credential files Oak writes on Windows (runtime env auth store, device registry,
...
- **Directories — async + path-cached for the process lifetime.**
  ...
  - _Known limitation:_ a directory that is deleted and recreated mid-process is
    not re-hardened until the next restart. The `.oak` secure dirs are not
...
Pre-req: a Windows client paired to a remote `oak serve` runtime.

Watcher (PowerShell, run before launching Oak):
...
3. Write a credential (e.g. sign in / register a device so a secure file is
   ...
   ```powershell
   icacls "$env:APPDATA\oak\oak-environments.json"
   ```
