...
5. Link to the most specific Linear settings URL Oak can build from `organizationUrlKey`, with global fallbacks when the workspace is unknown or `all`.
...
- Do not create, join, unarchive, or grant access to Linear teams from Oak. "Add team access" means paste a key whose owner/scope can already see the team.
...
2. Add `LinearApiKeyDialog` in `src/renderer/src/components/linear-api-key-dialog.tsx`.
   ...
   - Guidance:
     ...
     - Prefer full access when Oak should show every team the account can access in that workspace.
