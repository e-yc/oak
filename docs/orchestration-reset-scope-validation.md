...
6. Add CLI parser tests that `oak orchestration reset` calls RPC with `all: true`, and that explicit flags are passed through unchanged for RPC validation.

## Data Flow

- CLI no-flag path: `oak orchestration reset` -> CLI handler sends `{ all: true }` -> RPC schema validates -> handler calls `resetAll`.
...
- Residual risks: CLI users may still accidentally clear all state with `oak orchestration reset` because the compatibility shortcut remains, but direct RPC callers can no longer do so accidentally and CLI behavior is now explicit in the caller.
