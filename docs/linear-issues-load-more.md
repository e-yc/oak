...
- `src/main/ipc/linear.ts:115` and `src/main/runtime/oak-runtime.ts:12415` clamp plain issue list reads to 50. A 36-row "Load more" step would request 72 but receive at most 50 unless these clamps change.
...
Let users browse more issues from the Linear Issues All tab inside Oak with an explicit "Load more" action. Apply the same plain-list behavior to Assigned, Created, and Completed because they share the same store/runtime/main read path.
...
- Copy is direct: users should understand Oak can fetch more without opening Linear.
