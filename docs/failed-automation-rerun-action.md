...
- Failed Oak automation run details show only the disabled/open-target action when no workspace launched, so a user who sees a `dispatch_failed` error has no local recovery action in the failed-run view: `src/renderer/src/components/automations/AutomationsPage.tsx:1858`.
...
Add an easy rerun button to Oak automation run detail pages for failed launch/recovery statuses. The button creates a fresh manual run for the same automation through the existing `runNow` path.
...
1. Add a small pure predicate near the run view state code, for example `canRerunAutomationRun({ automation, run })`.
   - Return `true` only when an Oak automation still exists, `run.automationId === automation.id`, and the run status is one of `dispatch_failed`, `skipped_unavailable`, or `skipped_needs_interactive_auth`.
...
- `runNow` itself does not broadcast `oak:automations-changed`; the initiating page must call `refresh()` after the IPC resolves. Other windows update only when dispatch result handling fires the existing event or when focus/visibility refresh runs.
...
3. Render the new action in the selected Oak run detail header.
