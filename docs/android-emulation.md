...
Oak ships a built-in mobile emulator surface (live pane + `oak emulator` CLI +
...
`oak-emulator-android` agent skill.
...
- **RPC** is declared in `src/main/runtime/rpc/methods/emulator.ts` and
  implemented in `src/main/runtime/oak-runtime-emulator.ts`.
...
- **Full AVD lifecycle**: discover installed AVDs via the SDK, boot/shutdown
  them from Oak, and attach to already-running emulators + physical `adb`
  devices.
...
- A dedicated `skills/oak-emulator-android/SKILL.md`.
...
`src/main/runtime/oak-runtime-emulator.ts`, `src/cli/specs/emulator.ts`, and
`src/cli/handlers/emulator.ts`:

- `oak emulator list` gains a **platform column** and shows iOS + Android
...
New `skills/oak-emulator-android/SKILL.md`, mirroring
`skills/oak-emulator/SKILL.md`:

- Prerequisites: Android Studio / SDK installed, `ANDROID_HOME` (or
  `ANDROID_SDK_ROOT`) set, at least one AVD or a connected device.
- The `oak emulator ...` command table (shared CLI; Android examples).
- Gotchas: Oak handles pixel ↔ normalized conversion (agents always pass 0–1);
...
- Register it the same way `oak-emulator` is registered.
...
- Boot an AVD from Oak; confirm the live pane streams and is responsive.
...
3. Android device management (`android-sdk-discovery`, `adb-devices`,
   ...
   Android devices in `oak emulator list`.
...
8. Packaging (`scrcpy-server.jar` resource) + the `oak-emulator-android` skill.
9. Tests at each step; typecheck + lint; Electron validation on Windows + macOS.

## Open decisions

- Whether `oak emulator install`/`launch`/`logcat` should also be exposed for
  iOS later (iOS install is `xcrun simctl install`); v1 leaves them
  Android-only via capability flags.
- Whether to expose an explicit `oak emulator boot <avd>` verb vs. folding boot
