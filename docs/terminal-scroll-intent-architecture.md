...
an escalation point for Oak because private internals increase xterm upgrade
risk.

## Current Oak Risk Points

The current Oak branch already has several independent scroll actors:
...
The design must preserve Oak's terminal performance priorities:
...
- Whether Oak should patch private xterm bottom-scroll internals. The reference
  pattern does this for strict ownership, but Oak should first try public API
