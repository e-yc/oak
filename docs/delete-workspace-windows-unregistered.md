...
GitHub issue [#5864](https://github.com/e-yc/oak/issues/5864) reports that Oak on Windows v0.14.80 fails to delete a workspace created from a project `+` button:

`Error invoking remote method 'worktrees:remove': Error: Refusing to delete unregistered worktree path: C:/Users/andy/oak/workspaces/ops-tools/packaging-improvements-2`
...
- Runtime RPC delete has the same registered-worktree gate in `src/main/runtime/oak-runtime.ts`.
- Windows create/list coverage exists in `src/main/ipc/worktrees-windows.test.ts`, but Windows delete coverage is missing.

## Root Cause

Delete is right to refuse arbitrary paths. This bug is a false negative in the proof step: Oak asks Git for the authoritative registered worktree list, but the list does not contain an entry equivalent to the project-created target.
...
- Already-missing Oak-known worktrees still clean metadata only.
- Orphaned Oak-created worktree directories still require proof through the `.git` file before recursive deletion.
...
- Unit:
  ...
  - `pnpm vitest run src/main/runtime/oak-runtime.test.ts --testNamePattern "worktree removal|selected WSL project runtime|unregistered delete|contains another registered|already-missing"`
