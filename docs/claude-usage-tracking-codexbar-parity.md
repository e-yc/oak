...
Make Oak's Claude usage limit tracking behave like CodexBar's Claude implementation: automatic, resilient, and source-aware internally, without asking users to choose between OAuth, CLI, or other data sources.
...
- Show a specific, actionable status only when Oak cannot recover automatically.
...
## Current Oak Behavior To Preserve Or Change

Oak currently reads Claude OAuth credentials and calls Anthropic's OAuth usage endpoint:
...
3. Web usage source later, only if Oak intentionally adopts CodexBar's web/cookie machinery.
...
- After delegation, re-run Oak's existing credential read order and retry the OAuth usage request once.
- Keep this disabled for cases where Oak already knows live Claude is using/rotating that credential set.

### 5. Preserve Managed Live-Session Safety, But Change The State

Current Oak managed-account behavior avoids rotating refresh tokens while a live Claude terminal may rotate them. Keep that safety rule.
...
- Claude rotated credentials and Oak has stale cached data.
